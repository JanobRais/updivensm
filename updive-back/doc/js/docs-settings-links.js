...
function promptSettingUrl(e) {
    e.preventDefault();
    let UpdiveNSMUrl = prompt("Enter your UpdiveNSM URL to get direct settings link.\nNote: This URL is only stored in your browser.", localStorage.getItem('UpdiveNSM_url') || '');

    if (! isValidHttpUrl(UpdiveNSMUrl)) {
        alert("Invalid url, must start with http:// or https://")
        return false;
    }

    wrapSettingsLinks(UpdiveNSMUrl);
    return false;
}


function wrapSettingsLinks(UpdiveNSMUrl) {
    // fetch saved url
    if (! UpdiveNSMUrl) {
        UpdiveNSMUrl = localStorage.getItem('UpdiveNSM_url');
    }

    if (UpdiveNSMUrl) {
        localStorage.setItem('UpdiveNSM_url', UpdiveNSMUrl);
        UpdiveNSMUrl = UpdiveNSMUrl.replace(/\/+$/i, ''); // trim trailing /
        [].forEach.call(document.querySelectorAll('.admonition.setting>.admonition-title'), function (el) {
            ...
            link.href = UpdiveNSMUrl + '/settings/' + el.dataset.setting_url;
        ...
        [].forEach.call(document.querySelectorAll('.admonition.setting>.admonition-title'), function (el) {
            ...
            link.innerText = 'https://<your UpdiveNSM url>/' + el.dataset.setting_url;
            el.innerText = '';
            el.appendChild(link);
        });
    }
...
document.addEventListener('DOMContentLoaded', function () {
    wrapSettingsLinks(findGetParameter('UpdiveNSM_url'));
}, false);
