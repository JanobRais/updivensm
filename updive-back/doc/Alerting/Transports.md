...
Transports are located within `UpdiveNSM/Alert/Transport/` and can be
...
Device's SNMP configuration and also every UpdiveNSM user that has at
least `read` permissions on the entity that is to be alerted.

At the moment UpdiveNSM only supports Port or Device permissions.
