...
Another way to extend a template, is to reuse the content of other Alert templates in UpdiveNSM. This can be done by leveraging the AlertTemplate database model. All inside the included template needed variables, need to be passed through to the second parameter (e.g.```["alert" => $alert]```) of the method Blade:render(). 
...
```php
@if ($alert->transport == 'mail')<a href="https://my.UpdiveNSM.install/device/device={{ $alert->hostname }}/">{{ $alert->hostname }}</a>
@else
{{ $alert->hostname }}
@endif
```
...
access. Anyone using the signed url will be able to view the graph.
 ...
 - Your UpdiveNSM web must be accessible from the location where the graph is viewed.
...
```html
<img class="UpdiveNSM-graph" src="https://UpdiveNSM.org/graph?from=1662176216&amp;height=250&amp;id=20425&amp;to=1662219416&amp;type=port_bits&amp;width=700&amp;signature=f6e516e8fd893c772eeaba165d027cb400e15a515254de561a05b63bc6f360a4">
```

Specific graph using url input:

```php
@signedGraphTag('https://UpdiveNSM.org/graph.php?type=device_processor&from=-2d&device=2&legend=no&height=400&width=1200')
```
...
```php
[{{ $alert->title }}](https://your.UpdiveNSM.url/device/device={{ $alert->device_id }}/)
...
@if ($alert->name)
            "facts": [
                {
                    "name": "Rule:",
                    "value": "[{{ $alert->name }}](https://your.UpdiveNSM.url/device/device={{ $alert->device_id }}/tab=alert/)"
                },
@else
                {
                    "name": "Rule:",
                    "value": "[{{ $alert->rule }}](https://your.UpdiveNSM.url/device/device={{ $alert->device_id }}/tab=alert/)"
                },
@endif
                {
                    "name": "Severity:",
...
@endif
                {
                    "name": "Hostname:",
                    "value": "[{{ $alert->hostname }}](https://your.UpdiveNSM.url/device/device={{ $alert->device_id }}/)"
                },
                {
                    "name": "Hardware:",
...
@foreach ($alert->faults as $key => $value)
        },
        {
            "facts": [
                {
                    "name": "Port:",
                    "value": "[{{ $value['ifName'] }}](https://your.UpdiveNSM.url/device/device={{ $alert->device_id }}/tab=port/port={{ $value['port_id'] }}/)"
                },
                {
                    "name": "Description:",
                    "value": "{{ $value['ifAlias'] }}"
                ...
                "body": [
                    {
                        ...
                        "weight":  "Bolder",
                        "color":  "{{ $state_color }}",
                        "text":  "🚨 **UpdiveNSM Alert @if ($alert->state == 0) - Resolved @endif**",
                ...
                "actions":  [
                    {
                        ...
                        "title":  "View Alert",
                        "style": "positive",
                        "url":  "https://UpdiveNSM.server.utsc.utoronto.ca/device/{{ $alert->device_id }}/alerts"
                    }
                ]
                }
        }
