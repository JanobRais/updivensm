...
Here is the step for setup a LINE bot and using it in UpdiveNSM.
...
1. Go to "Messaging API" tab of your channel, here listing some important value.
	...
	- `Channel access token (long-lived)`, will use it in UpdiveNSM, keep it safe.

1. Use your real Line account add your LINE bot as a friend.

1. Recipient ID can be `groupID`, `userID` or `roomID`, it will be used in UpdiveNSM to send message to a group or a user. Use the following NodeJS program and `ngrok` for temporally https webhook to listen it.
