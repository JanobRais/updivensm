...
- Net-SNMP tools installed on the UpdiveNSM host (`snmptrap` binary).
- A MIB file accessible on the UpdiveNSM host describing the trap structure.
  The UpdiveNSM-contributed MIB (`UpdiveNSM-NOTIFICATIONS-MIB`) is shipped under
  `mibs/UpdiveNSM/` and covers the default alert template.
...
| Trap OID | `UpdiveNSM-NOTIFICATIONS-MIB::defaultAlertEvent` | Notification OID defined in the MIB |
| PDU Type | `TRAPv2` | `TRAPv2` (one-way) or `INFORM` (acknowledged) |
| MIB Directory | `/opt/UpdiveNSM/mibs/UpdiveNSM` | Directory containing the MIB file(s) |
...
| Trap OID | UpdiveNSM-NOTIFICATIONS-MIB::defaultAlertEvent |
| PDU Type | TRAPv2 |
| MIB Directory | /opt/UpdiveNSM/mibs/UpdiveNSM |
...
Copy the desired MIB directory to the UpdiveNSM host and configure the path:

```bash
# UpdiveNSM MIB (default)
cp -r /opt/UpdiveNSM/mibs/UpdiveNSM /opt/UpdiveNSM/mibs/UpdiveNSM
```

To make the MIB globally available to Net-SNMP tools:

```bash
cp /opt/UpdiveNSM/mibs/UpdiveNSM/UpdiveNSM-NOTIFICATIONS-MIB \
   /usr/share/snmp/mibs/
```
