...
Sensu Agent on your poller with the HTTP socket enabled on tcp/3031, UpdiveNSM
will start generating Sensu events as soon as you create the transport.

Acknowledging alerts within UpdiveNSM is not directly supported, but an
...
an annotation (`generated-by`) set, to allow you to treat UpdiveNSM events
...
/changed states
- The agent will buffer alerts, but UpdiveNSM will not - if your agent is
offline, alerts will be dropped
- There is no backchannel between Sensu and UpdiveNSM - if you make changes in
Sensu to UpdiveNSM alerts, they'll be lost on the next event (silences will work)
