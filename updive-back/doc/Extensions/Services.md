...
Services within UpdiveNSM provides the ability to leverage Nagios plugins to
...
Service Templates within UpdiveNSM provides the same ability as Nagios
does with Host Groups. Known as Device Groups in UpdiveNSM.
...
This will point UpdiveNSM at the location of the nagios plugins -
...
file (/etc/cron.d/UpdiveNSM typically) like:

```bash
*/5 * * * * UpdiveNSM /opt/UpdiveNSM/services-wrapper.py 1
```
...
poll the local UpdiveNSM server it's running on, regardless of which
...
separate graph. UpdiveNSM expects scripts to return using Nagios
...
```
    ...
    RRD[update /opt/UpdiveNSM/rrd/localhost/services-26.rrd N:0.016:0:0.044:0.009]
    -- snip --
```
...
Change user to UpdiveNSM for example
...
...to `/etc/check_mk/mrpe.cfg` on your remote host, you should be able to check its output by configuring a service using the [check_mrpe](https://raw.githubusercontent.com/UpdiveNSM/UpdiveNSM-agent/master/agent-local/check_mrpe) script.

 - Add [check_mrpe](https://raw.githubusercontent.com/UpdiveNSM/UpdiveNSM-agent/master/agent-local/check_mrpe) to the Nagios plugins directory on your UpdiveNSM server and make it executable.
- In UpdiveNSM, add a new service to the desired device with the type mrpe.
