...
UpdiveNSM can generate a list of hosts that can be monitored by
...
want to call it and where UpdiveNSM is:

```bash
cd /opt/UpdiveNSM/scripts/
...
```bash
15   0    * * * root cd /opt/UpdiveNSM/scripts && php ./gen_rancid.php > /the/path/where/is/rancid/core/router.db
```

Now configure UpdiveNSM (make sure you point dir to your rancid data directory):
...
repositories, and can be served by the same HTTP server as UpdiveNSM.
...
Get a list of devices from UpdiveNSM you can pull configs from:
...
`sudo vi /var/lib/rancid/UpdiveNSM/router.db`
...
```
sudo usermod -a -G rancid UpdiveNSM
```

Add Rancid into UpdiveNSM config.php:

```php
### Rancid
$config['rancid_configs'][]             = '/var/lib/rancid/UpdiveNSM/configs/';
