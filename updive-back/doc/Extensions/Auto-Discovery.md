...
UpdiveNSM provides the ability to automatically add devices on your
...
However, UpdiveNSM will only try to add the new devices discovered with LLDP/xDP if `$config['autodiscovery']['xdp'] = true;`.
...
Apart from the aforementioned Auto-Discovery options, UpdiveNSM is also
...
within your UpdiveNSM installation directory.
...
Scan network for snmp hosts and add them to UpdiveNSM.
