...
UpdiveNSM supports multiple authentication modules along with [Two Factor Auth](Two-Factor-Auth.md).
...
enabled. UpdiveNSM doesn't support multiple authentication mechanisms at
the same time.
...
When using SELinux on the UpdiveNSM server, you need to allow Apache
...
This is default option with UpdiveNSM so you should have already have the following configuration setup
...
On RHEL / CentOS / Fedora, in order for UpdiveNSM to reach Active Directory, you need to allow LDAP requests in SELinux:
```
setsebool -P httpd_can_connect_ldap 1
```
...
!!! setting "auth/ldap"
    ...
    lnms config:set auth_ldap_cacertfile /opt/UpdiveNSM/ldap-ca-cert
...
On RHEL / CentOS / Fedora, in order for UpdiveNSM to reach LDAP, you need to allow LDAP requests in SELinux:
```
setsebool -P httpd_can_connect_ldap 1
```
...
The string to send in `Filter-ID` reply attribute must start with `UpdiveNSM_role_` followed by the role name.
For example to set the admin role send `UpdiveNSM_role_admin`.

The following strings correspond to the built-in roles, but any defined role can be used:
- `UpdiveNSM_role_normal` - Sets the normal user .
- `UpdiveNSM_role_admin` - Sets the administrator role.
- `UpdiveNSM_role_global-read` - Sets the global-read role

UpdiveNSM will ignore any other strings sent in `Filter-ID` and revert to default role that is set in your config.
...
This may be utilized if you already use `Filter-ID` in your environment and also want to use radius with UpdiveNSM.
...
UpdiveNSM will expect the user to have authenticated via your
...
## View/embedded graphs without being logged into UpdiveNSM
...
authentication providers that are managed outside of UpdiveNSM - such
...
Relying Party to pass attributes to UpdiveNSM via header injection or
...
end user, and also prevent end users from contacting UpdiveNSM
directly.
...
addresses or CIDR prefixes that are allowed to connect to UpdiveNSM and
...
the name of the attribute that the Relying Party exposes to UpdiveNSM -
...
lnms config:set sso.group_level_map '{"UpdiveNSM-admins": 10, "UpdiveNSM-readers": 1, "UpdiveNSM-billingcontacts": 5}'
...
```bash
lnms config:set sso.group_filter "/UpdiveNSM-(.*)/i"
```
...
UpdiveNSM has no capability to log out a user authenticated via Single
...
session, you can configure UpdiveNSM to direct the user to it:
