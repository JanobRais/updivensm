...
UpdiveNSM can be used with a MariaDB Galera Cluster. This is a Multi Master cluster, meaning each
node in the cluster can read and write to the database. They all have the same ability. UpdiveNSM will
...
wsrep_cluster_name="UpdiveNSM_cluster"
...
UpdiveNSM supports up to 9 galera nodes, you define these nodes in the .env file. For each node we have the ability to define if this UpdiveNSM installation/poller is able to write, read or both to that node.
The galera nodes you define here can be the same or different for each UpdiveNSM poller. If you have a poller you only want to write/read to one galera node, you would simply add one DB_HOST, and omit all the rest. This allows you to precisely control what galera nodes a UpdiveNSM poller is reading and or writing too.
...
```
The above .env on a UpdiveNSM installation/poller would communicate to each galera node as follows.
