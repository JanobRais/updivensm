...
UpdiveNSM in combination with check_mk (found
[here](https://github.com/UpdiveNSM/UpdiveNSM-agent)). The agent can be
...
The agent uses TCP-Port 6556, please allow access from the **UpdiveNSM
...
1: Clone the `UpdiveNSM-agent` repository:

```bash
cd /opt/
git clone https://github.com/UpdiveNSM/UpdiveNSM-agent.git
cd UpdiveNSM-agent
```
...
8: Login to the UpdiveNSM web interface and edit the device you want to
...
2. Run the msi / exe
3. Make sure your UpdiveNSM instance can reach TCP port 6556 on your target.
