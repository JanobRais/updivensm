...
RRD will continue to function as normal so UpdiveNSM itself should
...
!!! setting "poller/prometheus"
    ...
    lnms config:set prometheus.job 'UpdiveNSM'
    lnms config:set prometheus.prefix 'UpdiveNSM'
    ```
...
Configuring a prefix name, for example 'UpdiveNSM', instead caused those 
metrics to be exposed with the following names:

```
UpdiveNSM_OUTUCASTPKTS
UpdiveNSM_ifOutUcastPkts_rate
UpdiveNSM_INOCTETS
UpdiveNSM_ifInErrors_rate
```
