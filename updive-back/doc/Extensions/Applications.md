...
before or after the hosts have been added to UpdiveNSM.
...
automatically, even if the device is already in UpdiveNSM. The
...
### JSON Return Optimization Using UpdiveNSM_return_optimizer
...
`UpdiveNSM_return_optimizer` fixes this via taking the extend output
...
wget https://raw.githubusercontent.com/UpdiveNSM/UpdiveNSM-agent/master/utils/UpdiveNSM_return_optimizer -O /usr/local/bin/UpdiveNSM_return_optimizer
chmod +x /usr/local/bin/UpdiveNSM_return_optimizer
```
...
case where UpdiveNSM does not automatically detect it.

**Note**: Only do this if an application was not auto-discovered by
UpdiveNSM during discovery and polling.
