...
UpdiveNSM uses a role-based access control (RBAC) system. Permissions are grouped into roles, and roles are
assigned to users to define what they can see and do within the application.

### 1. Introduction to Authorization

Authorization determines what an authenticated user is allowed to do. In UpdiveNSM, this is handled through
...
UpdiveNSM provides granular, resource-specific access control for users who lack viewAny functional permissions.
