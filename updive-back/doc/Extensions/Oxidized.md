...
Then you can procede to the UpdiveNSM Web UI and go to Oxidized
...
oxidized to pull them from UpdiveNSM [Feeding
Oxidized](#feeding-oxidized) Note: this means devices will be controlled by
the UpdiveNSM API, and not router.db, passwords will still need to be in the
oxidized config file.

UpdiveNSM will automatically map the OS to the Oxidized model name if
...
UpdiveNSM supports config versioning if Oxidized does.  This is known
...
Group level username/password or per device. UpdiveNSM currently
...
support for Oxidized has been added to the UpdiveNSM API. A sample
config for Oxidized is provided below.

You will need to configure default credentials for your devices in the
Oxidized config, UpdiveNSM doesn't provide login credentials at this
time.
...
```bash
      source:
        ...
        debug: false
        http:
          url: https://UpdiveNSM/api/v0/oxidized
          map:
            name: hostname
            model: os
            group: group
...
UpdiveNSM is able to reload the Oxidized list of nodes, each time a
device is added to UpdiveNSM. To do so, edit the option in Global
...
finally by defining the overriding value itself. UpdiveNSM does not
...
groups:
  <groupname>:
    ...
    password: <password>
```
## Have Oxidized add to UpdiveNSM Eventlog

Oxidized can be configured to add to UpdiveNSM's eventlog for devices. This gives better visibility when it is having issues checking for config changes, or when it observes the configuration has changed. This uses [Oxidized's Hooks](https://github.com/ytti/oxidized/blob/master/docs/Hooks.md#hook-type-exec).
...
When you're disabling or removing a device from UpdiveNSM, the
configuration will no longer be available via the UpdiveNSM web interface.  
...
If you want to purge saved config of a device that is not in UpdiveNSM anymore, you can run the following command:
