...
## Configure Redis Sentinel cluster with UpdiveNSM

Both UpdiveNSM WebUI and Poller can be configured to use a Redis Sentinel cluster as their backend.
...
To verify your Redis Sentinel setup is working correctly with UpdiveNSM you can connect to the Redis Sentinel cluster using the `redis-cli` and execute the `MONITOR` command. This command will show you the commands being executed on the Redis cluster.
