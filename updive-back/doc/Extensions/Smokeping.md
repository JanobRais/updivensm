...
UpdiveNSM has support for both new and pre-existing SmokePing installations.
...
to look at [Configure UpdiveNSM - All Operating Systems](#configure-UpdiveNSM-all-operating-systems).
...
Smokeping is available via EPEL, which if you're running UpdiveNSM, you probably
...
configuration file is updated. You can find an example in `misc/UpdiveNSM-smokeping-rhel.example`.
Put this into /etc/cron.d/hourly, and mark it executable:

```
sudo cp /opt/UpdiveNSM/misc/smokeping-rhel.example /etc/cron.hourly/UpdiveNSM-smokeping
sudo chmod +x /etc/cron.hourly/UpdiveNSM-smokeping
```
...
@include /etc/smokeping/UpdiveNSM-probes.conf
```
...
@include /etc/smokeping/UpdiveNSM-targets.conf
```
...
sure it comes *after* the UpdiveNSM configuration, and keep in mind that
...
```bash
sudo /etc/cron.hourly/UpdiveNSM-smokeping
...
Once installed, you should need a cron script installed to make sure that the
configuration file is updated. You can find an example in `misc/UpdiveNSM-smokeping-debian.example`.
Put this into /etc/cron.d/hourly, and mark it executable:

```
sudo cp /opt/UpdiveNSM/misc/smokeping-debian.example /etc/cron.hourly/UpdiveNSM-smokeping
sudo chmod +x /etc/cron.hourly/UpdiveNSM-smokeping
```
...
@include /etc/smokeping/config.d/UpdiveNSM-probes.conf
```
...
@include /etc/smokeping/config.d/UpdiveNSM-targets.conf
```
...
## Configure UpdiveNSM - All Operating Systems
...
UpdiveNSM does not need the Web UI - you can find the graphs in UpdiveNSM on the
*latency* tab.
...
owner    = UpdiveNSM-Admin
contact  = admin@ACME.xxx
cgiurl   = http://yourUpdiveNSM/cgi-bin/smokeping.cgi
```
...
```bash
UpdiveNSM@UpdiveNSM:~/scripts$ ls /etc/apache2/conf-available/ | grep smokeping
smokeping.conf
```
...
This section assumes you have configured UpdiveNSM with Nginx as
specified in [Configure Nginx](../Installation/Install-UpdiveNSM.md#__tabbed_5_1).
...
Add the following configuration to your `/etc/nginx/conf.d/UpdiveNSM.conf` file within `server` section.

The following will configure Nginx to respond to `http://yourUpdiveNSM/smokeping`:
```
location = /smokeping/ {
...
location ^~ /smokeping/images {
        alias /opt/UpdiveNSM/rrd/smokeping/images;
        gzip off;
}
```
...
You should be able to load the Smokeping web interface at `http://yourUpdiveNSM/smokeping`


### Nginx Configuration - Ubuntu, Debian and alike

This section assumes you have configured UpdiveNSM with Nginx as
specified in [Configure Nginx](../Installation/Install-UpdiveNSM.md#configure-web-server).
...
```
# Browsing to `http://yourUpdiveNSM/smokeping/` should bring up the smokeping web interface
...
of UpdiveNSM, but make sure that file permissions and SELinux labels allow the
...
#### Share RRDCached with UpdiveNSM

Move the RRD's and give smokeping access rights to the UpdiveNSM RRD directory:
```bash
sudo systemctl stop smokeping
sudo mv /var/lib/smokeping /opt/UpdiveNSM/rrd/
sudo usermod -a -G UpdiveNSM smokeping
```

Update data directory in */etc/smokeping*:

```
datadir = /opt/UpdiveNSM/rrd/smokeping
dyndir = /opt/UpdiveNSM/rrd/smokeping/__cgi
```
...
#### Configure SELinux to allow smokeping to write in UpdiveNSM directory on Centos / RHEL
If you are using RRDCached with the -B switch and smokeping RRD's inside the UpdiveNSM RRD base directory, you can install this SELinux profile:

```
cat > smokeping_UpdiveNSM.te << EOF
module smokeping_UpdiveNSM 1.0;
...
EOF
checkmodule -M -m -o smokeping_UpdiveNSM.mod smokeping_UpdiveNSM.te
semodule_package -o smokeping_UpdiveNSM.pp -m smokeping_UpdiveNSM.mod
semodule -i smokeping_UpdiveNSM.pp
```
...
If you're trying to integrate UpdiveNSM, smokeping *and* another source of
...
will have one. If you're trying to integrate UpdiveNSM, smokeping *and* another
