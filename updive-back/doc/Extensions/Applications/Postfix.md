...
1. Copy the shell script, postfix-queues, to the desired host
    ...
    ```bash
    wget https://github.com/UpdiveNSM/UpdiveNSM-agent/raw/master/snmp/postfix-queues -O /etc/snmp/postfix-queues
    ```

2. Copy the Perl script, postfixdetailed, to the desired host

    ```bash
    wget https://github.com/UpdiveNSM/UpdiveNSM-agent/raw/master/snmp/postfixdetailed -O /etc/snmp/postfixdetailed
    ```
...
!!! note 
    ...
    updated, so if this happens in between UpdiveNSM doing it then the
