...
1. Download the script onto the desired host.
    ...
    ```bash
    wget https://raw.githubusercontent.com/UpdiveNSM/UpdiveNSM-agent/master/snmp/http_access_log_combined -O /etc/snmp/http_access_log_combined
    ```
...
5.  If on large setups where it won't complete in a timely manner, run it via cron.
    ...
    === "If using cron"
        Add the following to `/etc/crontab.d/UpdiveNSM_http_access_log_combined`:
