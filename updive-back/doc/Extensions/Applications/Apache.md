...
=== "SNMP Extend"
    ...
    1. Download the script onto the desired host (the host must be added to UpdiveNSM devices)

    ```bash
    wget https://raw.githubusercontent.com/UpdiveNSM/UpdiveNSM-agent/master/snmp/apache-stats.py -O /etc/snmp/apache-stats.py
    ```
    ...
    3. Create the cache directory, '/var/cache/UpdiveNSM/' and make sure
    that it is owned by the user running the SNMP daemon.

    ```bash
    mkdir -p /var/cache/UpdiveNSM/
    ```
...
=== "Agent"
    ...
    2. Create the cache directory, '/var/cache/UpdiveNSM/' and make sure
    that it is owned by the user running the SNMP daemon.
    ...
    3. On the device page in UpdiveNSM, edit your host and check the
