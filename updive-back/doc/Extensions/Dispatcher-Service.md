...
The **UpdiveNSM Dispatcher Service** (`UpdiveNSM-service.py`) schedules core UpdiveNSM tasks without using cron jobs.
...
A systemd unit file is provided in `misc/UpdiveNSM.service`.
Update `ExecStart` and `WorkingDirectory` if UpdiveNSM is not installed in `/opt/UpdiveNSM`.

```bash
cp /opt/UpdiveNSM/misc/UpdiveNSM.service /etc/systemd/system/UpdiveNSM.service && systemctl enable --now UpdiveNSM.service
```
...
```bash
rm /etc/cron.d/UpdiveNSM
```

> **Note:** If using the *UpdiveNSM-scheduler* cron (systemd timer), do **not** disable it.
...
```bash
cp /opt/UpdiveNSM/misc/UpdiveNSM-watchdog.service /etc/systemd/system/UpdiveNSM.service && systemctl enable --now UpdiveNSM.service
```
