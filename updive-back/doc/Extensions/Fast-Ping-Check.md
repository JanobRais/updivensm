...
Normally, UpdiveNSM sends an ICMP ping to the device before polling to
...
quickly than that. UpdiveNSM offers a `ping.php` script to run ping
...
4: Add the following line to `/etc/cron.d/UpdiveNSM` to allow 1 minute
ping checks

```title="/etc/cron.d/UpdiveNSM"
*    *    * * *   UpdiveNSM    /opt/UpdiveNSM/ping.php >> /dev/null 2>&1
```
...
```title="/etc/cron.d/UpdiveNSM"
*    *    * * *   UpdiveNSM    /opt/UpdiveNSM/ping.php >> /dev/null 2>&1
*    *    * * *   UpdiveNSM    sleep 30 && /opt/UpdiveNSM/ping.php >> /dev/null 2>&1
*    *    * * *   UpdiveNSM    sleep 15 && /opt/UpdiveNSM/alerts.php >> /dev/null 2>&1
*    *    * * *   UpdiveNSM    sleep 45 && /opt/UpdiveNSM/alerts.php >> /dev/null 2>&1
```
