...
Currently, UpdiveNSM supports a lot of trap handlers. You can check them on
GitHub [here](https://github.com/UpdiveNSM/UpdiveNSM/tree/master/UpdiveNSM/Snmptrap/Handlers).
...
traphandle default /opt/UpdiveNSM/snmptrap.php
```
...
```ini
[Service]
Environment=MIBDIRS=+/opt/UpdiveNSM/mibs:/opt/UpdiveNSM/mibs/cisco
Environment=MIBS=+IF-MIB
```

For non-systemd systems, you can edit TRAPDOPTS in the init script in /etc/init.d/snmptrapd.

`TRAPDOPTS="-Lsd  -M /opt/UpdiveNSM/mibs -m IF-MIB -f -p $TRAPD_PID"`
...
[Service]
Environment="MIBSDIR=/opt/UpdiveNSM/mibs"
Type=simple
ExecStart=/usr/sbin/snmptrapd -f -m IF-MIB -M /opt/UpdiveNSM/mibs
...
|   -M   | DIRLIST: use DIRLIST as the list of locations to look for MIBs. Option is not recursive, so you need to specify each DIR individually, separated by `:`. (For example: /opt/UpdiveNSM/mibs:/opt/UpdiveNSM/mibs/cisco:/opt/UpdiveNSM/mibs/edgecos)|
...
Add the following config to your snmptrapd.service after `ExecStart=/usr/sbin/snmptrapd -f -m ALL -M /opt/UpdiveNSM/mibs`
...
On SELinux, you need to configure SELinux for SNMPd to communicate to UpdiveNSM:
...
You can also generate a trap using the `snmptrap` command from the UpdiveNSM server itself (if and only if the UpdiveNSM server is monitored).
...
and in UpdiveNSM your localhost device eventlog like:
