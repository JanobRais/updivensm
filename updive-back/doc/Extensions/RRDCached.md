...
This document will explain how to set up RRDCached for UpdiveNSM.
...
It is recommended that you monitor your UpdiveNSM server with UpdiveNSM
...
=== "Ubuntu"
    ...
    For info about version of rrdcached to install, see [Ubuntu packages](https://launchpad.net/ubuntu/+source/rrdtool/)
     ...
     2. Edit `/etc/default/rrdcached` to include:
        ...
        ```bash
        BASE_OPTIONS="-B -F -R"
        BASE_PATH=/opt/UpdiveNSM/rrd/
        DAEMON_GROUP=UpdiveNSM
        DAEMON_USER=UpdiveNSM
     ...
     3. Fix permissions
        ...
        ```bash
        chown UpdiveNSM:UpdiveNSM /var/lib/rrdcached/journal/
        ```
...
=== "Red Hat/CentOS 8"
    ...
    !!! note
        ...
        The intermediate files generated during the process for the SELinux policy (e.g., `rrdcached_UpdiveNSM.mod` and `rrdcached_UpdiveNSM.pp`) do not need to be saved after the policy module is successfully installed.


     1. link in the service and reload:

        ```bash
        ln -s /opt/UpdiveNSM/dist/rrdcached/rrdcached.service /etc/systemd/system/
     ...
     2. Configure SELinux for RRDCached
         ...
         1. Compile the SELinux Policy Compile the SELinux policy module using the following command:
            ...
            ```bash
            checkmodule -M -m -o /tmp/rrdcached_UpdiveNSM.mod /opt/UpdiveNSM/dist/rrdcached/rrdcached_UpdiveNSM.te
            ```
         ...
         2. Package the Policy Module Package the compiled module into a loadable policy package:
            ...
            ```bash
            semodule_package -o /tmp/rrdcached_UpdiveNSM.pp -m /tmp/rrdcached_UpdiveNSM.mod
            ```
         ...
         3. Apply the Policy Module Apply the policy module to the system:
            ...
            ```bash
            semodule -i /tmp/rrdcached_UpdiveNSM.pp
            ```
...
=== "CentOS 6"
    ...
    This example is based on a fresh UpdiveNSM install, on a minimal CentOS 6 installation.
    ...
    OPTIONS="-w 1800 -z 1800 -f 3600 -s UpdiveNSM -U UpdiveNSM -G UpdiveNSM -B -R -j /var/tmp -l unix:/run/rrdcached.sock -t 4 -F -b /opt/UpdiveNSM/rrd/"
    RRDC_USER=UpdiveNSM
    ```

    ```bash
    mkdir /var/run/rrdcached
    chown UpdiveNSM:UpdiveNSM /var/run/rrdcached/
    chown UpdiveNSM:UpdiveNSM /var/rrdtool/
    chown UpdiveNSM:UpdiveNSM /var/rrdtool/rrdcached/
    ```
...
Edit your UpdiveNSM config by running the following:
...
Check to see if the graphs are being drawn in UpdiveNSM. This might take a few minutes.
After at least one poll cycle (5 mins), check the UpdiveNSM disk I/O performance delta.
...
```bash
semodule -l | grep rrdcached_UpdiveNSM
```

Test Functionality: Ensure UpdiveNSM can successfully interact with RRDcached without SELinux denials. Check SELinux logs for any denials:

```bash
ausearch -m avc -ts recent
```

If there are no denials, the policy module has been successfully installed and UpdiveNSM can interact with RRDcached.
...
Replace `$UpdiveNSM_IP` with the ip of the server that will be using rrdcached. You can specify more than one `allow` statement. This will bind nginx to TCP 42217 (the default rrdcached port), allow the specified IPs to connect, and deny all others.
