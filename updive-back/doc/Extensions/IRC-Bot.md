...
UpdiveNSM has an easy to use IRC-Interface for basic tasks like viewing
...
`##UpdiveNSM` will be used. The default Nick for the bot is `UpdiveNSM`.
...
`$config['irc_chan']` | `##UpdiveNSM` | Optional; Multiple channels can be defined as Array or delimited with `,`. Passwords are defined after a `space-character`.
...
`$config['irc_nick']` | `UpdiveNSM` | Optional;
...
`$config['irc_ctcp_version']` | `UpdiveNSM IRCbot. https://www.UpdiveNSM.org/` | Optional; Reply-string to CTCP VERSION requests
...
```php
   ...
   $config['irc_chan'] = "#UpdiveNSM,#otherchan,#noc";
   ...
```
...
be authenticated as the "admin" user in UpdiveNSM, and clients matching
the last line will be authenticated as the user "john" in UpdiveNSM,
...
UpdiveNSM-irc.service script is located at /opt/UpdiveNSM/misc/

Once copied to /etc/systemd/system/ you must run the following commands:

a) chmod 664 /etc/systemd/system/UpdiveNSM-irc.service

b) systemctl daemon-reload

c) systemctl enable UpdiveNSM-irc.service

d) systemctl start UpdiveNSM-irc.service

It can be stopped or started just like any other systemd script such
as systemctl start UpdiveNSM-irc.service
