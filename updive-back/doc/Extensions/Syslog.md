...
This section explain different ways to receive and process syslog with UpdiveNSM.
Except of graylog, all Syslogs variants store their logs in the UpdiveNSM database. You need to enable the Syslog extension:

```bash
lnms config:set enable_syslog true
```
A Syslog integration gives you a centralized view of information within the UpdiveNSM (device view, traps, event). Further more you can trigger alerts based on syslog messages (see rule collections).
...
(/etc/syslog-ng/conf.d/UpdiveNSM.conf) and paste the following:
...
destination d_UpdiveNSM {
        program("/opt/UpdiveNSM/syslog.php" template ("$HOST||$FACILITY||$PRIORITY||$LEVEL||$TAG||$R_YEAR-$R_MONTH-$R_DAY $R_HOUR:$R_MIN:$R_SEC||$MSG||$PROGRAM\n") template-escape(yes));
};
...
If no messages make it to the syslog tab in UpdiveNSM, chances are you experience an issue with SELinux. If so, create a file mycustom-UpdiveNSM-rsyslog.te , with the following content:

```
module mycustom-UpdiveNSM-rsyslog 1.0;
...
```ssh
checkmodule -M -m -o mycustom-UpdiveNSM-rsyslog.mod mycustom-UpdiveNSM-rsyslog.te
semodule_package -o mycustom-UpdiveNSM-rsyslog.pp -m mycustom-UpdiveNSM-rsyslog.mod
semodule -i mycustom-UpdiveNSM-rsyslog.pp
```
...
Create a file called `/etc/rsyslog.d/30-UpdiveNSM.conf`and add the following depending on your version of rsyslog.

=== "Version 8"
    ```
    # Feed syslog messages to UpdiveNSM
    module(load="omprog")

    template(name="UpdiveNSM"
            type="string"
            string= "%fromhost%||%syslogfacility%||%syslogpriority%||%syslogseverity%||%syslogtag%||%$year%-%$month%-%$day% %timegenerated:8:25%||%msg%||%programname%\n")
            action(type="omprog"
            binary="/opt/UpdiveNSM/syslog.php"
            template="UpdiveNSM")
...
=== "Version 7"
    ...
    $template UpdiveNSM,"%fromhost%||%syslogfacility%||%syslogpriority%||%syslogseverity%||%syslogtag%||%$year%-%$month%-%$day% %timegenerated:8:25%||%msg%||%programname%\n"

    *.* action(type="omprog" binary="/opt/UpdiveNSM/syslog.php" template="UpdiveNSM")
...
=== "Legacy"
    ...
    # Feed syslog messages to UpdiveNSM
    $ModLoad omprog
    $template UpdiveNSM,"%FROMHOST%||%syslogfacility-text%||%syslogpriority-text%||%syslogseverity%||%syslogtag%||%$YEAR%-%$MONTH%-%$DAY%    %timegenerated:8:25%||%msg%||%programname%\n"

    $ActionOMProgBinary /opt/UpdiveNSM/syslog.php
    *.* :omprog:;UpdiveNSM
    ```
...
UpdiveNSM, here are some hints on how to get it working.
...
output {
        exec {
        command => "echo `echo %{host},,,,%{facility},,,,%{priority},,,,%{severity},,,,%{facility_label},,,,``date --date='%{timestamp}' '+%Y-%m-%d %H:%M:%S'``echo ',,,,%{message}'``echo ,,,,%{program} | sed 's/\x25\x7b\x70\x72\x6f\x67\x72\x61\x6d\x7d/%{facility_label}/'` | sed 's/,,,,/||/g' | /opt/UpdiveNSM/syslog.php &"
        }
        elasticsearch {
 ...
 └──────┘    └───────────────────┘        ▼
                                      ~~~WAN~~~
                                        ...
                                        └─┼─┘
                                          ...
                                          ▼
                         ┌────────────────────┐    ┌────────────────────┐
                         │UpdiveNSM Sink       ├┬──►│UpdiveNSM Master     │
...
output {
....
        #feed it to UpdiveNSM
     	http {
     		http_method => "post"
     		url => "https://sink.UpdiveNSM.org/api/v0/syslogsink/    # replace with your UpdiveNSM host
     		format => "json_batch"                                  # put multiple syslogs in on HTTP message
                ...
                headers => ["X-Auth-Token","xxxxxxxUpdiveNSMApiToken]
...
Sample test data:
```
curl -L -X POST 'https://sink.UpdiveNSM.org/api/v0/syslogsink/' -H 'X-Auth-Token: xxxxxxxUpdiveNSMApiToken' --data-raw '[   
    {
...
Replace UpdiveNSM.ip with IP or hostname of your UpdiveNSM install.
...
logging host <outside interface name> UpdiveNSM.ip
```
...
logging UpdiveNSM.ip
```

### Cisco NXOS

```config
logging server UpdiveNSM.ip 5 use-vrf default facility local6
```

### Juniper Junos

```config
set system syslog host UpdiveNSM.ip authorization any
set system syslog host UpdiveNSM.ip daemon any
set system syslog host UpdiveNSM.ip kernel any
set system syslog host UpdiveNSM.ip user any
set system syslog host UpdiveNSM.ip change-log any
set system syslog host UpdiveNSM.ip source-address <management ip>
set system syslog host UpdiveNSM.ip exclude-hostname
...
```config
info-center loghost UpdiveNSM.ip
...
```config
loghost add UpdiveNSM.ip UpdiveNSM
loghost activate name UpdiveNSM
```

### Allied Telesis Alliedware Plus

```config
log date-format iso // Required so syslog-ng/UpdiveNSM can correctly interpret the log message formatting.
log host x.x.x.x
...
logging facility local6
logging UpdiveNSM.ip control-descr "UpdiveNSM"
...
to a remote syslog server (UpdiveNSM).
...
=== "lnms"
    ```bash
    lnms config:set os.asa.syslog_hook '[{ "regex": "/%ASA-(config-)?5-111005/", "script": "/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php" }]'
    ```

=== "legacy config.php"
    ```php
    $config['os']['asa']['syslog_hook'][] = Array('regex' => '/%ASA-(config-)?5-111005/', 'script' => '/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php');
    ```
...
=== "lnms"
    ```bash
    lnms config:set os.ios.syslog_hook '[{"regex":"/%SYS-(SW[0-9]+-)?5-CONFIG_I/","script":"/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php"}]' 
    ```

=== "legacy config.php"
    ```php
    $config['os']['ios']['syslog_hook'][] = Array('regex' => '/%SYS-(SW[0-9]+-)?5-CONFIG_I/', 'script' => '/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php');
    ```
...
=== "lnms"
    ```bash
    lnms config:set os.nxos.syslog_hook '[{"regex":"/%VSHD-5-VSHD_SYSLOG_CONFIG_I/","script":"/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php"}]' 
    ```

=== "legacy config.php"
    ```php
    $config['os']['nxos']['syslog_hook'][] = Array('regex' => '/%VSHD-5-VSHD_SYSLOG_CONFIG_I/', 'script' => '/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php');
    ```
...
=== "lnms"
    ```bash
    lnms config:set os.iosxr.syslog_hook '[{"regex":"/%GBL-CONFIG-6-DB_COMMIT/","script":"/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php"}]'
    ```

=== "legacy config.php"
    ```php
    $config['os']['iosxr']['syslog_hook'][] = Array('regex' => '/%GBL-CONFIG-6-DB_COMMIT/', 'script' => '/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php');
    ```
...
=== "lnms"
    ```bash
    lnms config:set os.junos.syslog_hook '[{"regex":"/UI_COMMIT:/","script":"/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php"}]' 
    ```

=== "legacy config.php"
    ```php
    $config['os']['junos']['syslog_hook'][] = Array('regex' => '/UI_COMMIT:/', 'script' => '/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php');
    ```
...
=== "lnms"
    ```bash
    lnms config:set os.screenos.syslog_hook '[{"regex":"/System configuration saved/","script":"/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php"}]' 
    ```

=== "legacy config.php"
    ```php
    $config['os']['screenos']['syslog_hook'][] = Array('regex' => '/System configuration saved/', 'script' => '/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php');
    ```
...
=== "lnms"
    ```bash
    lnms config:set os.awplus.syslog_hook '[{"regex":"/IMI.+.Startup-config saved on/","script":"/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php"}]' 
    ```

=== "legacy config.php"
    ```php
    $config['os']['awplus']['syslog_hook'][] = Array('regex' => '/IMI.+.Startup-config saved on/', 'script' => '/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php');
    ```
...
=== "lnms"
    ```bash
    lnms config:set os.procurve.syslog_hook '[{"regex":"/Running Config Change/","script":"/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php"}]' 
    ```

=== "legacy config.php"
    ```php
    $config['os']['procurve']['syslog_hook'][] = Array('regex' => '/Running Config Change/', 'script' => '/opt/UpdiveNSM/scripts/syslog-notify-oxidized.php');
    ```
...
device in UpdiveNSM. For example, in UpdiveNSM the device is known as
"ne-core-01", and that's how DNS resolves. However, the received
syslogs are for "loopback.core-nw".

To fix this issue, you can configure UpdiveNSM to translate the
