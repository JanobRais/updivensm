...
=== "Cron"
    Edit `/etc/cron.d/UpdiveNSM` and add the following:
    ```bash
    */5 * * * * UpdiveNSM /opt/UpdiveNSM/poll-billing.php >> /dev/null 2>&1
    01  * * * * UpdiveNSM /opt/UpdiveNSM/billing-calculate.php >> /dev/null 2>&1
    ```
...
To create a new bill, from the UpdiveNSM menu select Ports -> Traffic Bills and
select `+ Create Bill`.
