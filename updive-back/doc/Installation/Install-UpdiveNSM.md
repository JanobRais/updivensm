...
```
useradd UpdiveNSM -d /opt/UpdiveNSM -M -r -s "$(which bash)"
```
...
```
cd /opt
git clone https://github.com/UpdiveNSM/UpdiveNSM.git
```

## Set permissions

```
chown -R UpdiveNSM:UpdiveNSM /opt/UpdiveNSM
chmod 771 /opt/UpdiveNSM
setfacl -d -m g::rwx /opt/UpdiveNSM/rrd /opt/UpdiveNSM/logs /opt/UpdiveNSM/bootstrap/cache/ /opt/UpdiveNSM/storage/
setfacl -R -m g::rwx /opt/UpdiveNSM/rrd /opt/UpdiveNSM/logs /opt/UpdiveNSM/bootstrap/cache/ /opt/UpdiveNSM/storage/
```

## Install PHP dependencies

Change to the UpdiveNSM user:
```
su - UpdiveNSM
```
...
```sql
CREATE DATABASE UpdiveNSM CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'UpdiveNSM'@'localhost' IDENTIFIED BY 'password';
GRANT ALL PRIVILEGES ON UpdiveNSM.* TO 'UpdiveNSM'@'localhost';
exit
```
...
=== "Ubuntu 24.04"
    ```bash
    cp /etc/php/8.3/fpm/pool.d/www.conf /etc/php/8.3/fpm/pool.d/UpdiveNSM.conf
    vi /etc/php/8.3/fpm/pool.d/UpdiveNSM.conf
    ```
...
=== "CentOS 8"
    ```bash
    cp /etc/php-fpm.d/www.conf /etc/php-fpm.d/UpdiveNSM.conf
    vi /etc/php-fpm.d/UpdiveNSM.conf
    ```

=== "Debian 12"
    ```bash
    cp /etc/php/8.2/fpm/pool.d/www.conf /etc/php/8.2/fpm/pool.d/UpdiveNSM.conf
    vi /etc/php/8.2/fpm/pool.d/UpdiveNSM.conf
    ```

=== "Debian 13"
    ```bash
    cp /etc/php/8.4/fpm/pool.d/www.conf /etc/php/8.4/fpm/pool.d/UpdiveNSM.conf
    vi /etc/php/8.4/fpm/pool.d/UpdiveNSM.conf
    ```

Change `[www]` to `[UpdiveNSM]`:
```
[UpdiveNSM]
```

Change `user` and `group` to "UpdiveNSM":
```
user = UpdiveNSM
group = UpdiveNSM
```
...
```
listen = /run/php-fpm-UpdiveNSM.sock
```

If there are no other PHP web applications on this server, you may remove www.conf to save some resources.
Feel free to tune the performance settings in UpdiveNSM.conf to meet your needs.
...
=== "Ubuntu 24.04"
    === "NGINX"
        ```bash
        vi /etc/nginx/conf.d/UpdiveNSM.conf
        ```
        ...
        server {
         listen      80;
         server_name UpdiveNSM.example.com;
         root        /opt/UpdiveNSM/html;
         index       index.php;
         ...
         location ~ [^/]\.php(/|$) {
          fastcgi_pass unix:/run/php-fpm-UpdiveNSM.sock;
...
=== "CentOS 8"
    ...
    === "Apache"
        Create the UpdiveNSM.conf:

        ```
        vi /etc/httpd/conf.d/UpdiveNSM.conf
        ```
        ...
        ```apache
        <VirtualHost *:80>
          DocumentRoot /opt/UpdiveNSM/html/
          ServerName  UpdiveNSM.example.com
          ...
          <FilesMatch ".+\.php$">
            SetHandler "proxy:unix:/run/php-fpm-UpdiveNSM.sock|fcgi://localhost"
          </FilesMatch>
        </VirtualHost>
        ```
...
=== "Debian 12"
    === "NGINX"
        ```bash
        vi /etc/nginx/sites-enabled/UpdiveNSM.vhost
        ```
...
=== "CentOS 8"
    ...
    <h3>Configure the contexts needed by UpdiveNSM</h3>

    ```
    semanage fcontext -a -t httpd_sys_content_t '/opt/UpdiveNSM/html(/.*)?'
    semanage fcontext -a -t httpd_sys_rw_content_t '/opt/UpdiveNSM/(rrd|storage)(/.*)?'
    semanage fcontext -a -t httpd_log_t "/opt/UpdiveNSM/logs(/.*)?"
    semanage fcontext -a -t httpd_cache_t '/opt/UpdiveNSM/cache(/.*)?'
    semanage fcontext -a -t bin_t '/opt/UpdiveNSM/UpdiveNSM-service.py'
    semanage fcontext -a -t httpd_cache_t '/opt/UpdiveNSM/cache(/.*)?'
    restorecon -RFvv /opt/UpdiveNSM
    setsebool -P httpd_can_sendmail=1
    setsebool -P httpd_execmem 1
    chcon -t httpd_sys_rw_content_t /opt/UpdiveNSM/.env
    ```
...
```
ln -s /opt/UpdiveNSM/lnms /usr/bin/lnms
cp /opt/UpdiveNSM/misc/lnms-completion.bash /etc/bash_completion.d/
```
...
```
cp /opt/UpdiveNSM/snmpd.conf.example /etc/snmp/snmpd.conf
```
...
```
curl -o /usr/bin/distro https://raw.githubusercontent.com/UpdiveNSM/UpdiveNSM-agent/master/snmp/distro
...
```
cp /opt/UpdiveNSM/dist/UpdiveNSM.cron /etc/cron.d/UpdiveNSM
```
...
!!! note
    ...
    finished UpdiveNSM install steps:
    <@= config.site_url =@/Support/Configuration/#proxy-support>

## Enable the scheduler

```
cp /opt/UpdiveNSM/dist/UpdiveNSM-scheduler.service /opt/UpdiveNSM/dist/UpdiveNSM-scheduler.timer /etc/systemd/system/

systemctl enable UpdiveNSM-scheduler.timer
systemctl start UpdiveNSM-scheduler.timer
```

## Enable logrotate

UpdiveNSM keeps logs in `/opt/UpdiveNSM/logs`. Over time these can
become large and be rotated out.  To rotate out the old logs you can
use the provided logrotate config file:

```
cp /opt/UpdiveNSM/misc/UpdiveNSM.logrotate /etc/logrotate.d/UpdiveNSM
```
...
<http://UpdiveNSM.example.com/install>

The web installer might prompt you to create a `config.php` file in
your UpdiveNSM install location manually, copying the content displayed
...
```
chown UpdiveNSM:UpdiveNSM /opt/UpdiveNSM/config.php
```

## Final steps

That's it!  You now should be able to log in to
<http://UpdiveNSM.example.com/>.

!!! danger
    Please note that we have not covered
    HTTPS setup in this example, so your UpdiveNSM install is not secure
...
<https://UpdiveNSM.example.com/addhost>
...
There are various options for getting help listed on the UpdiveNSM web
site: <https://www.UpdiveNSM.org/#support>

## What next?

Now that you've installed UpdiveNSM, we'd suggest that you have a read
...
We hope you enjoy using UpdiveNSM. If you do, it would be great if you
would consider opting into the stats system we have, please see [this
page](../General/Callback-Stats-and-Privacy.md) on
what it is and how to enable it.

If you would like to help make UpdiveNSM better there are [many ways to
help](../Support/FAQ.md#faq9). You
can also [back UpdiveNSM on Open Collective](https://t.libren.ms/donations).
