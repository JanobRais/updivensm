A UpdiveNSM user, [Dan](https://twitter.com/thedanbrown), has kindly
...
-   Build the RRD directories on UpdiveNSM
-   Convert the RRD files on Observium to XML (x86 to x64 move)
-   Copy the RRD/XML files to UpdiveNSM
-   Convert the XML files back to RRD files
-   Add the device to UpdiveNSM
...
-   mkdir.sh – this script creates the necessary directories on your UpdiveNSM server
-   destwork.sh – depending on the version you choose, this script will add the device to UpdiveNSM and possibly convert from XML to RRD
...
`ssh-copy-id UpdiveNSM`

Where UpdiveNSM is the hostname or IP of your destination server.
...
Also important, the nodelist.txt file must be on **both your Observium and UpdiveNSM server**. Once you have your list, edit nodelist.txt with nano:

`nano /tmp/nodelist.txt`

And replace the dummy data with the hosts you are converting. CTRL+X and then Y to save your modifications. Make the same changes on the UpdiveNSM server.
...
-   DEST – This should be the IP or hostname of your UpdiveNSM server
-   L_RRDPATH – This signifies the location of the UpdiveNSM RRD directory. The default value is the default install location
