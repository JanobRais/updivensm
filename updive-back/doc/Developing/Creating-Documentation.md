...
One of the goals of the UpdiveNSM project is to enable users to get all of the
...
- [X] `- [X] List items`
- Url's can be made `[like this](https://www.UpdiveNSM.org)` [like this](https://www.UpdiveNSM.org)
...
user option, with whatever user you are building as, e.g. `-u UpdiveNSM`

2. A configuration file for building UpdiveNSM docs is already included in the
distribution: `/opt/UpdiveNSM/mkdocs.yml`. The various configuration
directives are documented
[here](https://www.mkdocs.org/user-guide/configuration/).

3. Build from the UpdiveNSM base directory: `cd /opt/UpdiveNSM`.
...
This will output all the documentation in html format to `/opt/UpdiveNSM/out`
...
Now you will find the complete set of UpdiveNSM documentation by opening your
