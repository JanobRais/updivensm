...
UpdiveNSM simplifies sensor state monitoring by translating raw values 
...
and the standardized representation (generic state) that UpdiveNSM uses 
...
generic UpdiveNSM value, in order to make displaying and alerting more
generic. We also map these values to the actual state
sensor(state_index) where these values are actually returned from.*

*The UpdiveNSM generic states are derived from Nagios:*
...
UpdiveNSM offers flexibility in handling sensor states, which can be represented 
as either strings or numbers via SNMP. 

If the sensor state input is a string (i.e. "ONLINE") 
UpdiveNSM will use the 'descr' field and finally translate it to the desired 
...
UpdiveNSM will use the 'value' field and finally translate it to the desired 
generic state (0, 1, 2 or 3).  
