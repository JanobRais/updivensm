...
### <a name="faq1"> How do I install UpdiveNSM?</a>

This is currently well documented within the doc folder of the installation files.

Please see the following [doc](../Installation/Install-UpdiveNSM.md)

### <a name="faq2"> How do I add a device?</a>

You have two options for adding a new device into UpdiveNSM.

1: Using the command line via ssh you can add a new device by changing
   to the directory of your UpdiveNSM install and typing:
...
### <a name="faq4"> What are the supported OSes for installing UpdiveNSM on?</a>
...
We do indeed, you can find access to the demo [here](https://demo.UpdiveNSM.org)


## Support

### <a name='how-does-UpdiveNSM-use-mibs'>How does UpdiveNSM use MIBs?</a>

UpdiveNSM does not parse MIBs to discover sensors for devices.
UpdiveNSM uses static discovery definitions written in YAML or PHP.
Therefore, updating a MIB alone will not improve OS support, the
definitions must be updated.  UpdiveNSM only uses MIBs to make OIDs
easier to read.
...
UpdiveNSM from within your install directory. This should give you info on
...
using the [included
snmpd.conf](https://raw.githubusercontent.com/UpdiveNSM/UpdiveNSM/master/snmpd.conf.example)
file.
...
If this is related to your web service for UpdiveNSM then this has been
...
443](https://github.com/UpdiveNSM/UpdiveNSM/issues/443).
...
see [RRDTune doc](../Extensions/RRDTune.md)
 ...
 at 0 (at device boot) and count up from there. UpdiveNSM records the
...
There is a host rename tool called renamehost.php in your UpdiveNSM
...
Run `./validate.php` as UpdiveNSM from within your install.
...
### <a name="faq24"> How do I migrate my UpdiveNSM install to another server?</a>
...
scripts](https://web.archive.org/web/20180815212723/https://vlan50.com/2015/04/17/migrating-from-observium-to-UpdiveNSM/).
...
- Install UpdiveNSM as per our normal documentation; you don't need to
  run through the web installer or building the sql schema.
- Stop cron by commenting out all lines in `/etc/cron.d/UpdiveNSM`
- Dump the MySQL database `UpdiveNSM` from your old server (`mysqldump
  UpdiveNSM -u root -p > UpdiveNSM.sql`)...
- and import it into your new server (`mysql -u root -p UpdiveNSM < UpdiveNSM.sql`).
...
- Ensure ownership of the copied files and folders (substitute your
  user if necessary) - `chown -R UpdiveNSM:UpdiveNSM /opt/UpdiveNSM`
- Delete old pollers on the GUI (gear icon --> Pollers --> Pollers)
- Validate your installation (/opt/UpdiveNSM/validate.php)
- Re-enable cron by uncommenting all lines in `/etc/cron.d/UpdiveNSM`
...
issue](https://github.com/UpdiveNSM/UpdiveNSM/issues/new).
...
Restart snmpd and UpdiveNSM should populate the additional disk after a fresh discovery.
...
this from the UpdiveNSM directory:
...
update UpdiveNSM with those new details.
Please edit `.env`
...
make sure the timezone is set properly on both the UpdiveNSM **and**
...
to be delivered late if UpdiveNSM is on a timezone later than
MySQL/MariaDB.
...
As of [Ver. 1.55](https://community.UpdiveNSM.org/t/v1-55-release-changelog-august-2019/9428) a new feature has been added where you can view a simple linear prediction in port graphs.
...
There is already a reference how to move your whole LNMS installation to another server. But the following steps will help you to split up an "All-in-one" installation to one UpdiveNSM installation with a separate database install.
*Note: This section assumes you have a MySQL/MariaDB instance

- Stop the apache and mysql service in you UpdiveNSM installation.
- Edit out all the cron entries in `/etc/cron.d/UpdiveNSM`.
- Dump your `UpdiveNSM`database on your current install by issuing `mysqldump UpdiveNSM -u root -p > UpdiveNSM.sql`.
...
- Copy this over to your new database server and import it with `mysql -u root -p UpdiveNSM < UpdiveNSM.sql`.
- Enter to mysql and add permissions with the following two commands:
```sql
GRANT ALL PRIVILEGES ON UpdiveNSM.* TO 'UpdiveNSM'@'IP_OF_YOUR_LNMS_SERVER' IDENTIFIED BY 'PASSWORD' WITH GRANT OPTION;
GRANT ALL PRIVILEGES ON UpdiveNSM.* TO 'UpdiveNSM'@'FQDN_OF_YOUR_LNMS_SERVER' IDENTIFIED BY 'PASSWORD' WITH GRANT OPTION;
FLUSH PRIVILEGES;
exit;
```
- Enable and restart MySQL server.
- Edit your `config.php` file to point the install to the new database server location.
- **Very important**: On your UpdiveNSM server, inside your install directory is a `.env` file, in it you need to edit the `DBHOST` parameter to point to your new server location.
...
Please [open a feature request in the community forum](https://community.UpdiveNSM.org/c/feature-requests) and provide
...
- Code. This is a big thing. We want this community to grow by the
  ...
  - Poller / Discovery code. Improving it (we think a lot can be done
    ...
    ones.
  - The UpdiveNSM main website, this is hosted on GitHub like the main
...
- Feature requests. Can't code / won't code. No worries, chuck a
  ...
  forum](https://community.UpdiveNSM.org) with enough detail and
...
UpdiveNSM can and is developed by anyone, this means someone may be
...
```bash
git remote add f0o https://github.com/f0o/UpdiveNSM.git
