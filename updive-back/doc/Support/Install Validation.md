...
- Checks disk space for where /opt/UpdiveNSM is installed.
- Checks location to fping.
- Tests MySQL strict mode being enabled.
- Tests for files not owned by UpdiveNSM user (if configured).
...
You can run validate.php as `UpdiveNSM` by executing `./validate.php`
...
You can validate your UpdiveNSM install from the WebUI, using the nav
