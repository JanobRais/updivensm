...
1. Add in the "SNMP Host Access List" section your UpdiveNSM server IP address
...
snmp-server host <INTERFACE> <UpdiveNSM-IP> poll community <YOUR-COMMUNITY> version 2c
...
snmp-server contact <YOUR-CONTACT>
snmp-server location <YOUR-LOCATION>
snmp-server host <INTERFACE> <UpdiveNSM-IP> poll version 3 <USER-NAME>
```
...
1. Configure SNMP. The device defaults to both SNMP v1 and v3 enabled, with default credentials. Disable what you do not need. SNMP v3 works, but uses MD5/DES. You may have to add another section to your SNMP credentials table in UpdiveNSM. Save.
...
- Access the CLI
- Add an SNMP Manager with your UpdiveNSM IP address:

```bash
addsnmpmgr <UpdiveNSM ip>
```
...
!!! note
    * About the snmp community commands:
         ...
         * `<ALLOWED-SRC-IPs/NETMASK>` specify the address and host (not network) netmask of the UpdiveNSM server.  Example: 192.168.8.71/32
...
The UpdiveNSM server include a copy of this example here:

```bash
/opt/UpdiveNSM/snmpd.conf.example
```
...
username created above (rouser is what UpdiveNSM uses) :
...
#### PowerShell
The following example will install SNMP, set the UpdiveNSM IP and set a read only community string.  
