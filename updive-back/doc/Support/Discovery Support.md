...
Example: `/opt/UpdiveNSM/discovery-wrapper.py 1 -m bgp-peers`

If you want to switch back to `lnms device:discover` (not recommended), then you can replace:

`33  */6   * * *   UpdiveNSM    /opt/UpdiveNSM/discovery-wrapper.py 1 >> /dev/null 2>&1`

With:

`33  */6   * * *   UpdiveNSM    /opt/UpdiveNSM/lnms device:discover all >> /dev/null 2>&1`
