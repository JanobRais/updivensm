...
You have two options for adding a new device into UpdiveNSM. You can
...
Using the command line via ssh as the `UpdiveNSM` user, you can add a
new device by changing to the directory of your UpdiveNSM install and
...
You can add ping only devices into UpdiveNSM through the WebUI or CLI. When
adding the device switch the SNMP button to "off". The device will be
added into UpdiveNSM as a Ping Only Device and will show ICMP Response Graph.
