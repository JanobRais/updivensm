...
For configuration stored within the database, UpdiveNSM uses dot notation for config
...
!!! note
    ...
    Not all configuration settings have been defined in UpdiveNSM, they can still be 
...
When installing UpdiveNSM for the first time with a new database you can place yaml key value files
...
```bash
lnms config:set log_dir /opt/UpdiveNSM/logs
```

Log files created by UpdiveNSM will be stored within this directory.

## Database config

Set these variables either in .env (/opt/UpdiveNSM/.env by default) or in the environment.
...
You can change the memory limits for php within UpdiveNSM. The
...
!!! setting "system/server"
    ```bash
    lnms config:set base_url http://demo.UpdiveNSM.org
    ```

UpdiveNSM will attempt to detect the URL you are using, but you can override that here.
...
UpdiveNSM can optionally test for MTU issues.  The current implementation only works for devices with
...
UpdiveNSM can discover and monitor SSL/TLS certificates presented by your devices (for example, HTTPS on port 443). This helps you track expiry dates and receive alerts before certificates expire.
...
!!! setting "alerting/email"
    ...
    lnms config:set email_from UpdiveNSM@yourdomain.local
...
in UpdiveNSM should be consistent with the location set in
...
UpdiveNSM can interpret sysLocation information and map the device location based on GeoCoordinates or GeoCoding information.
...
sensors. By default, when no such value is given or UpdiveNSM doesn't have,
...
UpdiveNSM keeps track of average values in the database for some metrics so we
...
1. Generate a non-password-protected ssh key for use by UpdiveNSM, as the
    user which runs polling & discovery (usually `UpdiveNSM`).
2. On each VM host you wish to monitor:
   1. Configure public key authentication from your UpdiveNSM server/poller by
      adding the UpdiveNSM public key to `~root/.ssh/authorized_keys`.
...
`virsh -c xen+ssh://vmhost list` as your UpdiveNSM polling user.
