...
High Availability (HA) in UpdiveNSM ensures continuous operation and minimizes downtime by implementing redundancy for two critical components:
...
Also make sure that the **poller uses a distributed setup** which must be [UpdiveNSM Dispatcher service](../Extensions/Dispatcher-Service.md).
...
The WebUI achieves HA through multiple UpdiveNSM instances sharing these backend services:
...
3. **Deploy multiple UpdiveNSM instances**:
   - Install UpdiveNSM on multiple servers
