...
```bash
time snmpbulkwalk -v2c -cpublic HOSTNAME -Cr-REPEATERS- -M /opt/UpdiveNSM/mibs -m IF-MIB IfEntry
```
...
!!! note
    ...
    CPU. So be careful. This can be changed by going to the cron job for UpdiveNSM.
    Usually in `/etc/cron.d/UpdiveNSM` and changing the "16"

```
*/5  *    * * *   UpdiveNSM    /opt/UpdiveNSM/cronic /opt/UpdiveNSM/poller-wrapper.py 16
```
...
UpdiveNSM server. Something like pdns-recursor can be used and then
...
Running `./scripts/collect-port-polling.php` as the `UpdiveNSM` user
...
Create a cache directory that is writable by the UpdiveNSM user first:
`sudo mkdir -p /tmp/cache && sudo chmod 775 /tmp/cache && sudo chown -R UpdiveNSM /tmp/cache`
