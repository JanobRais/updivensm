...
the UpdiveNSM web interface by going to `/api-access/`.
...
```curl
curl -H 'X-Auth-Token: YOURAPITOKENHERE' https://UpdiveNSM.org/api/v0
```
...
```json
{
 "list_bgp": "https://UpdiveNSM.org/api/v0/bgp",
  ...
 "edit_rule": "https://UpdiveNSM.org/api/v0/rules"
}
```
...
```curl
curl -X POST -d '{"hostname":"localhost.localdomain","version":"v1","community":"public"}' -H 'X-Auth-Token: YOURAPITOKENHERE' https://UpdiveNSM.org/api/v0/devices
```
