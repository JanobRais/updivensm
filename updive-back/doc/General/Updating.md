...
By default, UpdiveNSM is set to automatically update once a day at  00:19 hours.
...
running the following command as the `UpdiveNSM` user:

```bash
./daily.sh
```

This will update the core UpdiveNSM files and also update the database
...
UpdiveNSM by default performs updates on a daily basis.
