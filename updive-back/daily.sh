...
# CONSTANTS
#######################################
# define DAILY_SCRIPT as the full path to this script and UpdiveNSM_DIR as the directory this script is in
DAILY_SCRIPT=$(readlink -f "$0")
UpdiveNSM_DIR=$(dirname "$DAILY_SCRIPT")
COMPOSER="php ${UpdiveNSM_DIR}/scripts/composer_wrapper.php --no-interaction"

# set log_file, using UpdiveNSM 'log_dir' config setting, if set
# otherwise we default to <UpdiveNSM Install Directory>/logs
LOG_DIR=$(php -r "@include '${UpdiveNSM_DIR}/config.php'; echo isset(\$config['log_dir']) ? \$config['log_dir'] : '${UpdiveNSM_DIR}/logs';")

# get the UpdiveNSM user
# shellcheck source=.env.example
source "${UpdiveNSM_DIR}/.env"
UpdiveNSM_USER="${UpdiveNSM_USER:-UpdiveNSM}"
UpdiveNSM_USER_ID=$(id -u "$UpdiveNSM_USER")
...
status_run() {
    ...
    # set log_file, using UpdiveNSM $config['log_dir'], if set
    ...
    if [[ "${exit_code}" == "0" ]]; then
        ...
        if [[ "${arg_option}" == "update" ]]; then
            php "${UpdiveNSM_DIR}/daily.php" -f notify -o "${tmp}"
        fi
        if [[ -n "${tmp}" ]]; then
...
call_daily_php() {
    ...
    for arg in "${args[@]}"; do
        php "${UpdiveNSM_DIR}/daily.php" -f "${arg}"
    done
}
...
set_notifiable_result() {
    ...
    php "${UpdiveNSM_DIR}/daily.php" -f handle_notifiable -t "${arg_type}" -r "${arg_result}"
}
...
check_dependencies() {
    ...
    python_deps=$("${UpdiveNSM_DIR}/scripts/check_requirements.py" > /dev/null 2>&1; echo $?)
    phpver="master"
    pythonver="master"
...
main () {
    ...
    cd "${UpdiveNSM_DIR}" || exit 1

    # if not running as $UpdiveNSM_USER (unless $UpdiveNSM_USER = root), relaunch
    if [[ "$UpdiveNSM_USER" != "root" ]]; then
        if [[ "$EUID" -ne "$UpdiveNSM_USER_ID" ]]; then
    ...
    if [ ! -f "${UpdiveNSM_DIR}/vendor/autoload.php" ]; then
