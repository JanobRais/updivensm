...
<div class="row">
@if (UpdiveNSMConfig::get('vertical_summary'))
    <div class="col-md-9">
@else
    <div class="col-md-8">
@endif
...
<div class="row">
    ...
    @foreach ($devices_down as $device)
        <div class="front-box device-down">
            {!! \UpdiveNSM\Util\Url::deviceLink($device, $device->shortDisplayName()) !!}
            <br />
    ...
    @foreach ($ports_down as $port)
        <div class="front-box alert alert-danger">
            {!! \UpdiveNSM\Util\Url::deviceLink($port->device, $port->device->shortDisplayName()) !!}
            <br />
            <span class="interface-updown">{{ __('Port Down') }}</span>
            <br />
            {!! \UpdiveNSM\Util\Url::PortLink($port) !!}
            @if($port->ifAlias)
                <br />
    ...
    @foreach ($services_down as $service)
        <div class="front-box service-down">
            {!! \UpdiveNSM\Util\Url::deviceLink($service->device, $service->device->shortDisplayName()) !!}
    ...
    @foreach ($bgp_down as $bgp)
        <div class="front-box bgp-down">
            {!! \UpdiveNSM\Util\Url::deviceLink($bgp->device, $bgp->device->shortDisplayName()) !!}
...
@if(
    ...
    <p class=welcome>
        ...
        You might have none because you run such a great network, or perhaps you've just started using {{ UpdiveNSMConfig::get('project_name') }}
        If you're new to {{ UpdiveNSMConfig::get('project_name') }}, you might
            ...
            <x-panel title="{{ __('Syslog entries') }}">
                <x-slot name="table">
                    ...
                    @foreach ($syslog as $entry)
                        <tr>
                            <td>{{ $entry->date }}</td>
                            <td><strong>{!! \UpdiveNSM\Util\Url::deviceLink($entry->device) !!}</strong></td>
