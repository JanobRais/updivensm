    ...
    <x-device.page :device="$device">
        ...
        <div class="row">
            <div class="col-sm-6 col-sm-offset-2 tw:justify-between tw:flex tw:flex-wrap">
                ...
                @if(UpdiveNSMConfig::get('enable_clear_discovery') && ! $device->snmp_disable)
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                ...
                @endif
                {{ __('device.edit.last_polled') }}: <b>{{ $device->last_polled ? \UpdiveNSM\Util\Time::format($device->last_polled, 'byminute') : $device->last_polled }}</b>
                @if($device->last_discovered)
                    | {{ __('device.edit.last_discovered') }}: <b>{{ $device->last_discovered ? \UpdiveNSM\Util\Time::format($device->last_discovered, 'byminute') : $device->last_discovered }}</b>
                @endif
            </div>
        </div>
    </x-device.page>
