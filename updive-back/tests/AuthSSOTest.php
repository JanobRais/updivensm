...
use UpdiveNSM\Authentication\LegacyAuth;
...
final class AuthSSOTest extends DBTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->original_auth_mech = UpdiveNSMConfig::get('auth_mechanism');
        UpdiveNSMConfig::set('auth_mechanism', 'sso');
    ...
    {
        UpdiveNSMConfig::set('sso.mode', 'env');
        UpdiveNSMConfig::set('sso.create_users', true);
        UpdiveNSMConfig::set('sso.update_users', true);
        UpdiveNSMConfig::set('sso.trusted_proxies', ['127.0.0.1', '::1']);
        UpdiveNSMConfig::set('sso.user_attr', 'REMOTE_USER');
        UpdiveNSMConfig::set('sso.realname_attr', 'displayName');
        UpdiveNSMConfig::set('sso.email_attr', 'mail');
        UpdiveNSMConfig::set('sso.descr_attr', null);
        UpdiveNSMConfig::set('sso.level_attr', null);
        UpdiveNSMConfig::set('sso.group_strategy', 'static');
        UpdiveNSMConfig::set('sso.group_attr', 'member');
        UpdiveNSMConfig::set('sso.group_filter', '/(.*)/i');
        UpdiveNSMConfig::set('sso.group_delimiter', ';');
        UpdiveNSMConfig::set('sso.group_level_map', null);
        UpdiveNSMConfig::set('sso.static_level', -1);
    }
    ...
    public function basicEnvironmentHeader()
    {
        ...
        UpdiveNSMConfig::set('sso.mode', 'header');
    ...
    public function testValidAuthNoCreateUpdate(): void
    {
        ...
        UpdiveNSMConfig::set('sso.create_users', false);
        UpdiveNSMConfig::set('sso.update_users', false);
    ...
    {
        $this->basicConfig();
        /** @var \UpdiveNSM\Authentication\SSOAuthorizer */
        $a = LegacyAuth::reset();
        ...
        $this->assertSame($dbuser->realname, $a->authSSOGetAttr(UpdiveNSMConfig::get('sso.realname_attr')));
        $this->assertEmpty($dbuser->getRoleNames());
        $this->assertSame($dbuser->email, $a->authSSOGetAttr(UpdiveNSMConfig::get('sso.email_attr')));

        // Change a few things and reauth
        $_SERVER['mail'] = 'test@example.net';
        $_SERVER['displayName'] = 'Testier User';
        UpdiveNSMConfig::set('sso.static_level', 10);
        $this->assertTrue($a->authenticate(['username' => $user]));

        // Retrieve it and validate the update was not persisted
        $dbuser = User::thisAuth()->where('username', $user)->firstOrNew();
        $this->assertFalse($a->authSSOGetAttr(UpdiveNSMConfig::get('sso.realname_attr')) === $dbuser->realname);
        $this->assertFalse($dbuser->roles()->where('name', 'admin')->exists());
        $this->assertFalse($a->authSSOGetAttr(UpdiveNSMConfig::get('sso.email_attr')) === $dbuser->email);
    }
    ...
    public function testBadAuth(): void
    {
        ...
        $this->expectException(\UpdiveNSM\Exceptions\AuthenticationException::class);
        $a->authenticate([]);
    ...
    public function testGetExternalUserName(): void
    {
        ...
        UpdiveNSMConfig::forget('sso.user_attr');
        $this->assertNull($a->getExternalUsername());
        $this->basicEnvironmentEnv();

        // Non-existant attribute
        UpdiveNSMConfig::set('sso.user_attr', 'foobar');
        $this->assertNull($a->getExternalUsername());
        $this->basicEnvironmentEnv();

        // null pointer to attribute
        UpdiveNSMConfig::set('sso.user_attr', null);
        $this->assertNull($a->getExternalUsername());
        $this->basicEnvironmentEnv();
    ...
    public function testTrustedProxies(): void
    {
        ...
        UpdiveNSMConfig::set('sso.trusted_proxies', ['127.0.0.1', '::1', '2001:630:50::/48', '8.8.8.0/25']);
        ...
        // Not a list
        UpdiveNSMConfig::set('sso.trusted_proxies', '8.8.8.0/25');
        $_SERVER['REMOTE_ADDR'] = '8.8.8.8';
        $this->assertFalse($a->authSSOProxyTrusted());
    ...
    {
        /** @var \UpdiveNSM\Authentication\SSOAuthorizer $a */
        $a = LegacyAuth::reset();

        UpdiveNSMConfig::set('sso.mode', 'env');
        UpdiveNSMConfig::set('sso.group_strategy', 'attribute');

        //Integer
        UpdiveNSMConfig::set('sso.level_attr', 'level');
        $_SERVER['level'] = 5;
        ...
        //Unset pointer
        UpdiveNSMConfig::forget('sso.level_attr');
        $_SERVER['level'] = '9';
        $this->expectException(\UpdiveNSM\Exceptions\AuthenticationException::class);
        $a->getRoles('');
    ...
    public function testGroupParsing(): void
    {
        ...
        UpdiveNSMConfig::set('sso.static_level', 0);
        UpdiveNSMConfig::set('sso.group_strategy', 'map');
        UpdiveNSMConfig::set('sso.group_delimiter', ';');
        UpdiveNSMConfig::set('sso.group_attr', 'member');
        UpdiveNSMConfig::set('sso.group_level_map', ['UpdiveNSM-admins' => 10, 'UpdiveNSM-readers' => 1, 'UpdiveNSM-billingcontacts' => 5]);
        $_SERVER['member'] = 'UpdiveNSM-admins;UpdiveNSM-readers;UpdiveNSM-billingcontacts;unrelatedgroup;confluence-admins';
        ...
        UpdiveNSMConfig::set('sso.static_level', 5);
        $this->assertSame(5, $a->authSSOParseGroups());
        UpdiveNSMConfig::forget('sso.static_level');
        ...
        // Empty
        UpdiveNSMConfig::set('sso.group_level_map', []);
        $this->assertSame(0, $a->authSSOParseGroups());

        // Not associative
        UpdiveNSMConfig::set('sso.group_level_map', ['foo', 'bar', 'UpdiveNSM-admins']);
        $this->assertSame(0, $a->authSSOParseGroups());
        ...
        // Unset
        UpdiveNSMConfig::forget('sso.group_level_map');
        $this->assertSame(0, $a->authSSOParseGroups());

        // No delimiter
        UpdiveNSMConfig::forget('sso.group_delimiter');
        $this->assertSame(0, $a->authSSOParseGroups());

        // Test group filtering by regex
        UpdiveNSMConfig::set('sso.group_filter', '/confluence-(.*)/i');
        UpdiveNSMConfig::set('sso.group_delimiter', ';');
        UpdiveNSMConfig::set('sso.group_level_map', ['UpdiveNSM-admins' => 10, 'UpdiveNSM-readers' => 1, 'UpdiveNSM-billingcontacts' => 5, 'confluence-admins' => 7]);
        $this->assertSame(7, $a->authSSOParseGroups());

        // Test group filtering by empty regex
        UpdiveNSMConfig::set('sso.group_filter', '');
        $this->assertSame(10, $a->authSSOParseGroups());

        // Test group filtering by null regex
        UpdiveNSMConfig::set('sso.group_filter', null);
        $this->assertSame(10, $a->authSSOParseGroups());
    }

    protected function tearDown(): void
    {
        UpdiveNSMConfig::set('auth_mechanism', $this->original_auth_mech);
        UpdiveNSMConfig::forget('sso');
        $_SERVER = $this->server;
        parent::tearDown();
    }
}
