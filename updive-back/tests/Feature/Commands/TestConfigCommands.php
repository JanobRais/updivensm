...
namespace UpdiveNSM\Tests\Feature\Commands;

use App\Facades\UpdiveNSMConfig;
use UpdiveNSM\Tests\InMemoryDbTestCase;
...
final class TestConfigCommands extends InMemoryDbTestCase
{
    ...
    {
        // simple
        UpdiveNSMConfig::set('login_message', null);
        $this->assertCliSets('login_message', 'hello');

        // nested
        UpdiveNSMConfig::forget('allow_entity_sensor.amperes');
        ...
        // test append
        $community = UpdiveNSMConfig::get('snmp.community');
        ...
        $this->assertEquals(['ethernet', 'psuedowire'], UpdiveNSMConfig::get('os.netonix.bad_iftype'));
        ...
        $this->artisan('config:get', ['--dump' => true])
            ->expectsOutput(UpdiveNSMConfig::toJson())
            ->assertExitCode(0);
    }
    ...
    public function testInvalidSetting(): void
    {
        ...
        // append to non-array
        UpdiveNSMConfig::set('login_message', 'blah');
        UpdiveNSMConfig::get('login_message');
    ...
    {
        $this->assertSame($expected, UpdiveNSMConfig::get($setting));
