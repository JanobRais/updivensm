...
final class SocialiteControllerTest extends TestCase
{
    ...
    private function runSetRolesFromClaimTest(
        ...
        \App\Facades\UpdiveNSMConfig::set('auth.socialite.scopes', $scopes);
        \App\Facades\UpdiveNSMConfig::set('auth.socialite.claims', $claimMap);
        \App\Facades\UpdiveNSMConfig::set('auth.socialite.debug', false);
    ...
    public function testSetRolesFromClaimSaml2Admin(): void
    {
        ...
        $result = $this->runSetRolesFromClaimTest(
            ...
            ['admin'],
            [
                'G_UpdiveNSM_admins' => ['roles' => ['admin']],
                'G_UpdiveNSM_users' => ['roles' => ['global-read']],
            ]
        );
        $this->assertTrue($result);
    }
