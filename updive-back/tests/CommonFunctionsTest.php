...
final class CommonFunctionsTest extends TestCase
{
    ...
    public function testRrdDescriptions(): void
    {
        ...
        $this->assertEquals('Toner, S/N CR_UM-16021314488.', \UpdiveNSM\Data\Store\Rrd::safeDescr($data));
    }
    ...
    {
        $this->assertSame('UpdiveNSM\OS\Os', StringHelpers::toClass('OS', 'UpdiveNSM\\OS\\'));
        ...
        $this->assertSame('UpdiveNSM\\AllOfThemName', StringHelpers::toClass('all OF-thEm_NaMe', 'UpdiveNSM\\'));
    }
...
//        $this->assertSame($expected, ResolveGlues(array('bill_data'), 'device_id'));
    ...
    public function testFormatHostname(): void
    {
        ...
        // default {{ $hostname }}
        UpdiveNSMConfig::set('device_display_default', null);
        $this->assertEquals('test.UpdiveNSM.org', format_hostname($device_dns));
        ...
        $this->assertEquals('Custom Display (test.UpdiveNSM.org sysName)', format_hostname($custom_display));

        // ip to sysname
        UpdiveNSMConfig::set('device_display_default', '{{ $sysName_fallback }}');
        $this->assertEquals('test.UpdiveNSM.org', format_hostname($device_dns));
        $this->assertEquals('Not DNS', format_hostname($invalid_dns));
        ...
        // sysname
        UpdiveNSMConfig::set('device_display_default', '{{ $sysName }}');
