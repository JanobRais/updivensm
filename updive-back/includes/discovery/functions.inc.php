...
/*
 * UpdiveNSM Network Management and Monitoring System
...
use UpdiveNSM\Exceptions\HostExistsException;
...
function discover_new_device($hostname, $device, $method, $interface = null)
{
    ...
    if (empty(UpdiveNSMConfig::get('nets'))) {
    ...
    if (IP::isValid($hostname)) {
        $ip = $hostname;
        if (! UpdiveNSMConfig::get('discovery_by_ip', false)) {
    ...
    } elseif (\UpdiveNSM\Util\Validate::hostname($hostname)) {
        if ($mydomain = UpdiveNSMConfig::get('mydomain')) {
    ...
    $ip = IP::parse($ip, true);
    if ($ip->inNetworks(UpdiveNSMConfig::get('autodiscovery.nets-exclude'))) {
        Log::debug("$ip in an excluded network - skipping\n");

        return false;
    }

    if (! $ip->inNetworks(UpdiveNSMConfig::get('nets'))) {
...
{
    $fringe = array_merge(UpdiveNSMConfig::get('bad_entity_sensor_regex', []), UpdiveNSMConfig::getOsSetting($device['os'], 'bad_entity_sensor_regex', []));
...
function sensors($types, $os, $pre_cache = [])
{
    ...
    foreach ((array) $types as $sensor_class) {
        ...
        if (UpdiveNSMConfig::getOsSetting($device['os'], 'rfc1628_compat', false)) {
...
function add_bgp_peer($device, $peer)
{
    if (dbFetchCell('SELECT COUNT(*) from `bgpPeers` WHERE device_id = ? AND bgpPeerIdentifier = ?', [$device['device_id'], $peer['ip']]) < '1') {
        ...
        if (UpdiveNSMConfig::get('autodiscovery.bgp')) {
...
{
    if ($sysName) {
        foreach ((array) UpdiveNSMConfig::get('autodiscovery.xdp_exclude.sysname_regexp') as $needle) {
    ...
    if ($sysDescr) {
        foreach ((array) UpdiveNSMConfig::get('autodiscovery.xdp_exclude.sysdesc_regexp') as $needle) {
    ...
    if ($platform) {
        foreach ((array) UpdiveNSMConfig::get('autodiscovery.cdp_exclude.platform_regexp') as $needle) {
...
function find_device_id($name = '', $ip = '', $mac_address = '')
{
    ...
    if ($name && \UpdiveNSM\Util\Validate::hostname($name)) {
        $where[] = '`hostname`=?';
        $params[] = $name;
