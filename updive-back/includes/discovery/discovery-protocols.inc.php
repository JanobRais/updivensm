    ...
    foreach ($fdp_array as $ifIndex => $fdp_if_array) {
        ...
        foreach ($fdp_if_array as $fdp) {
            ...
            if (! $remote_device_id &&
                ...
                if (UpdiveNSMConfig::get('autodiscovery.xdp') === true) {
    ...
    foreach ($cdp_array as $ifIndex => $cdp_if_array) {
        ...
        foreach ($cdp_if_array as $cdp) {
            ...
            if (
                ...
                UpdiveNSMConfig::get('autodiscovery.xdp') === true
            ) {
                $remote_device_id = discover_new_device($cdp['cdpCacheDeviceId'], $device, 'CDP', $port);

                if ($cdp_ip && ! $remote_device_id && UpdiveNSMConfig::get('discovery_by_ip', false)) {
...
if (($device['os'] == 'routeros') && version_compare($device['version'], '7.7', '<')) {
    ...
    if (! empty($lldp_array)) {
        ...
        foreach ($lldp_array as $key => $lldp) {
            ...
            if (! $remote_device_id &&
                    \UpdiveNSM\Util\Validate::hostname($lldp['lldpRemSysName']) &&
    ...
    foreach ($lldp_array as $lldp_array_inner) {
        foreach ($lldp_array_inner as $lldp) {
            ...
            if (UpdiveNSMConfig::get('autodiscovery.xdp') && isset($lldp['lldpRemSysName']) && ! $remote_device_id &&
                    \UpdiveNSM\Util\Validate::hostname($lldp['lldpRemSysName']) &&
                    ! can_skip_discovery($lldp['lldpRemSysName'], $lldp['lldpRemSysDesc'])
            ) {
                $remote_device_id = discover_new_device($lldp['lldpRemSysName'], $device, 'LLDP', $port);
        ...
        foreach ($sub_lldp_1 as $ifIndex => $sub_lldp_2) {
            foreach ($sub_lldp_2 as $sub_lldp_3) {
                foreach ($sub_lldp_3 as $lldp) {
                    ...
                    if (! $remote_device_id &&
                            \UpdiveNSM\Util\Validate::hostname($lldp['tmnxLldpRemSysName']) &&
    ...
    foreach ($lldp_array as $lldp_if_array) {
        foreach ($lldp_if_array as $entry_key => $lldp_instance) {
            ...
            foreach ($lldp_instance as $lldp) {
                ...
                if (! $remote_device_id && UpdiveNSMConfig::get('autodiscovery.xdp') && ! can_skip_discovery($lldp['lldpRemSysName'] ?? null, $lldp['lldpRemSysDesc'] ?? null)) {
                    if (isset($lldp['lldpRemSysName'])) {
                        $remote_device_id = discover_new_device($lldp['lldpRemSysName'], $device, 'LLDP', $port);
                    }
                    if (! $remote_device_id && UpdiveNSMConfig::get('discovery_by_ip', false)) {
...
if (UpdiveNSMConfig::get('autodiscovery.ospf') === true) {
    echo ' OSPF Discovery: ';
    ...
    foreach (dbFetchRows($sql, [$device['device_id']]) as $nbr) {
        ...
        } catch (\UpdiveNSM\Exceptions\InvalidIpException) {
            //
        }
    }
    echo PHP_EOL;
...
if (UpdiveNSMConfig::get('autodiscovery.ospfv3') === true) {
