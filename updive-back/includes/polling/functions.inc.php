...
use UpdiveNSM\Enum\Sensor;
use UpdiveNSM\Enum\Severity;
use UpdiveNSM\Exceptions\JsonAppBase64DecodeException;
use UpdiveNSM\Exceptions\JsonAppBlankJsonException;
use UpdiveNSM\Exceptions\JsonAppExtendErroredException;
use UpdiveNSM\Exceptions\JsonAppGzipDecodeException;
use UpdiveNSM\Exceptions\JsonAppMissingKeysException;
use UpdiveNSM\Exceptions\JsonAppParsingFailedException;
use UpdiveNSM\Exceptions\JsonAppPollingFailedException;
use UpdiveNSM\Exceptions\JsonAppWrongVersionException;
use UpdiveNSM\RRD\RrdDefinition;
use UpdiveNSM\Util\Debug;
use UpdiveNSM\Util\Number;
use UpdiveNSM\Util\Oid;
use UpdiveNSM\Util\UserFuncHelper;
...
function poll_sensor($device, $class)
{
    ...
    foreach ($sensors as $sensor) {
        ...
        if ($sensor['poller_type'] == 'snmp') {
            ...
            if (file_exists(UpdiveNSMConfig::get('install_dir') . '/includes/polling/sensors/' . $class . '/' . $device['os'] . '.inc.php')) {
                require UpdiveNSMConfig::get('install_dir') . '/includes/polling/sensors/' . $class . '/' . $device['os'] . '.inc.php';
            } elseif (isset($device['os_group']) && file_exists(UpdiveNSMConfig::get('install_dir') . '/includes/polling/sensors/' . $class . '/' . $device['os_group'] . '.inc.php')) {
                require UpdiveNSMConfig::get('install_dir') . '/includes/polling/sensors/' . $class . '/' . $device['os_group'] . '.inc.php';
            }
 ...
 * https://github.com/UpdiveNSM/UpdiveNSM-agent/blob/master/utils/lnms_return_optimizer
