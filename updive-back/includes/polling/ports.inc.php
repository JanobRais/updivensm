...
if ($device['os'] === 'f5' && (version_compare($device['version'], '11.2.0', '>=') && version_compare($device['version'], '11.7', '<'))) {
    ...
    if ($selected_attrib !== null ? $selected_attrib == 'true' : UpdiveNSMConfig::getOsSetting($device['os'], 'polling.selected_ports')) {
        ...
        $walk_base = false;
        if (! UpdiveNSMConfig::has("os.{$device['os']}.polling.selected_ports") && $selected_attrib === null) {
        ...
        if (! in_array(strtolower($device['hardware'] ?? ''), array_map(strtolower(...), (array) UpdiveNSMConfig::getOsSetting($device['os'], 'bad_ifXEntry', [])))) {
        ...
        if ($device['os'] != 'asa') {
            ...
            if (UpdiveNSMConfig::get('enable_ports_poe') || UpdiveNSMConfig::get('enable_ports_etherlike')) {
...
if (UpdiveNSMConfig::get('enable_ports_poe')) {
    // Code by OS device
...
// By default UpdiveNSM uses the ifIndex to associate ports on devices with ports discoverd/polled
...
$port_association_mode = UpdiveNSMConfig::get('default_port_association_mode');
...
foreach ($port_stats as $ifIndex => $port) {
    ...
    if (is_port_valid($port, $device)) {
        ...
        if (! $port_id || empty($ports[$port_id])) {
             ...
             * value is empty().
             */
            if (UpdiveNSMConfig::get('ignore_unmapable_port') === true && in_array($port[$port_association_mode], ['', null])) {
                continue;
            }
...
foreach ($ports as $port) {
    ...
    if ($port_stats[$ifIndex]) {
            ...
            if ($port[$oid] != $current_oid && ! isset($current_oid)) {
                ...
                if ($oid == 'ifSpeed') {
                    ...
                    if ($port_tune == 'true' ||
                        ...
                        (UpdiveNSMConfig::get('rrdtool_tune') == 'true' && $port_tune != 'false' && $device_tune != 'false')) {
        ...
        if (UpdiveNSMConfig::has('port_descr_parser') && is_file(UpdiveNSMConfig::get('install_dir') . '/' . UpdiveNSMConfig::get('port_descr_parser'))) {
            $port_attribs = [
                'type',
                'descr',
                'circuit',
            ...
            $port_parser ??= include UpdiveNSMConfig::get('install_dir') . '/' . UpdiveNSMConfig::get('port_descr_parser');
        ...
        if (! empty($port['skipped'])) {
            ...
            if (UpdiveNSMConfig::get('debug_port.' . $port['port_id'])) {
            ...
            // End Update PAgP
            // Do EtherLike-MIB
            if (UpdiveNSMConfig::get('enable_ports_etherlike')) {
