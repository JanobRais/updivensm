...
if ($device['os_group'] == 'unix' || $device['os'] == 'windows') {
    echo \App\Facades\UpdiveNSMConfig::get('project_name') . ' UNIX Agent: ';

    $agent_port = get_dev_attrib($device, 'override_Unixagent_port');
    if (empty($agent_port)) {
        $agent_port = \App\Facades\UpdiveNSMConfig::get('unix-agent.port');
    }

    $agent_start = microtime(true);
    $agent = null;
    try {
        $poller_target = \UpdiveNSM\Util\Rewrite::addIpv6Brackets(DeviceCache::getPrimary()->pollerTarget());
        $agent = @fsockopen($poller_target, $agent_port, $errno, $errstr, \App\Facades\UpdiveNSMConfig::get('unix-agent.connection-timeout'));
    ...
    if (! $agent) {
        ...
        stream_set_timeout($agent, \App\Facades\UpdiveNSMConfig::get('unix-agent.read-timeout'));
