...
function cleanPort($interface, $device = null)
{
    ...
    if (isset($device['os'])) {
        ...
        if (UpdiveNSMConfig::get("os.$os.ifname")) {
            $interface['label'] = $interface['ifName'];

            if ($interface['ifName'] == '') {
                $interface['label'] = $interface['ifDescr'];
            }
        } elseif (UpdiveNSMConfig::get("os.$os.ifalias")) {
            $interface['label'] = $interface['ifAlias'];
        } elseif (UpdiveNSMConfig::get("os.$os.ifindex")) {
    ...
    if (is_array(UpdiveNSMConfig::get('rewrite_if'))) {
        foreach (UpdiveNSMConfig::get('rewrite_if') as $src => $val) {
    ...
    if (is_array(UpdiveNSMConfig::get('rewrite_if_regexp'))) {
        foreach (UpdiveNSMConfig::get('rewrite_if_regexp') as $reg => $val) {
