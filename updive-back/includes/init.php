...
if (! module_selected('nodb', $init_modules)) {
    if (! \UpdiveNSM\DB\Eloquent::isConnected()) {
        echo "Could not connect to database, check logs/UpdiveNSM.log.\n";
        ...
        if (! extension_loaded('mysqlnd') || ! extension_loaded('pdo_mysql')) {
            ...
            echo "Check the install docs for more info: https://docs.UpdiveNSM.org/Installation/\n";
        }

        exit(1);
    }
}
\UpdiveNSM\DB\Eloquent::setStrictMode(false); // disable strict mode for legacy code...

if (is_numeric(UpdiveNSMConfig::get('php_memory_limit')) && UpdiveNSMConfig::get('php_memory_limit') > 128) {
    ini_set('memory_limit', UpdiveNSMConfig::get('php_memory_limit') . 'M');
}
