...
// FIXME port to UpdiveNSM\Util\IPv6 class
...
function is_port_valid($port, $device)
{
    ...
    if (empty($port['ifDescr'])) {
        ...
        if (! UpdiveNSMConfig::getOsSetting($device['os'], 'empty_ifdescr', UpdiveNSMConfig::get('empty_ifdescr', false))) {
    ...
    if (Str::contains($ifDescr, UpdiveNSMConfig::getOsSetting($device['os'], 'good_if', UpdiveNSMConfig::get('good_if')), ignoreCase: true)) {
        return true;
    }

    foreach (UpdiveNSMConfig::getCombined($device['os'], 'bad_if') as $bi) {
    ...
    foreach (UpdiveNSMConfig::getCombined($device['os'], 'bad_if_regexp') as $bir) {
    ...
    foreach (UpdiveNSMConfig::getCombined($device['os'], 'bad_ifname_regexp') as $bnr) {
    ...
    foreach (UpdiveNSMConfig::getCombined($device['os'], 'bad_ifalias_regexp') as $bar) {
    ...
    foreach (UpdiveNSMConfig::getCombined($device['os'], 'bad_iftype') as $bt) {
    ...
    foreach (UpdiveNSMConfig::getCombined($device['os'], 'bad_ifoperstatus') as $bos) {
...
function hytera_h2f($number, $nd)
{
    if (strlen(str_replace(' ', '', $number)) == 4) {
        $number = \UpdiveNSM\Util\StringHelpers::asciiToHex($number, ' ');
    }
    $r = '';
...
{
    if (UpdiveNSMConfig::get('peeringdb.enabled') === true) {
        ...
        if ($cached == 0) {
            ...
            foreach (dbFetchRows('SELECT `bgpLocalAs` FROM `devices` WHERE `disabled` = 0 AND `ignore` = 0 AND `bgpLocalAs` > 0 AND (`bgpLocalAs` < 64512 OR `bgpLocalAs` > 65535) AND `bgpLocalAs` < 4200000000 GROUP BY `bgpLocalAs`') as $as) {
                $asn = $as['bgpLocalAs'];
                $get = \UpdiveNSM\Util\Http::client()->get($peeringdb_url . '/net?depth=2&asn=' . $asn);
                $json_data = $get->body();
                ...
                foreach ($ixs ?? [] as $ix) {
                    ...
                    $ix_keep[] = $pdb_ix_id;
                    $get_ix = \UpdiveNSM\Util\Http::client()->get("$peeringdb_url/netixlan?ix_id=$ixid");
                    $ix_json = $get_ix->body();
                    $ix_data = json_decode($ix_json);
                    $peers = $ix_data->{'data'};
                    foreach ($peers ?? [] as $peer) {
                        $peer_name = \UpdiveNSM\Util\AutonomousSystem::get($peer->{'asn'})->name();
...
function get_device_oid_limit($device)
{
    ...
    // then os
    $os_max = UpdiveNSMConfig::getOsSetting($device['os'], 'snmp_max_oid', 0);
    if ($os_max > 0) {
        return $os_max;
    }

    // then global
    $global_max = UpdiveNSMConfig::get('snmp.max_oid', 10);
...
function lock_and_purge($table, $sql)
{
    ...
    if ($lock->get()) {
        $purge_days = UpdiveNSMConfig::get($purge_name);
...
function lock_and_purge_query($table, $sql, $msg)
{
    ...
    $purge_duration = UpdiveNSMConfig::get($purge_name);
