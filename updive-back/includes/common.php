...
function external_exec($command)
{
    ...
    $proc->setTimeout(UpdiveNSMConfig::get('snmp.exec_timeout', 1200));
...
function shorthost($hostname, $len = 12)
{
    ...
    $len = UpdiveNSMConfig::get('shorthost_target_length', $len);
...
function get_sensor_rrd_name($device, $sensor)
{
    ...
    if (UpdiveNSMConfig::getOsSetting($device['os'], 'sensor_descr') || $sensor['poller_type'] == 'ipmi') {
...
{
    // fake a port model
    return \UpdiveNSM\Util\Url::portLinkDisplayClass((object) [
        'ifOperStatus' => $ifOperStatus instanceof \UpdiveNSM\Enum\IfOperStatus ? $ifOperStatus : \UpdiveNSM\Enum\IfOperStatus::tryFrom($ifOperStatus),
        'ifAdminStatus' => $ifAdminStatus instanceof \UpdiveNSM\Enum\IfOperStatus ? $ifAdminStatus : \UpdiveNSM\Enum\IfOperStatus::tryFrom($ifAdminStatus),
    ]);
}
...
{
    if (UpdiveNSMConfig::get('allow_unauth_graphs', false)) {
        d_echo("Unauthorized graphs allowed\n");

        return true;
    }

    foreach (UpdiveNSMConfig::get('allow_unauth_graphs_cidr', []) as $range) {
        try {
...
{
    $smokeping = new \UpdiveNSM\Util\Smokeping(DeviceCache::get((int) $device['device_id']));
...
function format_hostname($device): string
{
    ...
    return \App\View\SimpleTemplate::parse(empty($device['display']) ? UpdiveNSMConfig::get('device_display_default', '{{ $hostname }}') : $device['display'], [
        'hostname' => $hostname,
        'sysName' => $sysName,
...
function get_vm_parent_id($device)
{
    ...
    return dbFetchCell('SELECT `device_id` FROM `vminfo` WHERE `vmwVmDisplayName` = ? OR `vmwVmDisplayName` = ?', [$device['hostname'], $device['hostname'] . '.' . UpdiveNSMConfig::get('mydomain')]);
}
