...
function prep_snmp_setting($device, $setting)
{
    ...
    return UpdiveNSMConfig::get("snmp.$setting");
...
function get_mib_dir($device)
{
    ...
    if (isset($device['os']) && file_exists(UpdiveNSMConfig::get('mib_dir') . '/' . $device['os'])) {
        $dirs[] = UpdiveNSMConfig::get('mib_dir') . '/' . $device['os'];
    }

    if (isset($device['os_group'])) {
        if (file_exists(UpdiveNSMConfig::get('mib_dir') . '/' . $device['os_group'])) {
            $dirs[] = UpdiveNSMConfig::get('mib_dir') . '/' . $device['os_group'];
        }

        if ($group_mibdir = UpdiveNSMConfig::get("os_groups.{$device['os_group']}.mib_dir")) {
            if (is_array($group_mibdir)) {
                foreach ($group_mibdir as $dir) {
                    $dirs[] = UpdiveNSMConfig::get('mib_dir') . '/' . $dir;
                }
            }
        }
    }

    if (isset($device['os']) && ($os_mibdir = UpdiveNSMConfig::get("os.{$device['os']}.mib_dir"))) {
        $dirs[] = UpdiveNSMConfig::get('mib_dir') . '/' . $os_mibdir;
    }
 ...
 *
 * @param  string  $mibdir  should be the name of the directory within \App\Facades\UpdiveNSMConfig::get('mib_dir')
...
function mibdir($mibdir = null, $device = null)
{
    ...
    $base = UpdiveNSMConfig::get('mib_dir');
    $dirs[] = "$base/$mibdir";
...
{
    $snmpcmd = [UpdiveNSMConfig::get('snmpget')];
...
function gen_snmpwalk_cmd($device, $oids, $options = null, $mib = null, $mibdir = null)
{
    ...
    if ($device['snmpver'] == 'v1'
        || (isset($device['os']) && (UpdiveNSMConfig::getOsSetting($device['os'], 'snmp_bulk', true) == false
                || ! empty(array_intersect($oids, UpdiveNSMConfig::getCombined($device['os'], 'oids.no_bulk', 'snmp.'))))) // skip for oids that do not work with bulk
    ) {
        $snmpcmd = [UpdiveNSMConfig::get('snmpwalk')];
    } else {
        $snmpcmd = [UpdiveNSMConfig::get('snmpbulkwalk')];
    ...
    if (! empty(array_intersect($oids, UpdiveNSMConfig::getCombined($device['os'], 'oids.unordered', 'snmp.')))) {
        $snmpcmd[] = '-Cc';
    }
...
function snmp_getnext($device, $oid, $options = null, $mib = null, $mibdir = null)
{
    ...
    $snmpcmd = [UpdiveNSMConfig::get('snmpgetnext', 'snmpgetnext')];
...
function get_device_max_repeaters($device)
{
    ...
    return $attrib ?? UpdiveNSMConfig::getOsSetting($device['os'], 'snmp.max_repeaters', UpdiveNSMConfig::get('snmp.max_repeaters', false));
}
