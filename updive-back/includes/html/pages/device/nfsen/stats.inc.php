...
echo '<form action = "' . \UpdiveNSM\Util\Url::generate($link_array, ['nfsen' => 'stats']) . '" iOCd = "FlowStats" method = "SUBMIT">';

echo 'Top N:
<select name = "topN" id = "topN" size = 1>
';

$option_default = $vars['topN'] ?? \App\Facades\UpdiveNSMConfig::get('nfsen_top_default');

$option_int = 0;
foreach (\App\Facades\UpdiveNSMConfig::get('nfsen_top_N') as $option) {
...
$option_keys = array_keys(\App\Facades\UpdiveNSMConfig::get('nfsen_lasts'));
$options = \App\Facades\UpdiveNSMConfig::get('nfsen_lasts');
...
$option_default = $vars['stattype'] ?? \App\Facades\UpdiveNSMConfig::get('nfsen_stats_default');
...
$option_default = \App\Facades\UpdiveNSMConfig::get('nfsen_order_default');
...
if (isset($vars['process'])) {
    ...
    if (isset($vars['lastN']) &&
         ...
         ($vars['lastN'] <= \App\Facades\UpdiveNSMConfig::get('nfsen_last_max'))
    ) {
    ...
    if (isset($vars['topN']) &&
         ...
         ($vars['topN'] <= \App\Facades\UpdiveNSMConfig::get('nfsen_top_max'))
    ) {
        $topN = (int) $vars['topN'];
    }
    ...
    $command = [
        \App\Facades\UpdiveNSMConfig::get('nfdump'),
        '-M',
