...
if ($mempools->isNotEmpty()) {
    ...
    $graph = \App\Http\Controllers\Device\Tabs\OverviewController::setGraphWidth([
        ...
        'type' => 'device_mempool',
        'from' => \App\Facades\UpdiveNSMConfig::get('time.day'),
        'legend' => 'no',
        'popup_title' => DeviceCache::getPrimary()->hostname . ' - Memory Usage',
    ]);
    echo \UpdiveNSM\Util\Url::graphPopup($graph, \UpdiveNSM\Util\Url::lazyGraphTag($graph), $mempools_url);
    echo '  </td>
            </tr>';
    ...
    foreach ($mempools as $mempool) {
        ...
        $graph_array = [
            ...
            'width' => 210,
            'from' => \App\Facades\UpdiveNSMConfig::get('time.day'),
            'to' => \App\Facades\UpdiveNSMConfig::get('time.now'),
            'legend' => 'no',
        ];
        ...
        echo '<tr>
            <td class="col-md-4">' . \UpdiveNSM\Util\Url::overlibLink($link, $mempool->mempool_descr, $overlib_content) . '</td>
            <td class="col-md-4">' . \UpdiveNSM\Util\Url::overlibLink($link, $minigraph, $overlib_content) . '</td>
            <td class="col-md-4">' . \UpdiveNSM\Util\Url::overlibLink($link, $percentageBar, $overlib_content) . '
            </a></td>
            </tr>';
    }//end foreach
