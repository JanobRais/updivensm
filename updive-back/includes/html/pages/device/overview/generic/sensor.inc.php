...
if ($sensors->isNotEmpty()) {
    ...
    foreach ($sensors as $sensor) {
        ...
        foreach (['day', 'week', 'month', 'year'] as $period) {
            $graph_array['from'] = \App\Facades\UpdiveNSMConfig::get("time.$period");
            $overlib_content .= str_replace('"', "\'", \UpdiveNSM\Util\Url::graphTag($graph_array));
        }
        ...
        $graph_array['from'] = \App\Facades\UpdiveNSMConfig::get('time.day');
        $sensor_minigraph = \UpdiveNSM\Util\Url::lazyGraphTag($graph_array);
        ...
        echo '<tr><td><div style="display: grid; grid-gap: 10px; grid-template-columns: 3fr 1fr 1fr;">
            <div>' . \UpdiveNSM\Util\Url::overlibLink($link, \UpdiveNSM\Util\Rewrite::shortenIfName($sensor->sensor_descr), $overlib_content, $sensor_class->value) . '</div>
            <div>' . \UpdiveNSM\Util\Url::overlibLink($link, $sensor_minigraph, $overlib_content, $sensor_class->value) . '</div>
            <div>' . \UpdiveNSM\Util\Url::overlibLink($link, $sensor_current, $overlib_content, $sensor_class->value) . '</div>
            </div></td></tr>';
    }//end foreach
