...
foreach (dbFetchRows('SELECT * FROM `ucd_diskio` WHERE device_id = ? ORDER BY diskio_descr', [$device['device_id']]) as $drive) {
    ...
    $graph_array_zoom['from'] = \App\Facades\UpdiveNSMConfig::get('time.twoday');
    $graph_array_zoom['to'] = \App\Facades\UpdiveNSMConfig::get('time.now');

    $overlib_link = \UpdiveNSM\Util\Url::overlibLink($fs_url, $drive['diskio_descr'], \UpdiveNSM\Util\Url::graphTag($graph_array_zoom));
