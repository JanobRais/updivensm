...
if (count($services) > '0') {
    ...
    foreach ($services as $service) {
        ...
        echo '<td class="col-sm-1 text-muted">' . ($service['service_changed'] ? \UpdiveNSM\Util\Time::formatInterval(time() - $service['service_changed']) : 'Waiting for first service check') . '</td>';
        ...
        if ($vars['view'] == 'details') {
            ...
            $check_ds = null;
            $check_script = \App\Facades\UpdiveNSMConfig::get('install_dir') . '/includes/services/check_' . strtolower((string) $service['service_type']) . '.inc.php';
