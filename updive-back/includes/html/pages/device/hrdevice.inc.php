...
foreach (dbFetchRows('SELECT * FROM `hrDevice` WHERE `device_id` = ? ORDER BY `hrDeviceIndex`', [$device['device_id']]) as $hrdevice) {
    ...
    if ($hrdevice['hrDeviceType'] == 'hrDeviceProcessor') {
        ...
        $proc_popup .= "</div><img src=\'graph.php?id=" . $proc_id . '&amp;type=processor_usage&amp;from=' . \App\Facades\UpdiveNSMConfig::get('time.month') . '&amp;to=' . \App\Facades\UpdiveNSMConfig::get('time.now') . "&amp;width=400&amp;height=125\'>";
        $proc_popup .= "', RIGHT" . \App\Facades\UpdiveNSMConfig::get('overlib_defaults') . ');" onmouseout="return nd();"';
        ...
        $mini_graph = \UpdiveNSM\Util\Url::overlibLink($proc_url, \UpdiveNSM\Util\Url::lazyGraphTag($graph_array), \UpdiveNSM\Util\Url::graphTag($graph_array_zoom));
        ...
        if ($interface && isset($interface['ifIndex'])) {
            ...
            $mini_graph = \UpdiveNSM\Util\Url::overlibLink(generate_port_url($interface), \UpdiveNSM\Util\Url::lazyGraphTag($graph_array), \UpdiveNSM\Util\Url::graphTag($graph_array_zoom));
