...
foreach (dbFetchRows("SELECT * FROM `bgpPeers` WHERE `device_id` = ? $extra_sql ORDER BY `bgpPeerRemoteAs`, `bgpPeerIdentifier`", [$device['device_id']]) as $peer) {
    ...
    $peeraddresslink = '<span class=list-large>' . \UpdiveNSM\Util\Url::overlibLink($link, $peerIdentifierIp?->compressed(), \UpdiveNSM\Util\Url::graphTag($graph_array_zoom)) . '</span>';
    ...
    echo '
        ...
        <td>' . $last_error . '</td>
        <td>' . \UpdiveNSM\Util\Time::formatInterval($peer['bgpPeerFsmEstablishedTime']) . "<br />
