...
if (Gate::allows('showConfig', DeviceCache::getPrimary())) {
    if (UpdiveNSMConfig::get('rancid_repo_type') == 'git-bare' && is_dir($rancid_path)) {
        echo '<div style="clear: both;">';

        print_optionbar_start('', '');
        echo is_null(UpdiveNSMConfig::get('rancid_repo_url')) ? 'Git repository non-browsable'
            : '<a href="' . htmlspecialchars(UpdiveNSMConfig::get('rancid_repo_url')) . '/?a=blob;hb=HEAD;p=' . basename((string) $rancid_path) . ';f=' . $rancid_file . '">Git repository</a>';
        print_optionbar_end();
        ...
        if (UpdiveNSMConfig::get('rancid_repo_type') == 'svn') {
            $svn_binary = UpdiveNSMConfig::locateBinary('svn');
            if (is_executable($svn_binary)) {
                ...
                foreach ($svnlogs as $svnlog) {
                    ...
                    $linktext = 'r' . $svnlog['rev'] . ' <small>' . date(UpdiveNSMConfig::get('dateformat.byminute'), $svnlog['date']) . '</small>';
        ...
        if (UpdiveNSMConfig::get('rancid_repo_type') == 'git') {
            $sep = ' | ';
            ...
            foreach ($gitlogs as $gitlog) {
                ...
                $linktext = 'r' . $gitlog['rev'] . ' <small>' . date(UpdiveNSMConfig::get('dateformat.byminute'), $gitlog['date']) . '</small>';
        ...
        } elseif (UpdiveNSMConfig::get('rancid_repo_type') == 'git') {
        ...
        if (UpdiveNSMConfig::get('rancid_ignorecomments')) {
            $lines = explode("\n", (string) $text);
    ...
    } elseif (UpdiveNSMConfig::get('oxidized.enabled') === true && UpdiveNSMConfig::has('oxidized.url')) {
        // Try with hostname as set in UpdiveNSM first
        ...
        if (! empty($node_info['last']['start'])) {
            $node_info['last']['start'] = date(UpdiveNSMConfig::get('dateformat.long'), strtotime((string) $node_info['last']['start']));
        }
        if (! empty($node_info['last']['end'])) {
            $node_info['last']['end'] = date(UpdiveNSMConfig::get('dateformat.long'), strtotime((string) $node_info['last']['end']));
        }
        ...
        if (! $node_info) {
            ...
            } elseif (UpdiveNSMConfig::get('mydomain')) {
                $oxidized_hostname = $device['hostname'] . '.' . UpdiveNSMConfig::get('mydomain');
            }
        ...
        if (UpdiveNSMConfig::get('oxidized.features.versioning') === true) { // fetch a list of versions
    ...
    if (! empty($text)) {
        $language = isset($previous_config) ? 'diff' : UpdiveNSMConfig::getOsSetting($device['os'], 'config_highlighting', 'ios');
