...
if (isset($_POST['editing'])) {
    if (Gate::allows('update', $device)) {
        ...
        if ($snmp_enabled) {
            ...
            $device->port = $_POST['port'] ?: UpdiveNSMConfig::get('snmp.port');
...
echo "
    ...
    <div class='col-sm-2'>
    <input type='number' name='port' placeholder='port' class='form-control input-sm' value='" . htmlspecialchars($device->port == UpdiveNSMConfig::get('snmp.port') ? '' : $device->port) . "'>
    </div>
    <div class='col-sm-1'>
    <select name='transport' id='transport' class='form-control input-sm'>";
foreach (UpdiveNSMConfig::get('snmp.transports') as $transport) {
...
if (! \UpdiveNSM\SNMPCapabilities::supportsSHA2()) {
    echo '<label class="text-left"><small>Some options are disabled. <a href="https://docs.UpdiveNSM.org/Support/FAQ/#optional-requirements-for-snmpv3-sha2-auth">Read more here</a></small></label>';
}
echo "
    </div>
    </div>
...
echo '</select>
    ';
if (! \UpdiveNSM\SNMPCapabilities::supportsAES256()) {
    echo '<label class="text-left"><small>Some options are disabled. <a href="https://docs.UpdiveNSM.org/Support/FAQ/#optional-requirements-for-snmpv3-sha2-auth">Read more here</a></small></label>';
}
    echo '
    </div>
...
var current_os = <?php echo json_encode(['id' => $device->os, 'text' => UpdiveNSMConfig::get("os.{$device->os}.text")], JSON_HEX_QUOT|JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS) ?>;
