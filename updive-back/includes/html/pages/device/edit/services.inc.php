...
if (Gate::allows('create', Service::class)) {
    if ($vars['addsrv']) {
        ...
        $service_id = \UpdiveNSM\Services::addService($vars['device'], $vars['type'], $vars['descr'], $vars['ip'], $vars['params'], $vars['ignore'] ?? 0, $vars['disabled'] ?? 0, 0, strip_tags((string) $vars['name']));
        if ($service_id) {
    ...
    foreach (scandir(\App\Facades\UpdiveNSMConfig::get('nagios_plugins')) as $file) {
