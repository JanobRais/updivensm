...
use UpdiveNSM\Exceptions\HostUnreachableException;
...
if (! empty($_POST['hostname'])) {
    ...
    if (! \UpdiveNSM\Util\Validate::hostname($hostname) && ! IP::isValid($hostname)) {
        ...
        if (Gate::allows('create', Device::class)) {
            ...
            if (! $snmp_enabled) {
                ...
                $communities = UpdiveNSMConfig::get('snmp.community');
                if ($_POST['community']) {
            ...
            try {
                ...
                if ($result) {
                    $link = \UpdiveNSM\Util\Url::deviceUrl($new_device->device_id);
        ...
        <div class="form-group">
          ...
          <div class="col-sm-3">
            <select name="snmpver" id="snmpver" class="form-control input-sm" onChange="changeForm();">
                <?php
                $snmpver_pref = UpdiveNSMConfig::get('snmp.version.0', 'v2c');
...
<?php
var_dump(UpdiveNSMConfig::get('snmp.transports', ['udp']));
foreach (UpdiveNSMConfig::get('snmp.transports', 'udp') as $transport) {
    echo '<option value="' . $transport . '"'. (UpdiveNSMConfig::get('snmp.transports.0') == $transport ? ' selected' : '') . '>' . $transport . '</option>';
}
?>
            </select>
          </div>
...
foreach (PortAssociationMode::getModes() as $mode) {
    $selected = $mode == UpdiveNSMConfig::get('default_port_association_mode') ? ' selected' : '';
          ...
          <div class="form-group">
            ...
            <div class="col-sm-3">
              <select name="authlevel" id="authlevel" class="form-control input-sm">
                  ...
                  $authlevel_pref = UpdiveNSMConfig::get('snmp.v3.0.authlevel', 'noAuthNoPriv');
          ...
          <div class="form-group">
            ...
            <div class="col-sm-9">
              <select name="authalgo" id="authalgo" class="form-control input-sm">
                  <?php
                  $algo_pref = UpdiveNSMConfig::get('snmp.v3.0.authalgo');
                  foreach (\UpdiveNSM\SNMPCapabilities::authAlgorithms() as $algo => $enabled) {
                      echo "<option value=\"$algo\"" . ($enabled ? '' : ' disabled') . ($algo == $algo_pref ? ' selected' : '') . ">$algo</option>";
                  }
                  ?>
              </select>
              <?php if (! \UpdiveNSM\SNMPCapabilities::supportsSHA2()) {?>
              <label class="text-left"><small>Some options are disabled. <a href="https://docs.UpdiveNSM.org/Support/FAQ/#optional-requirements-for-snmpv3-sha2-auth">Read more here</a></small></label>
              <?php } ?>
            </div>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-9">
              <select name="cryptoalgo" id="cryptoalgo" class="form-control input-sm">
                  <?php
                  $algo_pref = UpdiveNSMConfig::get('snmp.v3.0.cryptoalgo');
                  foreach (\UpdiveNSM\SNMPCapabilities::cryptoAlgoritms() as $algo => $enabled) {
                      echo "<option value=\"$algo\"" . ($enabled ? '' : ' disabled') . ($algo == $algo_pref ? ' selected' : '') . ">$algo</option>";
                  }
                  ?>
              </select>
              <?php if (! \UpdiveNSM\SNMPCapabilities::supportsAES256()) {?>
              <label class="text-left"><small>Some options are disabled. <a href="https://docs.UpdiveNSM.org/Support/FAQ/#optional-requirements-for-snmpv3-sha2-auth">Read more here</a></small></label>
              <?php } ?>
            </div>
          </div>
...
<?php
if (UpdiveNSMConfig::get('distributed_poller') === true) {
                      echo '
          <div class="form-group">
