...
$fromtext = dbFetchCell("SELECT DATE_FORMAT($datefrom, '" . \App\Facades\UpdiveNSMConfig::get('dateformat.mysql.date') . "')");
$totext = dbFetchCell("SELECT DATE_FORMAT($dateto, '" . \App\Facades\UpdiveNSMConfig::get('dateformat.mysql.date') . "')");
...
$total['bg'] = \UpdiveNSM\Util\Color::percentage($total['per'], null);
...
$in['bg'] = \UpdiveNSM\Util\Color::percentage($in['per'], null);
...
$out['bg'] = \UpdiveNSM\Util\Color::percentage($out['per'], null);

$ousage = [];
$ousage['over'] = ($bill_data['total_data'] - ($bill_data['bill_quota']));
$ousage['over'] = (($ousage['over'] < 0) ? '0' : $ousage['over']);
$ousage['data'] = \UpdiveNSM\Util\Number::formatBase($ousage['over'], \App\Facades\UpdiveNSMConfig::get('billing.base'), 2, 0, '');
...
$ousage['bg'] = \UpdiveNSM\Util\Color::percentage($ousage['per'], null);

function showPercent($per)
{
    $background = \UpdiveNSM\Util\Color::percentage($per, null);
    $right_background = $background['right'];
    $left_background = $background['left'];
    $res = \UpdiveNSM\Util\Html::percentageBar(200, 10, $per, null, $per . '%', null, null, [
...
$di .= '&amp;from=' . \App\Facades\UpdiveNSMConfig::get('time.day') . '&amp;to=' . \App\Facades\UpdiveNSMConfig::get('time.now');
$di .= '&amp;imgtype=hour';
