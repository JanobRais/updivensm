...
$vars['from'] = Time::parseAt($vars['from'] ?? '') ?: UpdiveNSMConfig::get('time.day');
$vars['to'] = Time::parseAt($vars['to'] ?? '') ?: UpdiveNSMConfig::get('time.now');
...
if (! $auth) {
    ...
    if (UpdiveNSMConfig::has("graph_types.$type.$subtype.descr")) {
        $title .= ' :: ' . UpdiveNSMConfig::get("graph_types.$type.$subtype.descr");
    } elseif ($type == 'device' && $subtype == 'collectd') {
        $title .= ' :: ' . \UpdiveNSM\Util\StringHelpers::niceCase($subtype) . ' :: ' . $vars['c_plugin'];
        ...
        $title .= ' :: ' . \UpdiveNSM\Util\StringHelpers::niceCase($subtype);
    }
    ...
    if (! in_array($type, ['sensor', 'wireless'])) {
        ...
        if (count($graph_subtypes) > 1) {
            ...
            foreach ($graph_subtypes as $avail_type) {
                echo "<option value='" . \UpdiveNSM\Util\Url::generate($vars, ['type' => $type . '_' . $avail_type, 'page' => 'graphs']) . "'";
    ...
    if (! $show_command) {
        $thumb_array = UpdiveNSMConfig::get('graphs.row.normal');
        ...
        foreach ($thumb_array as $period => $text) {
            ...
            echo '<b>' . $text . '</b>';
            echo '<a href="' . $link . '">';
            echo \UpdiveNSM\Util\Url::lazyGraphTag($graph_array);
            echo '</a>';
            echo '</td>';
        }
    ...
    $graph_array = $vars;
    $graph_array['height'] = UpdiveNSMConfig::get('webui.min_graph_height');
    ...
    if ($vars['type'] == 'port_bits') {
        echo ' | ';
        if ($vars['port_speed_zoom'] ?? UpdiveNSMConfig::get('graphs.port_speed_zoom')) {
    ...
    if (UpdiveNSMConfig::get('webui.dynamic_graphs', false) === true) {
    ...
    if (UpdiveNSMConfig::has('graph_descr.' . $vars['type'])) {
        print_optionbar_start();
        ...
        echo UpdiveNSMConfig::get('graph_descr.' . $vars['type']);
        print_optionbar_end();
    }
