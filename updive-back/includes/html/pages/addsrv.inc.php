...
if (! empty($vars['addsrv'])) {
    ...
    $service_id = \UpdiveNSM\Services::addService($vars['device'], $vars['type'], strip_tags((string) $vars['descr']), $vars['ip'], $vars['params'], $vars['ignore'] ?? 0, $vars['disabled'] ?? 0, 0, strip_tags((string) $vars['name']));
    if ($service_id) {
