 ...
 *
 * @author     UpdiveNSM Contributors <UpdiveNSM-project@google.groups.com>
 * @copyright  (C) 2006 - 2012 Adam Armstrong (as Observium)
 * @copyright  (C) 2013 UpdiveNSM Group
 */
...
use UpdiveNSM\Enum\ImageFormat;
use UpdiveNSM\Util\Number;
use UpdiveNSM\Util\Rewrite;
use UpdiveNSM\Util\Url;

function toner2colour($descr, $percent)
{
    $colour = \UpdiveNSM\Util\Color::percentage(100 - $percent, null);
...
function generate_overlib_content($graph_array, $text)
{
    ...
    foreach (['day', 'week', 'month', 'year'] as $period) {
        $graph_array['from'] = UpdiveNSMConfig::get("time.$period");
...
function generate_port_link($port, $text = null, $type = null, $overlib = 1, $single_graph = 0)
{
    ...
    $content = '<div class="overlib-text">' . ($port['hostname'] ?? '') . ' - ' . Rewrite::normalizeIfName(addslashes(\UpdiveNSM\Util\Clean::html($port['label'], []))) . '</div>';
    $content .= addslashes(\UpdiveNSM\Util\Clean::html($port['ifAlias'], [])) . '<br />';
    ...
    $graph_array['to'] = UpdiveNSMConfig::get('time.now');
    $graph_array['from'] = UpdiveNSMConfig::get('time.day');
    $graph_array['id'] = $port['port_id'];
    $content .= Url::graphTag($graph_array);
    if ($single_graph == 0) {
        $graph_array['from'] = UpdiveNSMConfig::get('time.week');
        $content .= Url::graphTag($graph_array);
        $graph_array['from'] = UpdiveNSMConfig::get('time.month');
        $content .= Url::graphTag($graph_array);
        $graph_array['from'] = UpdiveNSMConfig::get('time.year');
        $content .= Url::graphTag($graph_array);
    }
...
function generate_sensor_link($args, $text = null, $type = null)
{
    ...
    $graph_array = [
        ...
        'height' => '100',
        'width' => '340',
        'to' => UpdiveNSMConfig::get('time.now'),
        'from' => UpdiveNSMConfig::get('time.day'),
        'id' => $args['sensor_id'],
    ];
    $content .= Url::graphTag($graph_array);
    ...
    $url = Url::generate(['page' => 'graphs', 'id' => $args['sensor_id'], 'type' => $args['graph_type'], 'from' => \App\Facades\UpdiveNSMConfig::get('time.day')], []);
...
function graph_error($text, $short = null, $color = [128, 0, 0])
{
    ...
    echo \UpdiveNSM\Util\Graph::error($text, $short, 300, null, $color);
}
...
function generate_ap_link($args, $text = null, $type = null)
{
    ...
    if ($args['ifAlias']) {
        $content .= \UpdiveNSM\Util\Clean::html($args['ifAlias'], []) . '<br />';
    }
...
function get_ports_from_type($given_types)
{
    ...
    foreach ($given_types as $type) {
        if (UpdiveNSMConfig::has($type . '_descr')) {
            $type_descr = UpdiveNSMConfig::get($type . '_descr');
...
function search_oxidized_config($search_in_conf_textbox)
{
    ...
    $oxidized_search_url = UpdiveNSMConfig::get('oxidized.url') . '/nodes/conf_search?format=json';
    ...
    // Look up Oxidized node names to UpdiveNSM devices for a link
    foreach ($nodes as &$n) {
...
function get_oxidized_nodes_list()
{
    ...
    $data = json_decode(file_get_contents(UpdiveNSMConfig::get('oxidized.url') . '/nodes?format=json', false, $context), true);
...
{
    if (UpdiveNSMConfig::get('webui.graph_stacked') == true || $force_stack == true) {
...
function time_to_nfsen_subpath($time)
{
    ...
    $layout = UpdiveNSMConfig::get('nfsen_subdirlayout');
...
{
    $nfsen_hostname = str_replace('.', UpdiveNSMConfig::get('nfsen_split_char'), $hostname);

    if (! is_null(UpdiveNSMConfig::get('nfsen_suffix'))) {
        $nfsen_hostname = str_replace(UpdiveNSMConfig::get('nfsen_suffix'), '', $nfsen_hostname);
    }
...
function nfsen_live_dir($hostname)
{
    ...
    foreach (UpdiveNSMConfig::get('nfsen_base') as $base_dir) {
