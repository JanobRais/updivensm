...
$param = [];
$alert_states = [
    // divined from UpdiveNSM/alerts.php
    'recovered' => 0,
    'alerted' => 1,
    'acknowledged' => 2,
    'worse' => 3,
...
foreach (dbFetchRows($sql, $param) as $alert) {
    ...
    $hostname = '<div class="incident">' . \UpdiveNSM\Util\Url::modernDeviceLink(DeviceCache::get($alert['device_id'] ?? null)) . '<div id="incident' . $alert['id'] . '"';
    ...
    $response[] = [
        'id' => $rulei++,
        'rule' => '<i title="' . htmlentities((string) $alert['builder']) . '"><a href="' . \UpdiveNSM\Util\Url::generate(['page' => 'alert-rules']) . '">' . htmlentities((string) $alert['name']) . '</a></i>',
