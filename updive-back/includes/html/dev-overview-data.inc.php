...
if (UpdiveNSMConfig::get('overview_show_sysDescr')) {
    echo '<i class="fa fa-id-card fa-lg icon-theme" aria-hidden="true"></i> <strong>';
    echo UpdiveNSMConfig::get('overview_show_sysDescr', true) ? Clean::html($device['sysDescr'], []) : 'System';
    echo '</strong>';
}
...
if ($device['os'] == 'ios' || $device['os'] == 'iosxe') {
    \UpdiveNSM\Util\Rewrite::ciscoHardware($device, false);
}
...
$device['os_text'] = UpdiveNSMConfig::getOsSetting($device['os'], 'text');
...
if ($device['sysContact']) {
    ...
    $emails = \UpdiveNSM\Util\Mail::parseEmails($contactText);
...
if ($device['location_id'] && $location = Location::find($device['location_id'])) {
    $maps_api = UpdiveNSMConfig::get('geoloc.api_key');
    $maps_engine = $maps_api ? UpdiveNSMConfig::get('geoloc.engine') : '';
    ...
    <script>
        ...
        $("#toggle-map").on("shown.bs.collapse", function () {
             ...
             if (device_map == null) {
                ...
                var config = {
                    "tile_url": "' . UpdiveNSMConfig::get('leaflet.tile_url', '{s}.tile.openstreetmap.org') . '",
                    "engine": "' . $maps_engine . '",
                    "api_key": "' . $maps_api . '"
                };
    ...
    if (UpdiveNSMConfig::get('device_location_map_show_devices')) {
        // Get a list of devices we have access to and add them to the map
        echo'
                device_marker_cluster = L.markerClusterGroup({
                    maxClusterRadius: ' . UpdiveNSMConfig::get('leaflet.group_radius', 80) . ',
        ...
        if (UpdiveNSMConfig::get('device_location_map_show_device_dependencies')) {
            echo'
    ...
    if (UpdiveNSMConfig::get('device_location_map_open')) {
