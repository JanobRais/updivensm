...
use UpdiveNSM\Alert\AlertData;
use UpdiveNSM\Alerting\QueryBuilderParser;
use UpdiveNSM\Billing;
use UpdiveNSM\Enum\MaintenanceBehavior;
use UpdiveNSM\Enum\Severity;
use UpdiveNSM\Exceptions\InvalidIpException;
use UpdiveNSM\Exceptions\InvalidTableColumnException;
use UpdiveNSM\Util\Graph;
use UpdiveNSM\Util\IP;
use UpdiveNSM\Util\IPv4;
...
function api_get_graph(Request $request, array $additional = [])
{
    ...
    } catch (\UpdiveNSM\Exceptions\RrdGraphException $e) {
...
function add_device(Illuminate\Http\Request $request)
{
    ...
    if (! \UpdiveNSM\Util\Validate::hostname($data['hostname']) && ! IP::isValid($data['hostname'])) {
...
function maintenance_device(Illuminate\Http\Request $request)
{
    ...
    $behavior = MaintenanceBehavior::tryFrom((int) ($data['behavior'] ?? -1))
        ?? UpdiveNSMConfig::get('alert.scheduled_maintenance_default_behavior');
...
function get_components(Illuminate\Http\Request $request)
{
    ...
    return check_device_permission($device_id, function ($device_id) use ($options) {
        $COMPONENT = new UpdiveNSM\Component();
...
function get_graphs(Illuminate\Http\Request $request)
{
    ...
    return check_device_permission($device_id, function ($device_id) {
        ...
        foreach (dbFetchRows('SELECT * FROM device_graphs WHERE device_id = ? ORDER BY graph', [$device_id]) as $graph) {
            $desc = UpdiveNSMConfig::get("graph_types.device.{$graph['graph']}.descr");
            $graphs[] = [
                'desc' => $desc,
...
/**
 * @throws \UpdiveNSM\Exceptions\ApiException
 */
...
function ack_alert(Illuminate\Http\Request $request)
{
    ...
    $note .= date(UpdiveNSMConfig::get('dateformat.long')) . ' - Ack (' . Auth::user()->username . ") {$data['note']}";
...
function list_oxidized(Illuminate\Http\Request $request)
{
    ...
    $devices = Device::query()
            ->with('attribs')
             ...
             ->whereNotIn('type', UpdiveNSMConfig::get('oxidized.ignore_types', []))
             ->whereNotIn('os', UpdiveNSMConfig::get('oxidized.ignore_os', []))
    ...
    foreach ($devices as $device) {
        ...
        if (UpdiveNSMConfig::get('oxidized.group_support') === true && ! empty(UpdiveNSMConfig::get('oxidized.default_group'))) {
            $output['group'] = UpdiveNSMConfig::get('oxidized.default_group');
        }

        foreach (UpdiveNSMConfig::get('oxidized.maps') as $maps_column => $maps) {
            // Based on Oxidized group support we can apply groups by setting group_support to true
            if ($maps_column == 'group' && UpdiveNSMConfig::get('oxidized.group_support', true) !== true) {
                continue;
            }
        ...
        if (isset($output['group']) && in_array($output['group'], UpdiveNSMConfig::get('oxidized.ignore_groups'))) {
            continue;
        }
...
function validate_column_list(?string $columns, string $table, array $default = []): array
{
    ...
    static $schema;
    if (is_null($schema)) {
        $schema = new \UpdiveNSM\DB\Schema();
    }
...
function add_service_for_host(Illuminate\Http\Request $request)
{
    ...
    $service_id = \UpdiveNSM\Services::addService($device_id, $service_type, $service_desc, $service_ip, $service_param, (int) $service_ignore, (int) $service_disable, 0, $service_name);
    if ($service_id != false) {
...
/**
 * recieve syslog messages via json https://github.com/UpdiveNSM/UpdiveNSM/pull/14424
 */
...
/**
 * Display UpdiveNSM Instance Info
 */
function server_info()
{
    $version = \UpdiveNSM\Util\Version::get();
...
function list_poller_log(Illuminate\Http\Request $request)
{
    ...
    if ($request->input('unpolled')) {
        $overdue = (int) (UpdiveNSMConfig::get('rrd.step', 300) * 1.2);
