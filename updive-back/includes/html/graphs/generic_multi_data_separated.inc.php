...
foreach ($rrd_list as $rrd) {
    if (! \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours_in.$iter") || ! \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours_out.$iter")) {
        $iter = 0;
    }

    $colour_in = \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours_in.$iter");
    $colour_out = \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours_out.$iter");

    if (isset($rrd['descr_in'])) {
        $descr = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr($rrd['descr_in'], $descr_len) . '  In';
    } else {
        $descr = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr($rrd['descr'], $descr_len) . '  In';
    }

    $descr_out = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr($rrd['descr_out'], $descr_len) . ' Out';
...
if (! $nototal) {
    ...
    $rrd_options[] = 'VDEF:percentile_in=inbits,' . \App\Facades\UpdiveNSMConfig::get('percentile_value') . ',PERCENT';
    $rrd_options[] = 'VDEF:percentile_out=outbits,' . \App\Facades\UpdiveNSMConfig::get('percentile_value') . ',PERCENT';
    $rrd_options[] = 'CDEF:dpercentile_outn=doutbits,' . $stacked['stacked'] . ',* VDEF:dpercentile_outnp=dpercentile_outn,' . \App\Facades\UpdiveNSMConfig::get('percentile_value') . ',PERCENT CDEF:dpercentile_outnpn=doutbits,doutbits,-,dpercentile_outnp,' . $stacked['stacked'] . ',*,+ VDEF:dpercentile_out=dpercentile_outnpn,FIRST';
