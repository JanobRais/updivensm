...
$units_descr = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr($units_descr ?? '', $rrddescr_len + 5);
...
foreach ($rrd_list ?? [] as $rrd) {
    if (! UpdiveNSMConfig::get("graph_colours.$colours_in.$iter") || ! UpdiveNSMConfig::get("graph_colours.$colours_out.$iter")) {
        $iter = 0;
    }

    $colour_in = $rrd['colour_area_in'] ?? UpdiveNSMConfig::get("graph_colours.$colours_in.$iter");
    $colour_out = $rrd['colour_area_out'] ?? UpdiveNSMConfig::get("graph_colours.$colours_out.$iter");
    ...
    if (! $nodetails) {
        $descr = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr($rrd['descr'], $rrddescr_len) . '  In';
        $descr_out = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('', $rrddescr_len) . ' Out';
    }
...
if ($previous) {
    ...
    $rrd_options[] = 'VDEF:dpercentile_outXperc=dpercentile_outXn,' . UpdiveNSMConfig::get('percentile_value') . ',PERCENT';
...
if (! $nototal && ! empty($rrd_list)) {
    ...
    $rrd_options[] = 'HRULE:999999999999999#FFFFFF:' . \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('Total', $rrddescr_len) . '  In';
    ...
    $rrd_options[] = 'HRULE:999999999999990#FFFFFF:' . \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('', $rrddescr_len) . ' Out';
    ...
    $rrd_options[] = 'HRULE:999999999999990#FFFFFF:' . \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('', $rrddescr_len) . ' Agg';
