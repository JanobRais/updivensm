...
if ($height > 99) {
    $rrd_options[] = 'COMMENT:' . \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr($unit_text, $descr_len) . "      Now      Min      Max     Avg\l";
}
...
if (! isset($colour)) {
    $colour = \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours.$iter");
    $iter++;
}
...
if (! isset($colour25th)) {
    if (! \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours.$iter")) {
        $iter = 0;
    }
    $colour25th = \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours.$iter");
    $iter++;
}
...
if (! isset($colour50th)) {
    ...
    $colour50th = \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours.$iter");
    $iter++;
}
...
if (! isset($colour75th)) {
    ...
    $colour75th = \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours.$iter");
    $iter++;
}
...
if (! isset($colour1h)) {
    ...
    $colour1h = \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours.$iter");
    $iter++;
}
...
if (! isset($colour1d)) {
    ...
    $colour1d = \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours.$iter");
    $iter++;
}
...
if (! isset($colour1w)) {
    ...
    $colour1w = \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours.$iter");
    $iter++;
}

$graph_stat_percentile_disable = \App\Facades\UpdiveNSMConfig::get('graph_stat_percentile_disable');

$descr = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr($descr ?? '', $descr_len);

if ($height > 25) {
    if (! $no_hourly) {
        $descr_1h = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('1 hour avg', $descr_len);
        $descr_1h_min = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('1 hour min', $descr_len);
        $descr_1h_max = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('1 hour max', $descr_len);
    }
    if (! $no_daily) {
        $descr_1d = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('1 day avg', $descr_len);
        $descr_1d_min = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('1 day min', $descr_len);
        $descr_1d_max = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('1 day max', $descr_len);
    }
    if (! $no_weekly) {
        $descr_1w = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('1 week avg', $descr_len);
        $descr_1w_min = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('1 week min', $descr_len);
        $descr_1w_max = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('1 week max', $descr_len);
    }
}
