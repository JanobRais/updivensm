...
function error($code, $code_msg, $title, $msg)
{
    ...
    $w = (UpdiveNSMConfig::get('rrd_width') + 81);
    $h = (UpdiveNSMConfig::get('rrd_height') + 79);
    ...
    if (function_exists('imagettfbbox') && is_file(UpdiveNSMConfig::get('error_font'))) {
        // Detailled error message
        $errorfont = UpdiveNSMConfig::get('error_font');
...
$timespan = read_var('timespan', $_GET, UpdiveNSMConfig::get('timespan.0.name'));
$timespan_ok = false;
foreach (UpdiveNSMConfig::get('timespan') as &$ts) {
    if ($ts['name'] == $timespan) {
        $timespan_ok = true;
    }
}
...
if ($width <= '300') {
    $rrd_cmd .= ' --font LEGEND:7:' . UpdiveNSMConfig::get('mono_font') . ' --font AXIS:6:' . UpdiveNSMConfig::get('mono_font') . ' ';
} else {
    $rrd_cmd .= ' --font LEGEND:8:' . UpdiveNSMConfig::get('mono_font') . ' --font AXIS:7:' . UpdiveNSMConfig::get('mono_font') . ' ';
}
