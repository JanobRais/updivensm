...
$i = 0;
$pings = UpdiveNSMConfig::get('smokeping.pings');
$iter = 0;
$colourset = 'mixed';
...
foreach ($smokeping_files[$direction][$device['hostname']] as $source => $filename) {
    if (! UpdiveNSMConfig::has("graph_colours.$colourset.$iter")) {
        $iter = 0;
    }

    $colour = UpdiveNSMConfig::get("graph_colours.$colourset.$iter");
    $iter++;

    // FIXME: $descr unused? -- PDG 2015-11-14
    $descr = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr($source, $descr_len);
...
$descr = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr('Average', $descr_len);
