...
/*
 * UpdiveNSM per-module poller performance
 *
 * Copyright (c) 2016 Mike Rostermund <mike@kollegienet.dk>
 * Copyright (c) 2016 Paul D. Gear <paul@UpdiveNSM.org>
 *
...
$modules = UpdiveNSMConfig::get('poller_modules');
ksort($modules);
...
foreach ($modules as $module => $module_status) {
    ...
    if ((isset($attribs['poll_' . $module]) && $attribs['poll_' . $module]) || ($module_status && ! isset($attribs['poll_' . $module])) ||
        (UpdiveNSMConfig::getOsSetting($device->os, 'poller_modules.' . $module) && ! isset($attribs['poll_' . $module]))) {
