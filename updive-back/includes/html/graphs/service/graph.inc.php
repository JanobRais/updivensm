...
/*
 * UpdiveNSM module to display graphing for Nagios Service
 *
...
$check_script = \App\Facades\UpdiveNSMConfig::get('install_dir') . '/includes/services/check_' . strtolower((string) $services[$vars['service']]['service_type']) . '.inc.php';
...
if ($services[$vars['service']]['service_ds'] != '') {
    ...
    if (Rrd::checkRrdExists($rrd_filename)) {
        if (isset($check_graph)) {
            ...
            $color_avg = \App\Facades\UpdiveNSMConfig::get("graph_colours.$tint.2");
            $color_max = \App\Facades\UpdiveNSMConfig::get("graph_colours.$tint.0");
