...
UpdiveNSM Application for monitoring power consumption
...
foreach ($ds_list as $ds_item) {
    ...
    $descr = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr($ds_item['descr'], $pad_to);
    ...
    if (isset($ds_item['colour'])) {
        ...
        if (! \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours.$i")) {
            $i = 0;
        }
        $colour = \App\Facades\UpdiveNSMConfig::get("graph_colours.$colours.$i");
        $i++;
    }
    ...
    if ($vname == 'watts') {
        ...
        */
        ...
        $float_precision = 2;
        $descr = '  Total Consumed';
        $descr = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr($descr, $pad_to + 2);
        $rrd_options[] = "COMMENT:{$descr}";
        ...
        // Consumption Charge
        $float_precision = 2;
        $descr = \UpdiveNSM\Data\Store\Rrd::fixedSafeDescr($descr, $pad_to + 7);
