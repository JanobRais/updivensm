...
use UpdiveNSM\Data\Graphing\GraphParameters;
...
try {
    ...
    require UpdiveNSMConfig::get('install_dir') . "/includes/html/graphs/$type/auth.inc.php";

    if ($auth && is_customoid_graph($type, $subtype)) {
        $unit = $vars['unit'];
        include UpdiveNSMConfig::get('install_dir') . '/includes/html/graphs/customoid/customoid.inc.php';
    } elseif ($auth && is_file(UpdiveNSMConfig::get('install_dir') . "/includes/html/graphs/$type/$subtype.inc.php")) {
        include UpdiveNSMConfig::get('install_dir') . "/includes/html/graphs/$type/$subtype.inc.php";
    } elseif ($auth && is_file(UpdiveNSMConfig::get('install_dir') . "/includes/html/graphs/$type/generic.inc.php")) {
        include UpdiveNSMConfig::get('install_dir') . "/includes/html/graphs/$type/generic.inc.php";
    } else {
    ...
    if (isset($vars['device']) && $device->exists === false) {
        throw new \UpdiveNSM\Exceptions\RrdGraphException('Device not found');
    }

    // command output requested
    if (! empty($command_only)) {
        $command = implode(' ', array_map(escapeshellarg(...), ['rrdtool', ...Rrd::buildCommand('graph', UpdiveNSMConfig::get('temp_dir') . '/' . Str::random(), $rrd_options)]));
    ...
    // output the graph
    if (\UpdiveNSM\Util\Debug::isEnabled()) {
