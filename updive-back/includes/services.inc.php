...
function add_service($device, $type, $desc, $ip = '', $param = '', $ignore = 0, $disabled = 0, $template_id = '', $name = '')
{
    ...
    return \UpdiveNSM\Services::addService($device, $type, $desc, $ip, $param, $ignore, $disabled, $template_id, $name);
}
...
function poll_service($service)
{
    ...
    $check_script = UpdiveNSMConfig::get('install_dir') . '/includes/services/check_' . strtolower((string) $service['service_type']) . '.inc.php';
    ...
    if ($check_cmd == '') {
        $check_cmd = UpdiveNSMConfig::get('nagios_plugins') . '/check_' . $service['service_type'] . ' -H ' . ($service['service_ip'] ?: $service['hostname']);
...
{
    return \UpdiveNSM\Services::list();
}
