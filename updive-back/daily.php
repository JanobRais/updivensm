...
use UpdiveNSM\Alerting\QueryBuilderParser;
use UpdiveNSM\Util\Debug;
use UpdiveNSM\Util\Notifications;
use UpdiveNSM\Validations\Php;
...
if ($options['f'] === 'update') {
    if (! UpdiveNSMConfig::get('update')) {
        exit(0);
    }

    if (UpdiveNSMConfig::get('update_channel') == 'master') {
        exit(1);
    } elseif (UpdiveNSMConfig::get('update_channel') == 'release') {
        exit(3);
    }
    exit(0);
}
...
if ($options['f'] === 'rrd_purge') {
    ...
    if ($lock->get()) {
        $rrd_purge = UpdiveNSMConfig::get('rrd_purge');
        $rrd_dir = UpdiveNSMConfig::get('rrd_dir');
...
if ($options['f'] === 'callback') {
    \UpdiveNSM\Util\Stats::submit();
}

if ($options['f'] === 'ports_purge') {
    if (UpdiveNSMConfig::get('ports_purge')) {
...
if ($options['f'] === 'handle_notifiable') {
    if ($options['t'] === 'update') {
        ...
        $poller_name = UpdiveNSMConfig::get('distributed_poller_name');
        ...
        if ($options['r']) {
            ...
            Notifications::create($title, "The daily update script (daily.sh) has failed on $poller_name."
                ...
                . 'visit the <a href="https://www.UpdiveNSM.org/#support">UpdiveNSM Website</a> to find out how.',
                'daily.sh',
                2
            );
        }
        ...
        if (UpdiveNSMConfig::get('update') && $options['r']) {
            if (preg_match('/^php\d{2}/', $options['r'])) {
                ...
                Notifications::create($error_title,
                    "PHP version $phpver is the minimum supported version as of $eol_date.  We recommend you update to PHP a supported version of PHP (" . Php::PHP_RECOMMENDED_VERSION . ' suggested) to continue to receive updates.  If you do not update PHP, UpdiveNSM will continue to function but stop receiving bug fixes and updates.',
                    'daily.sh',
                    2
                );
                exit(1);
        ...
        if (UpdiveNSMConfig::get('update') && $options['r']) {
            if ($options['r'] === 'python3-missing') {
                Notifications::create($error_title,
                    'Python 3 is required to run UpdiveNSM as of May, 2020. You need to install Python 3 to continue to receive updates.  If you do not install Python 3 and required packages, UpdiveNSM will continue to function but stop receiving bug fixes and updates.',
                    'daily.sh',
                    2
                );
                exit(1);
                ...
                Notifications::create($error_title,
                    'Python 3 dependencies are missing. You need to install them via pip3 install -r requirements.txt or system packages to continue to receive updates.  If you do not install Python 3 and required packages, UpdiveNSM will continue to function but stop receiving bug fixes and updates.',
                    'daily.sh',
                    2
                );
                exit(1);
...
if ($options['f'] === 'purgeusers') {
    ...
    if ($lock->get()) {
        $purge = 0;
        if (is_numeric(\App\Facades\UpdiveNSMConfig::get('radius.users_purge')) && UpdiveNSMConfig::get('auth_mechanism') === 'radius') {
            $purge = \App\Facades\UpdiveNSMConfig::get('radius.users_purge');
        }
        if (is_numeric(\App\Facades\UpdiveNSMConfig::get('active_directory.users_purge')) && UpdiveNSMConfig::get('auth_mechanism') === 'active_directory') {
            $purge = \App\Facades\UpdiveNSMConfig::get('active_directory.users_purge');
        }
        if ($purge > 0) {
...
if ($options['f'] === 'notify') {
    if (\App\Facades\UpdiveNSMConfig::has('alert.default_mail')) {
        try {
            \UpdiveNSM\Util\Mail::send(\App\Facades\UpdiveNSMConfig::get('alert.default_mail'), '[UpdiveNSM] Auto update has failed for ' . UpdiveNSMConfig::get('distributed_poller_name'), "We just attempted to update your install but failed. The information below should help you fix this.\r\n\r\n" . $options['o'], false);
        } catch (Exception $e) {
