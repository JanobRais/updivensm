...
namespace UpdiveNSM\OS\Traits;

use App\Facades\UpdiveNSMConfig;
use Illuminate\Support\Collection;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Str;
use UpdiveNSM\Enum\PowerState;
...
trait VminfoLibvirt
{
    public function discoverVminfo(): Collection
    {
        ...
        if (! UpdiveNSMConfig::get('enable_libvirt')) {
        ...
        if (UpdiveNSMConfig::has('libvirt_username')) {
            $userHostname = UpdiveNSMConfig::get('libvirt_username') . '@' . $userHostname;
        }

        foreach (UpdiveNSMConfig::get('libvirt_protocols') as $method) {
            ...
            if ($ssh_ok || ! Str::contains($method, 'ssh')) {
                ...
                unset($domlist);
                exec(escapeshellarg(UpdiveNSMConfig::get('virsh')) . ' -rc ' . escapeshellarg($uri) . ' list', $domlist);
                ...
                foreach ($domlist as $dom) {
                    ...
                    if (is_numeric($dom_id)) {
                        ...
                        unset($vm_info_array);
                        exec(escapeshellarg(UpdiveNSMConfig::get('virsh')) . ' -rc ' . escapeshellarg($uri) . ' dumpxml ' . $dom_id, $vm_info_array);
                        ...
                        exec(escapeshellarg(UpdiveNSMConfig::get('virsh')) . ' -rc ' . escapeshellarg($uri) . ' domstate ' . escapeshellarg($dom_id), $vm_state);
