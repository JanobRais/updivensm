...
class Core implements Module
{
    ...
    public function discover(OS $os): void
    {
        ...
        if (! $device->getAttrib('override_device_type')) {
            $device->type = UpdiveNSMConfig::getOsSetting($device->os, 'type');
    ...
    public static function detectOS(Device $device, bool $fetch = true): string
    {
        ...
        // check yaml files
        $os_defs = UpdiveNSMConfig::get('os');
    ...
    private function calculateUptime(OS $os, ?string $sysUpTime, DataStorageInterface $datastore): void
    {
        ...
        if (UpdiveNSMConfig::get("os.$device->os.bad_uptime")) {
            return;
        }
        ...
        if (! empty($agent_data['uptime'])) {
            ...
            $uptime = max(
                ...
                UpdiveNSMConfig::get("os.$device->os.bad_snmpEngineTime") ? 0 : Number::cast($uptime_data['SNMP-FRAMEWORK-MIB::snmpEngineTime.0'] ?? 0),
                UpdiveNSMConfig::get("os.$device->os.bad_hrSystemUptime") ? 0 : round(Number::cast($uptime_data['HOST-RESOURCES-MIB::hrSystemUptime.0'] ?? 0) / 100)
            );
        ...
        if ($uptime > 0) {
            if ($uptime < $device->uptime) {
                ...
                if (UpdiveNSMConfig::get('discovery_on_reboot')) {
