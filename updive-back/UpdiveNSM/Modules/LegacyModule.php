...
class LegacyModule implements Module
{
    ...
    {
        if (! \UpdiveNSM\Util\Module::legacyDiscoveryExists($this->name)) {
    ...
    {
        if (! \UpdiveNSM\Util\Module::legacyPollingExists($this->name)) {
            Log::error("Module $this->name does not exist, please remove it from your configuration");
    ...
    {
        if ($type == 'discovery' && ! \UpdiveNSM\Util\Module::legacyDiscoveryExists($this->name)) {
            return null;
        }
        if ($type == 'poller' && ! \UpdiveNSM\Util\Module::legacyPollingExists($this->name)) {
            return null;
        }
