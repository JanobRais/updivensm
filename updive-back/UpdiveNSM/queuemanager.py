...
class QueueManager:
    def __init__(
        ...
        :param config: UpdiveNSM.ServiceConfig reference to the service config object
        :param lock_manager: A UpdiveNSM.Lock instance to help with locks
        ...
        self.config = config
        self.performance = UpdiveNSM.PerformanceCounter()
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if (
                    ...
                    with UpdiveNSM.TimeitContext.start() as t:
    ...
    def get_poller_config(self):
        """
        Returns the UpdiveNSM.PollerConfig for this QueueManager
        :return: UpdiveNSM.PollerConfig
        """
    ...
    def _create_queue(self, queue_type, group):
        ...
        try:
            return UpdiveNSM.RedisUniqueQueue(
...
class TimedQueueManager(QueueManager):
    def __init__(
        ...
        self.timer = UpdiveNSM.RecurringTimer(
...
class BillingQueueManager(TimedQueueManager):
    def __init__(self, config, lock_manager):
        ...
        self.calculate_timer = UpdiveNSM.RecurringTimer(
    ...
    def do_work(self, run_type, group):
        if run_type == "poll":
            ...
            exit_code, output = UpdiveNSM.call_script("poll-billing.php", args)
        else:  # run_type == 'calculate'
            logger.info("Calculating billing")
            args = ("-d", "-f") if self.config.debug else ("-f",)
            exit_code, output = UpdiveNSM.call_script("billing-calculate.php", args)
    ...
    def do_work(self, context, group):
        if self.lock(group, "group", timeout=self.config.ping.frequency):
            try:
                ...
                exit_code, output = UpdiveNSM.call_script("lnms", args)
    ...
    def do_work(self, device_id, group):
        if self.lock(device_id, timeout=self.config.services.frequency):
            ...
            output = (
                ...
                if self.config.log_output == UpdiveNSM.LogOutput.FILE
                else self.config.log_output
            )

            args = ("-d", "-h", device_id) if self.config.debug else ("-h", device_id)
            exit_code, output = UpdiveNSM.call_script("check-services.php", args, output)
...
class AlertQueueManager(TimedQueueManager):
    ...
    def do_work(self, device_id, group):
        ...
        args = ("-d", "-f") if self.config.debug else ("-f",)
        exit_code, output = UpdiveNSM.call_script("alerts.php", args, output)
...
class PollerQueueManager(QueueManager):
    ...
    def do_work(self, device_id, group):
        if self.lock(device_id, timeout=self.config.poller.frequency):
            ...
            if self.config.debug:
                args_list.append("-vv")
            elif self.config.log_output is UpdiveNSM.LogOutput.NONE:
                args_list.append("-q")
            args = tuple(args_list)

            exit_code, output = UpdiveNSM.call_script("lnms", args, output)
...
class DiscoveryQueueManager(TimedQueueManager):
    ...
    def do_work(self, device_id, group):
        if self.lock(
            device_id, timeout=UpdiveNSM.normalize_wait(self.config.discovery.frequency)
        ):
