...
use UpdiveNSM\Exceptions\FileExistsException;
use UpdiveNSM\Exceptions\RrdException;
use UpdiveNSM\Exceptions\RrdGraphException;
use UpdiveNSM\Proc;
use UpdiveNSM\RRD\RrdProcess;
...
class Rrd extends BaseDatastore
{
    ...
    {
        return UpdiveNSMConfig::get('rrd.enable', true);
    }

    protected function loadConfig(): void
    {
        $this->rrdcached = UpdiveNSMConfig::get('rrdcached', false);
        $this->rrd_dir = UpdiveNSMConfig::get('rrd_dir', UpdiveNSMConfig::get('install_dir') . '/rrd');
        $this->step = UpdiveNSMConfig::get('rrd.step', 300);
        $this->rra = preg_split('/s+/', trim(UpdiveNSMConfig::get(
            'rrd_rra',
        ...
        $this->version = UpdiveNSMConfig::get('rrdtool_version', '1.4');
        $this->rrdtool_executable = UpdiveNSMConfig::get('rrdtool', 'rrdtool');
    }
    ...
    /**
     * rename an rrdfile, can only be done on the UpdiveNSM server hosting the rrd files
     *
    ...
    private function command(string $command, string $filename, array $options = []): array
    {
        ...
        if ($this->disabled && ! in_array($command, $ro_commands)) {
            if (! UpdiveNSMConfig::get('hide_rrd_disabled')) {
