...
namespace UpdiveNSM\Data\Store;
...
class Kafka extends BaseDatastore
{
    ...
    public function __construct(private Producer $client)
    {
        ...
        $this->topicName = UpdiveNSMConfig::get('kafka.topic', 'UpdiveNSM');

        // Cache the flush timeout value early to avoid Config during shutdown
        if ($this->kafkaFlushTimeout == null) {
            $this->kafkaFlushTimeout = UpdiveNSMConfig::get('kafka.flush.timeout', 100);
        }

        // Load excluded values from config
        foreach (UpdiveNSMConfig::get('kafka.groups-exclude', []) as $exclude_group) {
        ...
        foreach (UpdiveNSMConfig::get('kafka.measurement-exclude', []) as $exclude_measurement) {
    ...
    public static function getClient(): Producer
    {
        ...
        $conf->set('bootstrap.servers', UpdiveNSMConfig::get('kafka.broker.list', '127.0.2.2:9092'));
        // Set the idempotence
        $conf->set('enable.idempotence', UpdiveNSMConfig::get('kafka.idempotence', false) ? 'true' : 'false');
        // Max queue allowed messages in poller memory
        $conf->set('queue.buffering.max.messages', UpdiveNSMConfig::get('kafka.buffer.max.message', 1_000));
        // Num of messages each call to kafka
        $conf->set('batch.num.messages', UpdiveNSMConfig::get('kafka.batch.max.message', 200));
        // Max wait time to acumulate before sending the batch
        $conf->set('linger.ms', UpdiveNSMConfig::get('kafka.linger.ms', default: 50));
        // Change ACK
        $conf->set(
            ...
            UpdiveNSMConfig::get('kafka.idempotence', false) ? 'all' :
                (UpdiveNSMConfig::get('kafka.request.required.acks', '-1'))
        );

        // check if debug for ssl was set and enable it
        $confKafkaSSLDebug = UpdiveNSMConfig::get('kafka.security.debug', null);
        $confKafkaSSLDebug != null || strlen($confKafkaSSLDebug) !== 0 ? $conf->set('debug', $confKafkaSSLDebug) : null;

        // config ssl
        $isSslEnabled = UpdiveNSMConfig::get('kafka.ssl.enable', false);
        if ($isSslEnabled) {
            $conf->set('security.protocol', UpdiveNSMConfig::get('kafka.ssl.protocol', 'ssl'));
            $conf->set('ssl.endpoint.identification.algorithm', 'none');

            // prepare all necessary UpdiveNSM kafka config with associated rdkafka key
            $kafkaSSLConfigs = [
            ...
            foreach ($kafkaSSLConfigs as $configKey => $kafkaKey) {
                $configValue = UpdiveNSMConfig::get($configKey, null);
    ...
    {
        return UpdiveNSMConfig::get('kafka.enable', false);
    }
    ...
    public function write(string $measurement, array $fields, array $tags = [], array $meta = []): void
    {
        try {
            ...
            if (UpdiveNSMConfig::get('kafka.debug') === true) {
