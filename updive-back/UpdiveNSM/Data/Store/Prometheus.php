...
class Prometheus extends BaseDatastore
{
    ...
    public function __construct()
    {
        ...
        $url = UpdiveNSMConfig::get('prometheus.url');
        $job = UpdiveNSMConfig::get('prometheus.job', 'UpdiveNSM');
        $this->base_uri = "$url/metrics/job/$job/instance/";

        $this->client = Http::client()->baseUrl($this->base_uri);

        $user = UpdiveNSMConfig::get('prometheus.user', '');
        $passwd = UpdiveNSMConfig::get('prometheus.password', '');
        if ($user && $passwd) {
            $this->client = $this->client->withBasicAuth($user, $passwd);
        }

        $this->prefix = UpdiveNSMConfig::get('prometheus.prefix', '');
        if ($this->prefix) {
    ...
    {
        return UpdiveNSMConfig::get('prometheus.enable', false);
    }
    ...
    public function write(string $measurement, array $fields, array $tags = [], array $meta = []): void
    {
        ...
        if (UpdiveNSMConfig::get('prometheus.attach_sysname', false)) {
