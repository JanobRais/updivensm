...
class InfluxDB extends BaseDatastore
{
    ...
    {
        parent::__construct();
        $this->batchSize = UpdiveNSMConfig::get('influxdb.batch_size', 0);
        $this->measurements = UpdiveNSMConfig::get('influxdb.measurements', []);
        ...
        // reception.
        if (UpdiveNSMConfig::get('influxdb.transport', 'http') !== 'udp') {
            try {
    ...
    {
        return UpdiveNSMConfig::get('influxdb.enable', false);
    }
    ...
    public function write(string $measurement, array $fields, array $tags = [], array $meta = []): void
    {
        ...
        if (UpdiveNSMConfig::get('influxdb.debug', false) === true) {
    ...
    {
        $host = UpdiveNSMConfig::get('influxdb.host', 'localhost');
        $transport = UpdiveNSMConfig::get('influxdb.transport', 'http');
        $port = UpdiveNSMConfig::get('influxdb.port', 8086);
        $db = UpdiveNSMConfig::get('influxdb.db', 'UpdiveNSM');
        $username = UpdiveNSMConfig::get('influxdb.username', '');
        $password = UpdiveNSMConfig::get('influxdb.password', '');
        $timeout = UpdiveNSMConfig::get('influxdb.timeout', 0);
        $verify_ssl = UpdiveNSMConfig::get('influxdb.verifySSL', false);
