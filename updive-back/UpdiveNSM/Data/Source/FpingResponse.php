...
class FpingResponse implements \Stringable
{
    ...
    public function saveStats(Device $device): void
    {
        ...
        if ($this->avg_latency) {
            ...
            $stats->ping_rtt_avg = $stats->ping_rtt_avg ? $stats->ping_rtt_avg + (($stats->ping_rtt_last - $stats->ping_rtt_avg) * UpdiveNSMConfig::get('device_stats_avg_factor')) : $stats->ping_rtt_last;
        }
        ...
        if ($this->transmitted) {
            ...
            // Average is calcualted as the exponential weighted moving average
            $stats->ping_loss_avg = $stats->ping_loss_avg ? $stats->ping_loss_avg + (($stats->ping_loss_last - $stats->ping_loss_avg) * UpdiveNSMConfig::get('device_stats_avg_factor')) : $stats->ping_loss_last;
        }
        $stats->save();
        ...
        app('Datastore')->put($device->toArray(), 'icmp-perf', [
            'rrd_step' => UpdiveNSMConfig::get('ping_rrd_step'),
