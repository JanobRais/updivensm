...
namespace UpdiveNSM\Data\Source;
...
class NetSnmpQuery implements SnmpQueryInterface
{
    ...
    private function buildCli(string $command, array $oids): array
    {
        ...
        $timeout = $this->device->timeout ?? UpdiveNSMConfig::get('snmp.timeout');
        if ($timeout && $timeout !== 1) {
            array_push($cmd, '-t', $timeout);
        }

        $retries = $this->device->retries ?? UpdiveNSMConfig::get('snmp.retries');
    ...
    private function initCommand(string $binary, array $oids): array
    {
        if ($binary == 'snmpwalk') {
            ...
            if (! empty(array_intersect($oids, UpdiveNSMConfig::getCombined($this->device->os, 'oids.unordered', 'snmp.')))) {
                $this->allowUnordered();
            }

            // handle bulk settings
            if ($this->device->snmpver !== 'v1'
                && UpdiveNSMConfig::getOsSetting($this->device->os, 'snmp_bulk', true)
                && empty(array_intersect($oids, UpdiveNSMConfig::getCombined($this->device->os, 'oids.no_bulk', 'snmp.'))) // skip for oids that do not work with bulk
            ) {
                $snmpcmd = [UpdiveNSMConfig::get('snmpbulkwalk', 'snmpbulkwalk')];

                $max_repeaters = $this->device->getAttrib('snmp_max_repeaters') ?: UpdiveNSMConfig::getOsSetting($this->device->os, 'snmp.max_repeaters', UpdiveNSMConfig::get('snmp.max_repeaters', false));
                if ($max_repeaters > 0) {
        ...
        return [UpdiveNSMConfig::get($binary, $binary)];
    }
    ...
    private function mibDirectories(): string
    {
        ...
        // os group
        if ($os_group = UpdiveNSMConfig::getOsSetting($this->device->os, 'group')) {
        ...
        // os directory
        $os_mibdir = UpdiveNSMConfig::getOsSetting($this->device->os, 'mib_dir');
    ...
    private function limitOids(array $oids): array
    {
        ...
        $configured_max = $this->device->getAttrib('snmp_max_oid') ?: UpdiveNSMConfig::getOsSetting($this->device->os, 'snmp_max_oid', UpdiveNSMConfig::get('snmp.max_oid', 10));
