...
use UpdiveNSM\Exceptions\FpingUnparsableLine;
use Log;
...
class Fping
{
    ...
    public function __construct()
    {
        ...
        $this->count = max(UpdiveNSMConfig::get('fping_options.count', 3), 1);
        $this->interval = max(UpdiveNSMConfig::get('fping_options.interval', 500), 20);
        $this->timeout = max(UpdiveNSMConfig::get('fping_options.timeout', 500), $this->interval);
        $this->retries = UpdiveNSMConfig::get('fping_options.retries', 2);
        $this->tos = UpdiveNSMConfig::get('fping_options.tos', 0);
    }
    ...
    {
        // build the command
        $cmd = array_merge(UpdiveNSMConfig::fpingCommand($address_family), [
            '-e',
            '-q',
            '-c',
            $this->count,
    ...
    public function bulkPing(array $hosts, callable $callback): void
    {
        $process = app()->make(Process::class, ['command' => [
            UpdiveNSMConfig::get('fping', 'fping'),
            '-f', '-',
            '-e',
            '-t', $this->timeout,
            '-r', $this->retries,
        ...
        $process->setTimeout(UpdiveNSMConfig::get('rrd.step', 300) * 2);
