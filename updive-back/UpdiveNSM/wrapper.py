...
Authors:       Orsiris de Jong <contact@netpower.fr>
               Neil Lathwood <neil@UpdiveNSM.org>
...
import UpdiveNSM
from UpdiveNSM.command_runner import command_runner
...
def wrapper(
    ...
    **kwargs,  # Type: dict, may contain modules
    ...
    db_connection = UpdiveNSM.DB(sconfig)
...
if __name__ == "__main__":
    parser = ArgumentParser(
        ...
        description="Spawn multiple UpdiveNSM php processes in parallel.",
    )
    parser.add_argument(
        "-d",
        "--debug",
    ...
    config = UpdiveNSM.get_config_data(
        os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
    )
    log_dir = config["log_dir"]
    log_file = os.path.join(log_dir, wrapper_type + ".log")
    logger = UpdiveNSM.logger_get_logger(log_file, debug=debug)
