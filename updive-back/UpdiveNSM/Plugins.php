...
class Plugins
{
    ...
    public static function start()
    {
        ...
        self::$plugins = [];
        $plugin_dir = UpdiveNSMConfig::get('plugin_dir');
    ...
    {
        chdir(UpdiveNSMConfig::get('install_dir') . '/html');
        ...
        chdir(UpdiveNSMConfig::get('install_dir'));
        ...
        if (! empty(self::$plugins[$hook])) {
            foreach (self::$plugins[$hook] as $plugin) {
                try {
                    ...
                    Notifications::create("Plugin $name disabled", "$name caused an error and was disabled, please check with the plugin creator to fix the error. The error can be found in logs/UpdiveNSM.log", 'plugins', 2);
    ...
    public static function scanNew()
    {
        ...
        if (file_exists(UpdiveNSMConfig::get('plugin_dir'))) {
            $plugin_files = array_diff(scandir(UpdiveNSMConfig::get('plugin_dir')), ['..', '.']);
            $plugin_files = array_diff($plugin_files, Plugin::versionOne()->pluck('plugin_name')->toArray());
            foreach ($plugin_files as $name) {
                if (is_dir(UpdiveNSMConfig::get('plugin_dir') . '/' . $name)
                    && is_file(UpdiveNSMConfig::get('plugin_dir') . '/' . $name . '/' . $name . '.php')) {
    ...
    public static function scanRemoved()
    {
        ...
        if (file_exists(UpdiveNSMConfig::get('plugin_dir'))) {
            $plugin_files = array_diff(scandir(UpdiveNSMConfig::get('plugin_dir')), ['.', '..', '.gitignore']);
