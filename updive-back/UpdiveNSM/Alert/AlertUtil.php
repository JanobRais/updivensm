...
use UpdiveNSM\Enum\AlertRuleOperationPhase;
use UpdiveNSM\Enum\MaintenanceStatus;
...
class AlertUtil
{
    ...
    public static function mergeProblemPhaseTimingFromOperations(int $ruleId, array &$details, array &$rextra): void
    {
        ...
        if ($opDefault === null) {
            $opDefault = max(0, 60 * (int) UpdiveNSMConfig::get('alert_rule.default_operation_step_duration', UpdiveNSMConfig::get('alert_rule.interval')));
        }
    ...
    public static function getContacts($results)
    {
        ...
        if (UpdiveNSMConfig::get('alert.default_only') === true || UpdiveNSMConfig::get('alerts.email.default_only') === true) {
            $email = UpdiveNSMConfig::get('alert.default_mail', UpdiveNSMConfig::get('alerts.email.default'));
        ...
        if (UpdiveNSMConfig::get('alert.syscontact')) {
            $contacts = array_merge($contacts, self::findContactsSysContact($results));
        }

        if (UpdiveNSMConfig::get('alert.users')) {
            $contacts = array_merge($contacts, self::findContactsOwners($results));
        }

        $roles = UpdiveNSMConfig::get('alert.globals')
            ? ['admin', 'global-read']
            : (UpdiveNSMConfig::get('alert.admins') ? ['admin'] : []);
        if ($roles) {
        ...
        $default_mail = UpdiveNSMConfig::get('alert.default_mail');
        if (! isset($tmp_contacts[$default_mail]) && UpdiveNSMConfig::get('alert.default_copy')) {
            $tmp_contacts[$default_mail] = '';
        }
        // Send email to default contact if no other contact found
        if (empty($tmp_contacts) && UpdiveNSMConfig::get('alert.default_if_none') && $default_mail) {
            $tmp_contacts[$default_mail] = '';
        }
    ...
    {
        $macros = UpdiveNSMConfig::get('alert.macros.rule', []);
        krsort($macros);
