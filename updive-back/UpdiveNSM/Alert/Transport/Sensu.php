 ...
 * @copyright 2020 Adam Bishop, UpdiveNSM
 * @license GPL
 */

namespace UpdiveNSM\Alert\Transport;

use App\Facades\UpdiveNSMConfig;
use Illuminate\Support\Facades\Log;
use UpdiveNSM\Alert\Transport;
use UpdiveNSM\Enum\AlertState;
use UpdiveNSM\Exceptions\AlertTransportDeliveryException;
use UpdiveNSM\Util\Http;
...
class Sensu extends Transport
{
    ...
    public function deliverAlert(array $alert_data): bool
    {
        ...
        if ($alert_data['state'] !== AlertState::RECOVERED && $alert_data['state'] !== AlertState::ACKNOWLEDGED && $alert_data['alerted'] === 0) {
            ...
            $data = $this->generateData($alert_data, self::OK, (int) round(UpdiveNSMConfig::get('rrd.step', 300) / 2));
    ...
    private function generateData(array $alert_data, int $status, int $offset = 0): array
    {
        ...
        return [
            'check' => [
                ...
                'command' => sprintf('UpdiveNSM: %s', $alert_data['builder']),
                'executed' => time() - $offset,
                'interval' => UpdiveNSMConfig::get('rrd.step', 300),
                'issued' => time() - $offset,
    ...
    private function generateAnnotations(array $alert_data): array
    {
        return array_filter([
            ...
            'UpdiveNSM-notes' => $alert_data['notes'],
            'UpdiveNSM-device-id' => strval($alert_data['device_id']),
            'UpdiveNSM-rule-id' => strval($alert_data['rule_id']),
            'UpdiveNSM-status-reason' => $alert_data['status_reason'],
    ...
    {
        // Sensu only has a single short (status) to indicate both severity and status, so we need to map UpdiveNSM' state and severity onto it

        if ($state === AlertState::RECOVERED) {
            // UpdiveNSM alert is resolved, send ok
            return self::OK;
        }
    ...
    {
        // Shrink the last domain components - e.g. UpdiveNSM.corp.example.net becomes UpdiveNSM.cen
