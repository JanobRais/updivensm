...
class RunAlerts
{
    ...
    {
        if (UpdiveNSMConfig::get('alert.fixed-contacts') == false) {
        ...
        if (is_array($obj)) {
            ...
            if ($alert['state'] != AlertState::ACKNOWLEDGED || UpdiveNSMConfig::get('alert.acknowledged') === true) {
    ...
    public function runAlerts()
    {
        foreach ($this->loadAlerts('alerts.state != ' . AlertState::ACKNOWLEDGED . ' && alerts.open = 1') as $alert) {
            ...
            $tolerence_window = UpdiveNSMConfig::get('alert.tolerance_window');
    ...
    public function extTransports($obj)
    {
        ...
        if (UpdiveNSMConfig::get('alert.transports.mail') === true && ! empty($obj['contacts'])) {
            $transport_maps[] = [
                'transport_id' => null,
                'transport_type' => 'mail',
