...
class Programs extends BaseValidation
{
    ...
    {
        $validator->info('fping FAILURES can be ignored if running UpdiveNSM in a jail without ::1. You may want to test it manually: fping ::1');
        $validator->fail(
    ...
    {
        if (is_executable(UpdiveNSMConfig::get($bin))) {
            return UpdiveNSMConfig::get($bin);
        }

        $located = UpdiveNSMConfig::locateBinary($bin);
