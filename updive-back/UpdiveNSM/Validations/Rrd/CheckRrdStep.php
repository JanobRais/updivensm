...
namespace UpdiveNSM\Validations\Rrd;

use App\Facades\UpdiveNSMConfig;
use UpdiveNSM\Exceptions\RrdException;
use UpdiveNSM\Interfaces\Validation;
...
class CheckRrdStep implements Validation
{
    ...
    {
        $this->ping_rrd_step = (int) UpdiveNSMConfig::get('ping_rrd_step', self::DEFAULT_PING_RRD_STEP);
        $this->rrd_step = (int) UpdiveNSMConfig::get('rrd.step', self::DEFAULT_RRD_STEP);
    ...
    {
        return UpdiveNSMConfig::get('rrd.step') !== self::DEFAULT_RRD_STEP || UpdiveNSMConfig::get('ping_rrd_step') !== self::DEFAULT_PING_RRD_STEP;
    }
    ...
    {
        $dir = UpdiveNSMConfig::get('rrdcached') ? '/' : UpdiveNSMConfig::get('rrd_dir');
