...
class Python extends BaseValidation
{
    ...
    private function checkExtensions(Validator $validator)
    {
        ...
        $process = new Process([UpdiveNSMConfig::get('install_dir') . $pythonExtensions, '-v']);
        $process->run();

        if ($process->getExitCode() !== 0) {
            $user = \config('UpdiveNSM.user');
            ...
            if ($user_mismatch) {
                $validator->warn(
                    ...
                    'The install docs show how this is done on a new install: https://docs.UpdiveNSM.org/Installation/Install-UpdiveNSM/#configure-php-fpm'
                );
            } else {
                $validator->fail("Python3 module issue found: '" . $process->getOutput() . "'", 'pip3 install -r ' . UpdiveNSMConfig::get('install_dir') . '/requirements.txt');
            }
        }
    }
}
