...
namespace UpdiveNSM\Snmptrap;

use App\Facades\UpdiveNSMConfig;
use App\Models\Eventlog;
use UpdiveNSM\Alert\AlertRules;
use UpdiveNSM\Snmptrap\Handlers\Fallback;
use Log;
...
class Dispatcher
{
    ...
    public static function handle(Trap $trap): bool
    {
        ...
        if ($trap->findOid('iso.3.6.1.6.3.1.1.4.1.0')) {
            ...
            // UpdiveNSM snmptraps code cannot process received data. Let's inform the user.
            Eventlog::log('Misconfigured MIBS or MIBDIRS for snmptrapd, check https://docs.UpdiveNSM.org/Extensions/SNMP-Trap-Handler/ : ' . $trap->raw, $trap->getDevice(), 'system');

            return false;
        }

        // note, this doesn't clear the resolved SnpmtrapHandler so only one per run
        /** @var \UpdiveNSM\Interfaces\SnmptrapHandler $handler */
        $handler = app(\UpdiveNSM\Interfaces\SnmptrapHandler::class, [$trap->getTrapOid()]);
        $handler->handle($trap->getDevice(), $trap);

        // log an event if appropriate
        $fallback = $handler instanceof Fallback;
        $logging = UpdiveNSMConfig::get('snmptraps.eventlog', 'unhandled');
        $detailed = UpdiveNSMConfig::get('snmptraps.eventlog_detailed', false);
