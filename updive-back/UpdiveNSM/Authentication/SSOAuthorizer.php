...
class SSOAuthorizer extends MysqlAuthorizer
{
    ...
    public function authenticate($credentials)
    {
        ...
        if (UpdiveNSMConfig::get('sso.create_users') || UpdiveNSMConfig::get('sso.update_users')) {
            $user = User::thisAuth()->firstOrNew(['username' => $credentials['username']]);

            $create = ! $user->exists && UpdiveNSMConfig::get('sso.create_users');
            $update = $user->exists && UpdiveNSMConfig::get('sso.update_users');

            if ($create || $update) {
                $user->auth_type = LegacyAuth::getType();
                $user->can_modify_passwd = 0;
                $user->realname = $this->authSSOGetAttr(UpdiveNSMConfig::get('sso.realname_attr'));
                $user->email = $this->authSSOGetAttr(UpdiveNSMConfig::get('sso.email_attr'));
                $user->descr = $this->authSSOGetAttr(UpdiveNSMConfig::get('sso.descr_attr')) ?: 'SSO User';
                $user->save();
            }
        }
    ...
    {
        return $this->authSSOGetAttr(UpdiveNSMConfig::get('sso.user_attr'), '');
    }
    ...
    public function authSSOGetAttr(?string $attr, string $prefix = 'HTTP_')
    {
        ...
        if ($this->authSSOProxyTrusted()) {
            ...
            if (UpdiveNSMConfig::get('sso.mode') === 'header' && array_key_exists($header_key, $_SERVER)) {
                return $_SERVER[$header_key];
            } elseif (UpdiveNSMConfig::get('sso.mode') === 'env' && array_key_exists($attr, $_SERVER)) {
                return $_SERVER[$attr];
            } else {
                return null;
            }
    ...
    public function authSSOProxyTrusted()
    {
        ...
        if (UpdiveNSMConfig::get('sso.trusted_proxies')) {
            try {
                ...
                $proxies = UpdiveNSMConfig::get('sso.trusted_proxies');
    ...
    {
        if (UpdiveNSMConfig::get('sso.group_strategy') === 'attribute') {
            if (UpdiveNSMConfig::get('sso.level_attr')) {
                if (is_numeric($this->authSSOGetAttr(UpdiveNSMConfig::get('sso.level_attr')))) {
                    return Arr::wrap(LegacyAuthLevel::tryFrom((int) $this->authSSOGetAttr(UpdiveNSMConfig::get('sso.level_attr')))?->getName());
                } else {
        ...
        } elseif (UpdiveNSMConfig::get('sso.group_strategy') === 'map') {
            if (UpdiveNSMConfig::get('sso.group_level_map') && is_array(UpdiveNSMConfig::get('sso.group_level_map')) && UpdiveNSMConfig::get('sso.group_delimiter') && UpdiveNSMConfig::get('sso.group_attr')) {
        ...
        } elseif (UpdiveNSMConfig::get('sso.group_strategy') === 'static') {
            if (UpdiveNSMConfig::get('sso.static_level')) {
                return Arr::wrap(LegacyAuthLevel::tryFrom((int) UpdiveNSMConfig::get('sso.static_level'))?->getName());
            } else {
    ...
    public function authSSOParseGroups()
    {
        ...
        $groups = explode(UpdiveNSMConfig::get('sso.group_delimiter', ';'), $this->authSSOGetAttr(UpdiveNSMConfig::get('sso.group_attr')) ?? '');

        $valid_groups = [];

        // Only consider groups that match the filter expression - this is an optimisation for sites with thousands of groups
        if (UpdiveNSMConfig::get('sso.group_filter')) {
            foreach ($groups as $group) {
                if (preg_match(UpdiveNSMConfig::get('sso.group_filter'), $group)) {
        ...
        $level = (int) UpdiveNSMConfig::get('sso.static_level', 0);

        $config_map = UpdiveNSMConfig::get('sso.group_level_map');
