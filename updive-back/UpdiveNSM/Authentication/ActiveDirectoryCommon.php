...
trait ActiveDirectoryCommon
{
    ...
    protected function userFilter($username)
    {
        ...
        $extra = UpdiveNSMConfig::get('auth_ad_user_filter');
        if ($extra) {
            $user_filter .= $extra;
        }
        $user_filter .= ')';
    ...
    protected function groupFilter($groupname)
    {
        ...
        $extra = UpdiveNSMConfig::get('auth_ad_group_filter');
        if ($extra) {
    ...
    public function getGroupList()
    {
        ...
        if (UpdiveNSMConfig::has('auth_ad_group')) {
            if (UpdiveNSMConfig::get('auth_ad_group') !== $default_group) {
                $ldap_groups[] = UpdiveNSMConfig::get('auth_ad_group');
            }
        }

        if (! UpdiveNSMConfig::has('auth_ad_groups') && ! UpdiveNSMConfig::has('auth_ad_group')) {
            $ldap_groups[] = $this->getDn($default_group);
        }

        foreach (UpdiveNSMConfig::get('auth_ad_groups') as $key => $value) {
    ...
    public function getUser($user_id): array
    {
        ...
        $search = ldap_search($connection, UpdiveNSMConfig::get('auth_ad_base_dn'), $search_filter, $attributes);
    ...
    protected function getDomainSid(): string
    {
        ...
        $dn_candidate = preg_replace('/^.*?DC=/i', 'DC=', UpdiveNSMConfig::get('auth_ad_base_dn'));
