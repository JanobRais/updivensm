...
use UpdiveNSM\Exceptions\LdapMissingException;
...
class LdapAuthorizer extends AuthorizerBase
{
    ...
    public function authenticate($credentials)
    {
        ...
        if (! empty($credentials['username'])) {
            ...
            if (UpdiveNSMConfig::get('auth_ldap_wildcard_ou', false)) {
            ...
            if (! empty($credentials['password']) && ldap_bind($connection, $this->getFullDn($username), $credentials['password'])) {
                ...
                if ((UpdiveNSMConfig::has('auth_ldap_binduser') || UpdiveNSMConfig::has('auth_ldap_binddn')) && UpdiveNSMConfig::has('auth_ldap_bindpassword')) {
                    $this->bind();
                }

                if (UpdiveNSMConfig::get('auth_ldap_require_groupmembership') === false) {
                ...
                if (empty($ldap_groups)) {
                    ...
                    foreach ($ldap_groups as $ldap_group) {
                        if (UpdiveNSMConfig::get('auth_ldap_userdn') === true) {
                            $ldap_comparison = ldap_compare(
                                $connection,
                                $ldap_group,
                                UpdiveNSMConfig::get('auth_ldap_groupmemberattr', 'memberUid'),
    ...
    public function userExists($username, $throw_exception = false)
    {
        try {
            ...
            $filter = '(' . UpdiveNSMConfig::get('auth_ldap_prefix') . $username . ')';
            $search = ldap_search($connection, trim(UpdiveNSMConfig::get('auth_ldap_suffix'), ','), $filter);
    ...
    public function getRoles(string $username): array|false
    {
        try {
            $connection = $this->getLdapConnection();
            $groups = UpdiveNSMConfig::get('auth_ldap_groups');
            ...
            if (UpdiveNSMConfig::get('auth_ldap_userdn') === true) {
                $filter = "(&{$ldap_group_filter}(" . trim(UpdiveNSMConfig::get('auth_ldap_groupmemberattr', 'memberUid')) . '=' . $this->getFullDn($username) . '))';
            } else {
                $filter = "(&{$ldap_group_filter}(" . trim(UpdiveNSMConfig::get('auth_ldap_groupmemberattr', 'memberUid')) . '=' . $this->getMembername($username) . '))';
            }
            $search = ldap_search($connection, UpdiveNSMConfig::get('auth_ldap_groupbase'), $filter);
            $entries = ldap_get_entries($connection, $search);
    ...
    public function getUserid($username)
    {
        try {
            ...
            if ($entries['count']) {
                $uid_attr = strtolower(UpdiveNSMConfig::get('auth_ldap_uid_attribute', 'uidnumber'));
    ...
    public function getUser($user_id): false|array
    {
        ...
        $filter = '(' . UpdiveNSMConfig::get('auth_ldap_prefix') . $this->userloginname . ')';
        if (UpdiveNSMConfig::get('auth_ldap_userlist_filter') != null) {
            $filter = '(' . UpdiveNSMConfig::get('auth_ldap_userlist_filter') . ')';
        }
    ...
    {
        $type = UpdiveNSMConfig::get('auth_ldap_groupmembertype');
        ...
        if ($type == 'puredn') {
            try {
                $connection = $this->getLdapConnection();
                $filter = '(' . UpdiveNSMConfig::get('auth_ldap_attr.uid') . '=' . $username . ')';
                $search = ldap_search($connection, UpdiveNSMConfig::get('auth_ldap_groupbase'), $filter);
                $entries = ldap_get_entries($connection, $search);
    ...
    public function getGroupList(): array
    {
        ...
        if (UpdiveNSMConfig::get('auth_ldap_group', $default_group) !== $default_group) {
            $ldap_groups[] = UpdiveNSMConfig::get('auth_ldap_group');
        }

        foreach (UpdiveNSMConfig::get('auth_ldap_groups') as $key => $value) {
            $ldap_groups[] = "cn=$key," . UpdiveNSMConfig::get('auth_ldap_groupbase');
        }
    ...
    {
        return UpdiveNSMConfig::get('auth_ldap_prefix', '') . $username . UpdiveNSMConfig::get('auth_ldap_suffix', '');
    }

    /**
     * Set auth_ldap_suffix ou according to $username dn
     * useful if UpdiveNSMConfig::get('auth_ldap_wildcard_ou) is set
     *
     * @internal
     *
     * @return false|true
    ...
    protected function setAuthLdapSuffixOu($username): bool
    {
        $connection = $this->getLdapConnection();
        $filter = '(' . UpdiveNSMConfig::get('auth_ldap_attr.uid') . '=' . $username . ')';
        $base_dn = preg_replace('/,ou=[^,]+,/', ',', UpdiveNSMConfig::get('auth_ldap_suffix'));
        ...
        foreach ($results as $entry) {
            if (isset($entry['uid'][0]) && $entry['uid'][0] == $username) {
                ...
                $user_ou = $matches[1] ?? '';
                $new_auth_ldap_suffix = preg_replace('/,ou=[^,]+,/', ',ou=' . $user_ou . ',', UpdiveNSMConfig::get('auth_ldap_suffix'));
                UpdiveNSMConfig::set('auth_ldap_suffix', $new_auth_ldap_suffix);
    ...
    private function ldapToUser(array $entry): array
    {
        ...
        return [
            ...
            'email' => $entry[UpdiveNSMConfig::get('auth_ldap_emailattr', 'mail')][0] ?? null,
        ];
    }
    ...
    private function connect(): void
    {
        ...
        $port = UpdiveNSMConfig::get('auth_ldap_port');
        $uri = UpdiveNSMConfig::get('auth_ldap_server');
        ...
        ldap_set_option($this->ldap_connection, LDAP_OPT_PROTOCOL_VERSION, UpdiveNSMConfig::get('auth_ldap_version', 3));

        $use_tls = UpdiveNSMConfig::get('auth_ldap_starttls');
    ...
    {
        if (UpdiveNSMConfig::get('auth_ldap_debug')) {
         ...
         */
        if (UpdiveNSMConfig::get('auth_ldap_cacertfile')) {
            ldap_set_option($this->ldap_connection, LDAP_OPT_X_TLS_CACERTFILE, UpdiveNSMConfig::get('auth_ldap_cacertfile'));
        }
        if (UpdiveNSMConfig::get('auth_ldap_ignorecert')) {
        ...
        if ((UpdiveNSMConfig::has('auth_ldap_binduser') || UpdiveNSMConfig::has('auth_ldap_binddn')) && UpdiveNSMConfig::has('auth_ldap_bindpassword')) {
            if (UpdiveNSMConfig::get('auth_ldap_binddn') == null) {
                UpdiveNSMConfig::set('auth_ldap_binddn', $this->getFullDn(UpdiveNSMConfig::get('auth_ldap_binduser')));
            }
            $username = UpdiveNSMConfig::get('auth_ldap_binddn');
            $password = UpdiveNSMConfig::get('auth_ldap_bindpassword');
        } elseif (! empty($credentials['username'])) {
            $username = $this->getFullDn($credentials['username']);
        }

        // With specified bind user
        ldap_set_option($this->ldap_connection, LDAP_OPT_NETWORK_TIMEOUT, UpdiveNSMConfig::get('auth_ldap_timeout', 5));
