...
class ActiveDirectoryAuthorizer extends AuthorizerBase
{
    ...
    public function authenticate($credentials)
    {
        ...
        if ($this->ldap_connection) {
            ...
            if (! empty($credentials['username']) && ! empty($credentials['password']) && ldap_bind($this->ldap_connection, $credentials['username'] . '@' . UpdiveNSMConfig::get('auth_ad_domain'), $credentials['password'])) {
                $this->is_bound = true;
                // group membership in one of the configured groups is required
                if (UpdiveNSMConfig::get('auth_ad_require_groupmembership', true)) {
                    // cycle through defined groups, test for memberOf-ship
                    foreach (UpdiveNSMConfig::get('auth_ad_groups', []) as $group => $level) {
                    ...
                    // failed to find user
                    if (UpdiveNSMConfig::get('auth_ad_debug', false)) {
        ...
        } elseif (UpdiveNSMConfig::get('auth_ad_debug', false)) {
    ...
    protected function userInGroup($username, $groupname)
    {
        ...
        $search = ldap_search(
            $connection,
            UpdiveNSMConfig::get('auth_ad_base_dn'),
            $search_filter,
            ['cn']
        );
    ...
    public function getRoles(string $username): array|false
    {
        $roles = [];
        if (! UpdiveNSMConfig::get('auth_ad_require_groupmembership', true)) {
            if (UpdiveNSMConfig::get('auth_ad_global_read', false)) {
                $roles[] = 'global-read';
            }
        }

        // cycle through defined groups, test for memberOf-ship
        foreach (UpdiveNSMConfig::get('auth_ad_groups', []) as $group => $data) {
            try {
    ...
    protected function connect()
    {
        ...
        if (UpdiveNSMConfig::has('auth_ad_check_certificates') &&
            ! UpdiveNSMConfig::get('auth_ad_check_certificates')) {
            putenv('LDAPTLS_REQCERT=never');
        }

        if (UpdiveNSMConfig::has('auth_ad_check_certificates') && UpdiveNSMConfig::get('auth_ad_debug')) {
            ldap_set_option(null, LDAP_OPT_DEBUG_LEVEL, 7);
        }

        $this->ldap_connection = ldap_connect(UpdiveNSMConfig::get('auth_ad_url'));
        if (empty($this->ldap_connection)) {
            throw new AuthenticationException('Fatal error while connecting to AD, uri not valid: ' . UpdiveNSMConfig::get('auth_ad_url'));
        }
        ...
        $starttls = UpdiveNSMConfig::get('auth_ad_starttls');
    ...
    public function bind($credentials = [])
    {
        ...
        if (UpdiveNSMConfig::has('auth_ad_binduser') && UpdiveNSMConfig::has('auth_ad_bindpassword')) {
            $username = UpdiveNSMConfig::get('auth_ad_binduser');
            $password = UpdiveNSMConfig::get('auth_ad_bindpassword');
        }
        $username .= '@' . UpdiveNSMConfig::get('auth_ad_domain');

        ldap_set_option($this->ldap_connection, LDAP_OPT_NETWORK_TIMEOUT, UpdiveNSMConfig::get('auth_ad_timeout', 5));
