...
/**
 * UpdiveNSM HTTP-Authentication and LDAP Authorization Library
 *
 * @author Maximilian Wilhelm <max@rfc2324.org>
 * @copyright 2016 UpdiveNSM, Barbarossa
 * @license GPL
 */
...
class LdapAuthorizationAuthorizer extends AuthorizerBase
{
    ...
    public function __construct()
    {
        ...
        if (UpdiveNSMConfig::get('auth_ldap_version')) {
            ldap_set_option($this->ldap_connection, LDAP_OPT_PROTOCOL_VERSION, UpdiveNSMConfig::get('auth_ldap_version'));
        }

        if (UpdiveNSMConfig::get('auth_ldap_starttls') && (UpdiveNSMConfig::get('auth_ldap_starttls') == 'optional' || UpdiveNSMConfig::get('auth_ldap_starttls') == 'required')) {
            $tls = ldap_start_tls($this->ldap_connection);
            if (UpdiveNSMConfig::get('auth_ldap_starttls') == 'required' && $tls === false) {
    ...
    public function authenticate($credentials)
    {
        ...
        $guest = UpdiveNSMConfig::get('http_auth_guest');
    ...
    public function userExists($username, $throw_exception = false)
    {
        ...
        $search = ldap_search($this->ldap_connection, trim(UpdiveNSMConfig::get('auth_ldap_suffix'), ','), $filter);
        if ($search === false) {
    ...
    public function getRoles(string $username): array|false
    {
        ...
        $filter = '(&(|(cn=' . implode(')(cn=', array_keys(UpdiveNSMConfig::get('auth_ldap_groups'))) . '))(' . UpdiveNSMConfig::get('auth_ldap_groupmemberattr') . '=' . $this->getMembername($username) . '))';
        $search = ldap_search($this->ldap_connection, UpdiveNSMConfig::get('auth_ldap_groupbase'), $filter);
        if ($search === false) {
            throw new AuthenticationException('Role search failed: ' . ldap_error($this->ldap_connection));
        }
        $entries = ldap_get_entries($this->ldap_connection, $search);

        $authLdapGroups = UpdiveNSMConfig::get('auth_ldap_groups');
        // Collect all roles
    ...
    public function getUserid($username)
    {
        ...
        $guest_username = UpdiveNSMConfig::get('http_auth_guest');
    ...
    public function getUser($user_id)
    {
        ...
        if ($entries['count']) {
            ...
            $email = $entry[UpdiveNSMConfig::get('auth_ldap_emailattr')][0];
            $ldap_groups = $this->getGroupList();
            foreach ($ldap_groups as $ldap_group) {
                $ldap_comparison = ldap_compare(
                    $this->ldap_connection,
                    $ldap_group,
                    UpdiveNSMConfig::get('auth_ldap_groupmemberattr'),
                    $this->getMembername($username)
                );
                if (! UpdiveNSMConfig::has('auth_ldap_group') || $ldap_comparison === true) {
                    return [
                        'username' => $username,
                        'realname' => $realname,
                        'user_id' => $user_id,
    ...
    {
        if (UpdiveNSMConfig::get('auth_ldap_groupmembertype') == 'fulldn') {
            $membername = UpdiveNSMConfig::get('auth_ldap_prefix') . $username . UpdiveNSMConfig::get('auth_ldap_suffix');
        } elseif (UpdiveNSMConfig::get('auth_ldap_groupmembertype') == 'puredn') {
    ...
    public function getGroupList()
    {
        ...
        if (UpdiveNSMConfig::has('auth_ldap_group')) {
            if (UpdiveNSMConfig::get('auth_ldap_group') !== $default_group) {
                $ldap_groups[] = UpdiveNSMConfig::get('auth_ldap_group');
            }
        }

        foreach (UpdiveNSMConfig::get('auth_ldap_groups') as $key => $value) {
            $dn = "cn=$key," . UpdiveNSMConfig::get('auth_ldap_groupbase');
            $ldap_groups[] = $dn;
        }
