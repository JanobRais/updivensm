...
use UpdiveNSM\Enum\ProcessType;
use UpdiveNSM\Modules\LegacyModule;
...
class Module
{
    ...
    {
        if (class_exists(StringHelpers::toClass($module_name, '\\UpdiveNSM\\Modules\\'))) {
            return true;
        }
        ...
        return UpdiveNSMConfig::has('discovery_modules.' . $module_name) || UpdiveNSMConfig::has('poller_modules.' . $module_name);
    }

    public static function fromName(string $module_name): \UpdiveNSM\Interfaces\Module
    {
        $module_class = StringHelpers::toClass($module_name, '\\UpdiveNSM\\Modules\\');
