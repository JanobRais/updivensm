...
use UpdiveNSM\Data\Graphing\GraphImage;
...
class Graph
{
    ...
    public static function getSubtypes($type, $device = null): array
    {
        ...
        if ($device != null) {
            ...
            foreach (UpdiveNSMConfig::get('graph_types') as $type => $type_data) {
    ...
    {
        return UpdiveNSMConfig::get("graph_types.$type.$subtype.section") == 'mib';
    }

    public static function getOverviewGraphsForDevice(Device $device): array
    {
        if ($device->snmp_disable) {
            return Arr::wrap(UpdiveNSMConfig::getOsSetting('ping', 'over'));
        }

        if ($graphs = UpdiveNSMConfig::getOsSetting($device->os, 'over')) {
            return Arr::wrap($graphs);
        }

        $os_group = UpdiveNSMConfig::getOsSetting($device->os, 'group');

        return Arr::wrap(UpdiveNSMConfig::get("os_group.$os_group.over", UpdiveNSMConfig::get('os.default.over')));
    }
    ...
    {
        $type = UpdiveNSMConfig::get('webui.graph_type');
        $height ??= $width / 3;
