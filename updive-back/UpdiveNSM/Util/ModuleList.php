...
class ModuleList
{
    ...
    public function modulesWithStatus(ProcessType $type, Device $device): array
    {
        $default_modules = match ($type) {
            ProcessType::Poller => UpdiveNSMConfig::get('poller_modules', []),
            ProcessType::Discovery => UpdiveNSMConfig::get('discovery_modules', []),
        };
    ...
    private function moduleStatus(ProcessType $type, string $module_name, Device $device): ModuleStatus
    {
        ...
        return match ($type) {
            ProcessType::Discovery => new ModuleStatus(
                UpdiveNSMConfig::get("discovery_modules.$module_name"),
                UpdiveNSMConfig::get("os.{$device->os}.discovery_modules.$module_name"),
            ...
            ProcessType::Poller => new ModuleStatus(
                UpdiveNSMConfig::get("poller_modules.$module_name"),
                UpdiveNSMConfig::get("os.{$device->os}.poller_modules.$module_name"),
