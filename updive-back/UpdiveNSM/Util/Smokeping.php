...
class Smokeping
{
    ...
    {
        if (is_null($this->files) && UpdiveNSMConfig::has('smokeping.dir')) {
                ...
                foreach (array_diff(scandir($dir), ['.', '..']) as $file) {
                    if (stripos($file, '.rrd') !== false) {
                        if (str_contains($file, '~')) {
                            ...
                            $this->files['in'][$target][UpdiveNSMConfig::get('own_hostname')] = $file;
                            $this->files['out'][UpdiveNSMConfig::get('own_hostname')][$target] = $file;
                        }
                    }
                }
            }
    ...
    {
        if (UpdiveNSMConfig::get('smokeping.integration') === true) {
            return UpdiveNSMConfig::get('smokeping.dir') . '/' . ($this->device->type ?: 'Ungrouped') . '/' . $file;
        } else {
            return UpdiveNSMConfig::get('smokeping.dir') . '/' . $file;
        }
    }
        ...
        foreach ($this->getFiles()[$direction][$this->device->hostname] as $remote_host => $file) {
            if (Str::contains($file, '~')) {
                ...
                if (empty($device->device_id)) {
                    \Log::debug('Could not find smokeping slave device in UpdiveNSM', ['slave' => $remote_host]);
                    continue;
                }
