...
use UpdiveNSM\Data\Source\SnmpResponse;
use UpdiveNSM\Exceptions\FileNotFoundException;
use UpdiveNSM\Exceptions\InvalidModuleException;
...
class ModuleTestHelper
{
    ...
    public function __construct(private readonly ModuleList $modules, string $os, string $variant = '')
    {
        ...
        $install_dir = UpdiveNSMConfig::get('install_dir');
        ...
        UpdiveNSMConfig::set('hide_rrd_disabled', true);
        UpdiveNSMConfig::set('influxdb.enable', false);
        UpdiveNSMConfig::set('influxdbv2.enable', false);
        UpdiveNSMConfig::set('graphite.enable', false);
        UpdiveNSMConfig::set('prometheus.enable', false);
    ...
    {
        global $device;
        UpdiveNSMConfig::set('rrd.enable', false); // disable rrd
        UpdiveNSMConfig::set('rrdtool_version', '1.7.2'); // don't detect rrdtool version, rrdtool is not install on ci
    ...
    {
        if ($short) {
            return ltrim(str_replace(UpdiveNSMConfig::get('install_dir'), '', $this->json_file), '/');
        }
