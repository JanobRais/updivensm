...
namespace UpdiveNSM\Util;

use App\Facades\UpdiveNSMConfig;
use UpdiveNSM\Exceptions\RrdGraphException;
...
class Mail
{
    ...
    public static function parseEmails(string $emails): array
    {
        ...
        foreach ($emails as $email) {
            ...
            if (filter_var($email, FILTER_VALIDATE_EMAIL) !== false) {
                $result[$email] = $out[1][0] ?? UpdiveNSMConfig::get('email_user');
            }
        }
    ...
    public static function send($emails, $subject, $message, bool $html = false, bool $bcc = false, ?bool $embedGraphs = null): bool
    {
        if (is_array($emails) || ($emails = self::parseEmails($emails))) {
            ...
            foreach (self::parseEmails((string) UpdiveNSMConfig::get('email_from')) as $from => $from_name) {
            ...
            $mail->XMailer = UpdiveNSMConfig::get('project_name');
            $mail->CharSet = 'utf-8';
            $mail->WordWrap = 76;
            $mail->Body = $message;
            if ($embedGraphs ?? UpdiveNSMConfig::get('email_attach_graphs')) {
                self::embedGraphs($mail, $html);
            }
            if ($html) {
                $mail->isHTML();
            }
            switch (strtolower(trim(UpdiveNSMConfig::get('email_backend')))) {
                case 'sendmail':
                    $mail->Mailer = 'sendmail';
                    $mail->Sendmail = UpdiveNSMConfig::get('email_sendmail_path');
                    break;
                case 'smtp':
                    $mail->isSMTP();
                    $mail->Host = UpdiveNSMConfig::get('email_smtp_host');
                    $mail->Timeout = UpdiveNSMConfig::get('email_smtp_timeout');
                    $mail->SMTPAuth = UpdiveNSMConfig::get('email_smtp_auth');
                    $mail->SMTPSecure = UpdiveNSMConfig::get('email_smtp_secure');
                    $mail->Port = UpdiveNSMConfig::get('email_smtp_port');
                    $mail->Username = UpdiveNSMConfig::get('email_smtp_username');
                    $mail->Password = UpdiveNSMConfig::get('email_smtp_password');
                    $mail->SMTPAutoTLS = UpdiveNSMConfig::get('email_auto_tls');
                    $mail->SMTPDebug = 0;
                    $mail->SMTPOptions = [
                        'ssl' => [
                            'verify_peer' => UpdiveNSMConfig::get('email_smtp_verifypeer', true),
                            'allow_self_signed' => UpdiveNSMConfig::get('email_smtp_allowselfsigned', false),
                        ],
                    ];
                    break;
                default:
    ...
    private static function embedGraphs(PHPMailer $mail, bool $html = false): void
    {
        ...
        preg_match_all('#<img class=\"UpdiveNSM-graph\" src=\"(.*?)\" ?/?>#', $body, $matches);
