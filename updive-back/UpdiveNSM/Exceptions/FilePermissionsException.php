...
use UpdiveNSM\Interfaces\Exceptions\UpgradeableException;
use Throwable;
...
class FilePermissionsException extends \Exception implements UpgradeableException
{
    ...
    {
        $log_file = config('app.log') ?: UpdiveNSMConfig::get('log_file', base_path('logs/UpdiveNSM.log'));
    ...
    private function generateCommands($log_file): array
    {
        ...
        $group = config('UpdiveNSM.group');
        $install_dir = base_path();
        $commands = [];
        $dirs = [
            base_path('bootstrap/cache'),
            base_path('storage'),
            UpdiveNSMConfig::get('log_dir', base_path('logs')),
            UpdiveNSMConfig::get('rrd_dir', base_path('rrd')),
        ];
        ...
        // check if webserver is in the UpdiveNSM group
