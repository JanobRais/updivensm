...
import UpdiveNSM
from UpdiveNSM.config import DBConfig
...
class ServiceConfig(DBConfig):
    def __init__(self):
        """
        Stores all of the configuration variables for the UpdiveNSM service in a common object
    ...
    watchdog_logfile = "logs/UpdiveNSM.log"
    health_file = ""  # disabled by default

    def populate(self):
        config = UpdiveNSM.get_config_data(self.BASE_DIR)
        ...
        self.watchdog_logfile = config.get("log_file", self.logdir + "/UpdiveNSM.log")
...
class Service:
    ...
    def __init__(self):
        ...
        self._db = UpdiveNSM.DB(self.config)
        ...
        self.daily_timer = UpdiveNSM.RecurringTimer(
            self.config.update_frequency, self.run_maintenance, "maintenance"
        )
        self.stats_timer = UpdiveNSM.RecurringTimer(
        ...
        if self.config.watchdog_enabled:
            ...
            self.watchdog_timer = UpdiveNSM.RecurringTimer(
        ...
        self.systemd_watchdog_timer = UpdiveNSM.RecurringTimer(
    ...
    def start(self):
        ...
        self.poller_manager = UpdiveNSM.PollerQueueManager(self.config, self._lm)
        self.queue_managers["poller"] = self.poller_manager
        self.discovery_manager = UpdiveNSM.DiscoveryQueueManager(self.config, self._lm)
        self.queue_managers["discovery"] = self.discovery_manager
        self.queue_managers["alerting"] = UpdiveNSM.AlertQueueManager(
            self.config, self._lm
        )
        self.queue_managers["services"] = UpdiveNSM.ServicesQueueManager(
            self.config, self._lm
        )
        self.queue_managers["billing"] = UpdiveNSM.BillingQueueManager(
            self.config, self._lm
        )
        self.queue_managers["ping"] = UpdiveNSM.PingQueueManager(self.config, self._lm)
        ...
        logger.info("UpdiveNSM Service: {} started!".format(self.config.unique_name))
        logger.info(
            "Poller group {}. Using Python {} and {} locks and queues".format(
                "0 (default)" if self.config.group == [0] else self.config.group,
                python_version(),
                "redis" if isinstance(self._lm, UpdiveNSM.RedisLock) else "internal",
            )
        )
        logger.info(
    ...
    def run_maintenance(self):
        ...
        exit_code, output = UpdiveNSM.call_script("daily.sh")
        if exit_code == 0:
    ...
    def create_lock_manager(self):
        ...
        return UpdiveNSM.ThreadingLock()
