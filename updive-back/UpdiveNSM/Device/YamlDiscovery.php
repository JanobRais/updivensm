...
use UpdiveNSM\Interfaces\Discovery\DiscoveryItem;
use UpdiveNSM\OS;
...
class YamlDiscovery
{
     ...
     * @return string
     *
     * @throws \UpdiveNSM\Exceptions\InvalidOidException
     */
        ...
        if (! empty($discovery_yaml['modules'])) {
            ...
            foreach ($discovery_yaml['modules'] as $module => $discovery_data) {
                ...
                foreach ($discovery_data as $key => $data_array) {
                    ...
                    $saved_nobulk = UpdiveNSMConfig::getOsSetting($os->getName(), 'snmp_bulk', true);
                        ...
                        foreach (Arr::wrap($data['oid'] ?? []) as $oid) {
                            if (! array_key_exists($oid, $pre_cache)) {
                                ...
                                if (isset($data['snmp_bulk'])) {
                                    UpdiveNSMConfig::set('os.' . $os->getName() . '.snmp_bulk', (bool) $data['snmp_bulk']);
                                }
                                ...
                                UpdiveNSMConfig::set('os.' . $os->getName() . '.snmp_bulk', $saved_nobulk);
                            }
                        }
                    }
                }
