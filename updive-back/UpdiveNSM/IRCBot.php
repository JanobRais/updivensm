...
class IRCBot
{
    ...
    public function __construct()
    {
        ...
        $this->config = UpdiveNSMConfig::getAll();
    ...
    {
        $container_dir = '/data';
        if (file_exists($container_dir) and posix_getpwuid(fileowner($container_dir))['name'] == 'UpdiveNSM') {
    ...
    private function _auth($params)
    {
        ...
        if (strlen($params[0]) == 64) {
            ...
            if ($user->email && $user->username == $params[0]) {
                ...
                try {
                    Mail::send($user->email, 'UpdiveNSM IRC-Bot Authtoken', "Your Authtoken for the IRC-Bot:\r\n\r\n" . $token . "\r\n\r\n");
    ...
    private function _reload($params)
    {
        if ($this->user['user']->can('irc.reload')) {
            ...
            UpdiveNSMConfig::reload();
            $this->respond('Reloading configuration & defaults');
            if (UpdiveNSMConfig::getAll() != $this->config) {
                $this->__construct();
