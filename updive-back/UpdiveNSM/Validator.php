...
use UpdiveNSM\Interfaces\ValidationGroup;
...
class Validator
{
    ...
    public function __construct()
    {
        ...
        $pattern = $this->getBaseDir() . '/UpdiveNSM/Validations/*.php';

        foreach (glob($pattern) as $file) {
            $class_name = basename($file, '.php');
            $class = '\UpdiveNSM\Validations\\' . $class_name;
    ...
    public function result(ValidationResult $result, ?string $group = null): void
    {
        ...
        if (empty($group)) {
            ...
            $bt = debug_backtrace();
            foreach ($bt as $entry) {
                if (Str::startsWith($entry['class'], 'UpdiveNSM\Validations')) {
                    $group = str_replace('UpdiveNSM\Validations\\', '', $entry['class']);
                    break;
                }
            }
        }
    ...
    /**
     * Execute a command, but don't run it as root.  If we are root, run as the UpdiveNSM user.
    ...
    public function execAsUser(string $command, ?array &$output = null, ?int &$code = null): void
    {
        if (self::getUsername() === 'root') {
            $command = 'su ' . \config('UpdiveNSM.user') . ' -s /bin/sh -c "' . $command . '"';
        }
    ...
    /**
     * Get the base url for this UpdiveNSM install, this will only work from web pages.
    ...
    {
        $url = function_exists('get_url') ? get_url() : UpdiveNSMConfig::get('base_url');
