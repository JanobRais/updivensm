...
if (isset($options['v'])) {
    ...
    UpdiveNSMConfig::set('auth_ad_debug', 1);
    UpdiveNSMConfig::set('auth_ldap_debug', 1);
}

echo 'Authentication Method: ' . UpdiveNSMConfig::get('auth_mechanism') . PHP_EOL;

// if ldap like, check selinux
if (UpdiveNSMConfig::get('auth_mechanism') == 'ldap' || UpdiveNSMConfig::get('auth_mechanism') == 'active_directory') {
...
try {
    ...
    // AD bind tests
    if ($authorizer instanceof \UpdiveNSM\Authentication\ActiveDirectoryAuthorizer) {
        ...
        if (UpdiveNSMConfig::has('auth_ad_binduser') && UpdiveNSMConfig::has('auth_ad_bindpassword')) {
            ...
            if (! $bind_success) {
                ...
                if ($ldap_error == 'Invalid credentials') {
                    print_error('AD bind failed for user ' . UpdiveNSMConfig::get('auth_ad_binduser') . '@' . UpdiveNSMConfig::get('auth_ad_domain') .
