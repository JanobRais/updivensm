...
use UpdiveNSM\Exceptions\InsufficientDataException;
...
class Storage extends DeviceRelatedModel implements Keyable
{
    ...
    public function isValid(string $os): bool
    {
        ...
        foreach (\App\Facades\UpdiveNSMConfig::getCombined($os, 'ignore_mount') as $im) {
        ...
        foreach (\App\Facades\UpdiveNSMConfig::getCombined($os, 'ignore_mount_string') as $ims) {
        ...
        foreach (\App\Facades\UpdiveNSMConfig::getCombined($os, 'ignore_mount_regexp') as $imr) {
        ...
        // filter by type
        foreach (\App\Facades\UpdiveNSMConfig::getCombined($os, 'ignore_mount_type') as $imt) {
