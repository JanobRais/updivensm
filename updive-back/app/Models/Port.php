...
class Port extends DeviceRelatedModel
{
    ...
    /**
     * @return array{ifOperStatus: 'UpdiveNSM\Enum\IfOperStatus', ifOperStatus_prev: 'UpdiveNSM\Enum\IfOperStatus', ifAdminStatus: 'UpdiveNSM\Enum\IfOperStatus', ifAdminStatus_prev: 'UpdiveNSM\Enum\IfOperStatus'}
     */
    ...
    public function getLabel()
    {
        ...
        if (\App\Facades\UpdiveNSMConfig::getOsSetting($os, 'ifname')) {
            $label = $this->ifName;
        } elseif (\App\Facades\UpdiveNSMConfig::getOsSetting($os, 'ifalias')) {
            $label = $this->ifAlias;
        }
        ...
        if (empty($label)) {
            ...
            if (\App\Facades\UpdiveNSMConfig::getOsSetting($os, 'ifindex')) {
                $label .= " $this->ifIndex";
            }
        }

        foreach ((array) \App\Facades\UpdiveNSMConfig::get('rewrite_if', []) as $src => $val) {
            if (Str::contains(strtolower($label), strtolower((string) $src))) {
                $label = $val;
            }
        }

        foreach ((array) \App\Facades\UpdiveNSMConfig::get('rewrite_if_regexp', []) as $reg => $val) {
