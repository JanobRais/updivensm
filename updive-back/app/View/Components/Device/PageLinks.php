...
class PageLinks extends Component
{
    ...
    public function __construct(
        ...
        $primary_device_link_name = UpdiveNSMConfig::get('html.device.primary_link', 'edit');
    ...
    private function deviceLinkMenu(Device $device, string $currentTab): array
    {
        ...
        foreach (array_values(Arr::wrap(UpdiveNSMConfig::get('html.device.links'))) as $index => $link) {
        ...
        $ssh_url = UpdiveNSMConfig::get('gateone.server')
            ? UpdiveNSMConfig::get('gateone.server') . '?ssh=ssh://' . (UpdiveNSMConfig::get('gateone.use_UpdiveNSM_user') ? Auth::user()->username . '@' : '') . $device['hostname'] . '&location=' . $device['hostname']
