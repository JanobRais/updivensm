...
class ErrorReporting
{
    ...
    protected array $upgradable = [
        \UpdiveNSM\Exceptions\FilePermissionsException::class,
        \UpdiveNSM\Exceptions\DatabaseConnectException::class,
        \UpdiveNSM\Exceptions\DuskUnsafeException::class,
        \UpdiveNSM\Exceptions\UnserializableRouteCache::class,
        \UpdiveNSM\Exceptions\MaximumExecutionTimeExceeded::class,
        \UpdiveNSM\Exceptions\DatabaseInconsistentException::class,
    ];
    ...
    public function __construct(Exceptions $exceptions)
    {
        ...
        $exceptions->throttle(fn (Throwable $e) => Limit::perMinute(UpdiveNSMConfig::get('reporting.throttle', 30)));
        $exceptions->reportable($this->reportable(...));
        $exceptions->report($this->report(...));
        $exceptions->render($this->render(...));

        Flare::determineVersionUsing(fn () => \UpdiveNSM\Util\Version::VERSION);
    }
    ...
    public function isReportingEnabled(): bool
    {
        ...
        if (UpdiveNSMConfig::get('reporting.error') !== true) {
        ...
        $git = Git::make(180);
        if ($git->isAvailable()) {
            if (! Str::contains($git->remoteUrl(), ['git@github.com:UpdiveNSM/UpdiveNSM.git', 'https://github.com/UpdiveNSM/UpdiveNSM.git'])) {
                \Log::debug('Reporting disabled because UpdiveNSM is not from the official repository');
