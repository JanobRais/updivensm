...
class SmokepingGenerateCommand extends LnmsCommand
{
    ...
    {
        $probes = $this->assembleProbes(UpdiveNSMConfig::get('smokeping.probes'));
    ...
    public function buildTargetsConfiguration($devices)
    {
        ...
        $targets = $this->buildTargets($smokelist, UpdiveNSMConfig::get('smokeping.probes'), $this->option('single-process'));
        $header = $this->buildHeader($this->option('no-header'), $this->option('compat'));
    ...
    public function assembleProbes($probeCount)
    {
        ...
        return array_merge(
            $this->buildProbes('FPing', self::DEFAULTIP4PROBE, self::IP4PROBE, UpdiveNSMConfig::get('fping'), $probeCount),
            $this->buildProbes('FPing6', self::DEFAULTIP6PROBE, self::IP6PROBE, UpdiveNSMConfig::get('fping6'), $probeCount)
        );
    }
    ...
    {
        if (! UpdiveNSMConfig::has('smokeping.probes') ||
            ! UpdiveNSMConfig::has('fping') ||
            ! UpdiveNSMConfig::has('fping6')
        ) {
        ...
        if (UpdiveNSMConfig::get('smokeping.probes') < 1) {
     ...
     *
     * @param  string  $entry  The UpdiveNSM device hostname to rewrite
     * @return string
     */
