...
class KeyRotate extends LnmsCommand
{
    ...
    {
        if (! \App\Facades\UpdiveNSMConfig::has($key)) {
            return true;
        }

        try {
            $data = $this->decrypt->decryptString(\App\Facades\UpdiveNSMConfig::get($key));
            \App\Facades\UpdiveNSMConfig::persist($key, $this->encrypt->encryptString($data));

            return true;
        } catch (DecryptException) {
            try {
                $this->encrypt->decryptString(\App\Facades\UpdiveNSMConfig::get($key));
