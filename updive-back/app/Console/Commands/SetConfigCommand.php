...
class SetConfigCommand extends LnmsCommand
{
    ...
    public function handle(DynamicConfig $definition): int
    {
        ...
        if (Str::endsWith($setting, '.+')) {
            ...
            $sub_data = UpdiveNSMConfig::get($setting, []);
        ...
        if ($parent && $parent !== $setting) {
            $parent_data = UpdiveNSMConfig::get($parent);
        ...
        if (UpdiveNSMConfig::persist($setting, $value)) {
            return 0;
        }
    ...
    {
        if ($parent) {
            $data = UpdiveNSMConfig::get($parent);
            ...
            return UpdiveNSMConfig::persist($parent, $data);
        }

        return UpdiveNSMConfig::erase($setting);
    }
    ...
    private function validateOsSetting(string $os, string $setting, $value)
    {
        ...
        $os_data = UpdiveNSMConfig::get("os.$os");
        if ($os_data === null) {
