...
use UpdiveNSM\Cache\PermissionsCache;
use UpdiveNSM\Util\IP;
use UpdiveNSM\Util\Validate;
...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function register(): void
    {
        ...
        $this->app->singleton('device-cache', fn () => new \UpdiveNSM\Cache\Device());
        $this->app->singleton('port-cache', fn () => new \UpdiveNSM\Cache\Port());
        $this->app->singleton('git', fn () => new \UpdiveNSM\Util\Git());

        $this->app->bind(\App\Models\Device::class, function (Application $app) {
            /** @var \UpdiveNSM\Cache\Device $cache */
    ...
    public function boot(): void
    {
        ...
        Password::defaults(function () {
            $validation = Password::min(UpdiveNSMConfig::get('password.min_length', 8));

            if (UpdiveNSMConfig::get('password.uncompromised', true)) {
    ...
    {
        Blade::if('config', fn ($key, $value = true) => UpdiveNSMConfig::get($key) == $value);
        Blade::if('notconfig', fn ($key) => ! UpdiveNSMConfig::get($key));
        Blade::if('admin', fn () => auth()->check() && auth()->user()->hasRole('admin')); // TODO remove

        Blade::directive('deviceUrl', fn ($arguments) => "<?php echo \UpdiveNSM\Util\Url::deviceUrl($arguments); ?>");

        // Graphing
        Blade::directive('signedGraphUrl', fn ($vars) => "<?php echo \UpdiveNSM\Util\Url::forExternalGraph($vars); ?>");

        Blade::directive('signedGraphTag', fn ($vars) => "<?php echo '<img class=\"UpdiveNSM-graph\" src=\"' . \UpdiveNSM\Util\Url::forExternalGraph($vars) . '\" />'; ?>");

        Blade::directive('graphImage', fn ($vars, $flags = 0) => "<?php echo \UpdiveNSM\Util\Graph::getImageData($vars, $flags); ?>");
 ...
 ?>");
    ...
    {
        $sensor_types = [];
        foreach (\UpdiveNSM\Enum\Sensor::values() as $sensor_type) {
    ...
    {
        $this->app->alias(\UpdiveNSM\Interfaces\Geocoder::class, 'geocoder');
        $this->app->bind(\UpdiveNSM\Interfaces\Geocoder::class, function ($app) {
            $engine = UpdiveNSMConfig::get('geoloc.engine');
