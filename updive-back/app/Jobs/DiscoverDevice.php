...
EOH, $this->device->hostname, $os_group ? " ($os_group)" : '', $this->device->device_id, $this->device->os, $this->device->ip));
            ...
            try {
                ...
                if ($should_discover) {
                    ...
                    if ($module_status->hasSubModules()) {
                        UpdiveNSMConfig::set('discovery_submodules.' . $module, $module_status->submodules);
                    }
    ...
    private function handleOsChange(OS $os): OS
    {
        ...
        $this->deviceArray['os_group'] = UpdiveNSMConfig::getOsSetting($this->device->os, 'group');
        $os = OS::make($this->deviceArray);

        Log::info('OS Changed ');
        Log::notice('OS: ' . UpdiveNSMConfig::getOsSetting($this->device->os, 'text') . " ({$this->device->os})\n");
