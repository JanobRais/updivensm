...
class PollDevice implements ShouldQueue, ShouldBeUnique
{
    ...
    public function handle(): void
    {
        ...
        if ($measurement->getDuration() > UpdiveNSMConfig::get('rrd.step')) {
            Eventlog::log('Polling took longer than ' . round(UpdiveNSMConfig::get('rrd.step') / 60, 2) .
            ...
            try {
                ...
                if ($should_poll) {
                    ...
                    if ($module_status->hasSubModules()) {
                        UpdiveNSMConfig::set('poller_submodules.' . $module, $module_status->submodules);
                    }
    ...
    private function initDevice(): void
    {
        ...
        if ($os_group = UpdiveNSMConfig::get("os.{$this->device->os}.group")) {
...
EOH, $this->device->hostname, $os_group ? " ($os_group)" : '', $this->device->device_id, $this->device->os, $this->device->ip));
    ...
    private function initRrdDirectory(): void
    {
        ...
        if (UpdiveNSMConfig::get('rrd.enable', true) && ! is_dir($host_rrd)) {
            try {
                mkdir($host_rrd);
