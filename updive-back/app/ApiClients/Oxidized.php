...
class Oxidized extends BaseApi
{
    ...
    {
        $this->timeout = 90;
        $this->base_uri = UpdiveNSMConfig::get('oxidized.url') ?? '';
        $this->enabled = UpdiveNSMConfig::get('oxidized.enabled') === true && $this->base_uri;
    }

    /**
     * Ask oxidized to refresh the node list for the source (likely the UpdiveNSM API).
     */
    public function reloadNodes(): void
    {
        if ($this->enabled && UpdiveNSMConfig::get('oxidized.reload_nodes') === true) {
            try {
