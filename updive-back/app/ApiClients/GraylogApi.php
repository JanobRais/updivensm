...
class GraylogApi
{
    ...
    {
        if (version_compare(UpdiveNSMConfig::get('graylog.version', '2.4'), '2.1', '>=')) {
            $this->api_prefix = '/api';
        }

        $base_uri = UpdiveNSMConfig::get('graylog.server');
        if ($port = UpdiveNSMConfig::get('graylog.port')) {
            $base_uri .= ':' . $port;
        }

        $this->client = Http::client()
            ->baseUrl($base_uri)
            ->withBasicAuth(UpdiveNSMConfig::get('graylog.username'), UpdiveNSMConfig::get('graylog.password'))
            ->acceptJson();
    }
    ...
    public function query(string $query = '*', int $range = 0, int $limit = 0, int $offset = 0, ?string $sort = null, ?string $filter = null): array
    {
        ...
        $uri = UpdiveNSMConfig::get('graylog.base_uri');
        if (! $uri) {
    ...
    {
        $field = UpdiveNSMConfig::get('graylog.query.field');
        $query = [];
        if ($search) {
    ...
    public function getAddresses(Device $device): \Illuminate\Support\Collection
    {
        ...
        if (UpdiveNSMConfig::get('graylog.match-any-address')) {
    ...
    {
        return (bool) UpdiveNSMConfig::get('graylog.server');
    }
}
