...
class SocialiteController extends Controller
{
    ...
    public function redirect(Request $request, string $provider)
    {
        ...
        if ($driver instanceof \Laravel\Socialite\Two\AbstractProvider) {
            $scopes = UpdiveNSMConfig::get('auth.socialite.scopes');
    ...
    {
        if (! UpdiveNSMConfig::get('auth.socialite.register', false)) {
            return;
        }
        ...
        $default_role = UpdiveNSMConfig::get('auth.socialite.default_role');
    ...
    {
        $scopes = UpdiveNSMConfig::get('auth.socialite.scopes');
        $claims = UpdiveNSMConfig::get('auth.socialite.claims');
        ...
        $claimField = UpdiveNSMConfig::get("auth.socialite.configs.$provider.claim_field");
    ...
    /**
     * Take the config from UpdiveNSM Config, and insert it into Laravel Config
     */
    private function injectConfig(): void
    {
        foreach (UpdiveNSMConfig::get('auth.socialite.configs', []) as $provider => $config) {
