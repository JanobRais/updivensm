...
class AlertRuleTemplateController extends Controller
{
    public function template(int $template_id)
    {
        ...
        return response()->json([
            ...
            'severity' => $rule['severity'] ?? UpdiveNSMConfig::get('alert_rule.severity'),
            'invert_map' => UpdiveNSMConfig::get('alert_rule.invert_map'),
            'alert_operation_id' => null,
            'operations' => [],
            'default_operation_step_duration_seconds' => max(0, 60 * (int) UpdiveNSMConfig::get('alert_rule.default_operation_step_duration', UpdiveNSMConfig::get('alert_rule.interval'))),
        ]);
    }
    ...
    public function rule(AlertRule $alertRule)
    {
        ...
        return response()->json([
            ...
            'severity' => $alertRule->severity ?: UpdiveNSMConfig::get('alert_rule.severity'),
    ...
    {
        $default_extra = [
            'invert' => UpdiveNSMConfig::get('alert_rule.invert_rule_match'),
            'recovery' => UpdiveNSMConfig::get('alert_rule.recovery_alerts'),
            'acknowledgement' => UpdiveNSMConfig::get('alert_rule.acknowledgement_alerts'),
        ];
