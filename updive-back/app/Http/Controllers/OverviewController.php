...
class OverviewController extends Controller
{
    ...
    {
        $view = UpdiveNSMConfig::get('front_page');
    ...
    public function simple(Request $request)
    {
        ...
        $devices_down = Device::hasAccess(Auth::user())
            ->isDown()
            ->limit(UpdiveNSMConfig::get('front_page_down_box_limit'))
            ->get();

        if (UpdiveNSMConfig::get('warn.ifdown')) {
        ...
        if (filter_var(UpdiveNSMConfig::get('uptime_warning'), FILTER_VALIDATE_FLOAT) !== false
            && UpdiveNSMConfig::get('uptime_warning') > 0
        ) {
            $devices_uptime = Device::hasAccess(Auth::user())
                ->isUp()
                ->whereUptime(UpdiveNSMConfig::get('uptime_warning'))
                ->limit(UpdiveNSMConfig::get('front_page_down_box_limit'))
                ->get();

            $devices_uptime = $devices_uptime->reject(fn ($device) => UpdiveNSMConfig::getOsSetting($device->os, 'bad_uptime') == true);
        }

        if (UpdiveNSMConfig::get('enable_syslog')) {
