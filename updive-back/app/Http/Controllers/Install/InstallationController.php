...
use UpdiveNSM\Interfaces\InstallerStep;
...
class InstallationController extends Controller
{
    ...
    protected function configureDatabase()
    {
        ...
        if (! empty($db)) {
            Eloquent::setConnection(
                $this->connection,
                $db['host'] ?? 'localhost',
                $db['username'] ?? 'UpdiveNSM',
                $db['password'] ?? null,
                $db['database'] ?? 'UpdiveNSM',
                $db['port'] ?? 3306,
                $db['socket'] ?? null
            );
    ...
    {
        if (is_string(config('UpdiveNSM.install'))) {
            $this->steps = array_intersect_key($this->steps, array_flip(explode(',', config('UpdiveNSM.install'))));
        }
