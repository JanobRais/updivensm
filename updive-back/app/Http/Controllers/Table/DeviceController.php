...
class DeviceController extends TableController
{
    ...
    {
        $os_text = htmlspecialchars(UpdiveNSMConfig::getOsSetting($device->os, 'text'));
    ...
    private function getActions(Device $device): array
    {
        ...
        if ($server = UpdiveNSMConfig::get('gateone.server')) {
            $ssh_href = UpdiveNSMConfig::get('gateone.use_UpdiveNSM_user')
        ...
        foreach (array_values(Arr::wrap(UpdiveNSMConfig::get('html.device.links'))) as $index => $custom) {
    ...
    protected function formatExportRow($device)
    {
        ...
        return [
            ...
            'os' => UpdiveNSMConfig::getOsSetting($device->os, 'text', $device->os),
