...
class MapDataController extends Controller
{
    ...
    protected function linkUseColour(float $link_pct): string
    {
        ...
        return UpdiveNSMConfig::get("network_map_legend.$link_pct", '#000000');
    }
    ...
    protected function nodeDisabledStyle(): array
    {
        return [
            'color' => [
                'highlight' => [
                    'background' => UpdiveNSMConfig::get('network_map_legend.di.node'),
                ],
                'border' => UpdiveNSMConfig::get('network_map_legend.di.border'),
                'background' => UpdiveNSMConfig::get('network_map_legend.di.node'),
            ],
            'borderWidth' => null,
        ];
    ...
    protected function nodeHighlightStyle(): array
    {
        return [
            'color' => [
                'highlight' => [
                    'border' => UpdiveNSMConfig::get('network_map_legend.highlight.border'),
                ],
                'border' => UpdiveNSMConfig::get('network_map_legend.highlight.border'),
            ],
            'borderWidth' => UpdiveNSMConfig::get('network_map_legend.highlight.borderWidth'),
        ];
    }
    ...
    protected function nodeDownStyle(): array
    {
        return [
            'color' => [
                'highlight' => [
                    'background' => UpdiveNSMConfig::get('network_map_legend.dn.node'),
                    'border' => UpdiveNSMConfig::get('network_map_legend.dn.border'),
                ],
                'border' => UpdiveNSMConfig::get('network_map_legend.dn.border'),
                'background' => UpdiveNSMConfig::get('network_map_legend.dn.node'),
            ],
    ...
    public function getDevices(Request $request): JsonResponse
    {
        ...
        foreach (self::deviceList($request) as $device) {
            if ($device->status) {
                $updowntime = \UpdiveNSM\Util\Time::formatInterval($device->uptime);
            } elseif ($device->last_polled) {
                $updowntime = \UpdiveNSM\Util\Time::formatInterval(time() - strtotime((string) $device->last_polled));
            } else {
                $updowntime = '';
            }

            $deviceTypes = collect(UpdiveNSMConfig::get('device_types', []))->pluck('icon', 'type');
                ...
                foreach ($port->{$remote_port_attr} as $remote_port) {
                    ...
                    if ($port->device->status == 0 && $remote_port->device->status == 0) {
                        ...
                        $link_style = [
                            ...
                            'color' => [
                                'border' => UpdiveNSMConfig::get('network_map_legend.dn.border'),
                                'highlight' => UpdiveNSMConfig::get('network_map_legend.dn.edge'),
                                'color' => UpdiveNSMConfig::get('network_map_legend.dn.edge'),
                            ],
                        ];
    ...
    public function getServices(Request $request): JsonResponse
    {
        ...
        foreach ($services->get() as $service) {
            if ($service->device->status) {
                $updowntime = \UpdiveNSM\Util\Time::formatInterval($service->device->uptime);
            } elseif ($service->device->last_polled) {
                $updowntime = \UpdiveNSM\Util\Time::formatInterval(time() - strtotime((string) $service->device->last_polled));
            } else {
                $updowntime = '';
            }
            ...
            $service_list[] = [
                ...
                'updowntime' => $updowntime,
                'compact' => UpdiveNSMConfig::get('webui.availability_map_compact'),
                'box_size' => UpdiveNSMConfig::get('webui.availability_map_box_size'),
            ];
        }
