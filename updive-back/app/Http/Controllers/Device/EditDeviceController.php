...
use UpdiveNSM\Exceptions\HostRenameException;
...
class EditDeviceController
{
    ...
    {
        $types = collect(UpdiveNSMConfig::get('device_types'))->keyBy('type');
        ...
        return view('device.edit.device', [
            ...
            'types' => $types,
            'default_type' => UpdiveNSMConfig::getOsSetting($device->os, 'type'),
            'parents' => $device->parents()->pluck('hostname', 'device_id'),
            'poller_groups' => PollerGroup::orderBy('group_name')->pluck('group_name', 'id'),
            'default_poller_group' => UpdiveNSMConfig::get('distributed_poller_group'),
            ...
            'default_maintenance_behavior' => MaintenanceBehavior::from((int) UpdiveNSMConfig::get('alert.scheduled_maintenance_default_behavior'))->value,
