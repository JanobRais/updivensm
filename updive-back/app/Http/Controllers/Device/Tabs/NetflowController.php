...
class NetflowController implements DeviceTab
{
    ...
    {
        if (UpdiveNSMConfig::get('nfsen_enable')) {
            foreach ((array) UpdiveNSMConfig::get('nfsen_rrds', []) as $nfsenrrds) {
                if ($nfsenrrds[strlen((string) $nfsenrrds) - 1] != '/') {
                    $nfsenrrds .= '/';
                }

                $nfsensuffix = UpdiveNSMConfig::get('nfsen_suffix', '');

                if (UpdiveNSMConfig::get('nfsen_split_char')) {
                    $basefilename_underscored = preg_replace('/\./', UpdiveNSMConfig::get('nfsen_split_char'), $device->hostname);
                } else {
