...
class ModuleController extends Controller
{
    public function update(Device $device, string $module, Request $request): JsonResponse
    {
        ...
        return response()->json([
            'discovery' => (bool) $device->getAttrib('discover_' . $module, UpdiveNSMConfig::getCombined($device->os, 'discovery_modules')[$module] ?? false),
            'polling' => (bool) $device->getAttrib('poll_' . $module, UpdiveNSMConfig::getCombined($device->os, 'poller_modules')[$module] ?? false),
        ]);
    }
