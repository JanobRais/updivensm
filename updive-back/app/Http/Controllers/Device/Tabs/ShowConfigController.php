...
use UpdiveNSM\Interfaces\UI\DeviceTab;
...
class ShowConfigController extends Controller implements DeviceTab
{
    ...
    {
        return UpdiveNSMConfig::get('oxidized.enabled') === true
                && UpdiveNSMConfig::has('oxidized.url')
                && $device->getAttrib('override_Oxidized_disable') !== 'true'
                && ! in_array($device->type, UpdiveNSMConfig::get('oxidized.ignore_types', []))
                && ! in_array($device->os, UpdiveNSMConfig::get('oxidized.ignore_os', []));
    }
    ...
    {
        if (UpdiveNSMConfig::has('rancid_configs') && ! is_array(UpdiveNSMConfig::get('rancid_configs'))) {
            UpdiveNSMConfig::set('rancid_configs', (array) UpdiveNSMConfig::get('rancid_configs', []));
        }

        if (UpdiveNSMConfig::has('rancid_configs.0')) {
            $device = DeviceCache::getPrimary();
            foreach (UpdiveNSMConfig::get('rancid_configs') as $configs) {
                if (UpdiveNSMConfig::get('rancid_repo_type') == 'git-bare') {
                    ...
                    if (! empty(UpdiveNSMConfig::get('mydomain')) && in_array($configPath . $device['hostname'] . '.' . UpdiveNSMConfig::get('mydomain'), $config_files)) {
                        return $configPath . $device['hostname'] . '.' . UpdiveNSMConfig::get('mydomain');
                    }
                } else {
                    ...
                    if (is_file($configs . $device['hostname'])) {
                        ...
                        if (! empty(UpdiveNSMConfig::get('mydomain'))) { // Try with domain name if set
                            if (is_file($configs . $device['hostname'] . '.' . UpdiveNSMConfig::get('mydomain'))) {
                                $this->rancidPath = $configs;

                                return $configs . $device['hostname'] . '.' . UpdiveNSMConfig::get('mydomain');
                            }
                        }
                    }
                }
